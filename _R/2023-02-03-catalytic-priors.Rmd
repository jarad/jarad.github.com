...
Of course, in this simple example, we could just use $\psi$ instead of $\hat{p}_\tilde{y}$. 
...
&= (\sigma^2)^{-(\tau+n)/2}\exp(-\frac{1}{2\sigma^2}([\tau + n]\mu^2-2\mu[\tau\overline{\tilde{y}}+n\overline{y}]))
\end{array}.$$

Thus, $\mu|y,\sigma^2 \sim N\left(\frac{\tau\overline{\tilde{y}}+n\overline{y}]}{\tau+n}, \sigma^2/[\tau+n]\right)$ 
and $\sigma^2|y \sim IG([\tau+n]/2, ?)$.
