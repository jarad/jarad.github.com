...
sample mean, so $\hat\mu = overline{y}$
\pause
\item What is our estimator for $\sigma^2$?
\pause
...
At level $\alpha$, you 
\begin{itemize}
\item reject $H_0$ if \pvalue{} $\le \alpha$ and conclude that there is 
statistically significant evidence that $\mu > 0$ 
\item fail to reject $H_0$ if \pvalue{} $> \alpha$ and conclude that there is 
...
\[ 
SE[\hat\beta_1] = SE[b_1] = \hat\sigma \sqrt{\frac{1}{(n-1)s_x^2}}
\]
and 
\[ 
...
\[ 
t = \frac{\hat\beta_1 - 0}{SE[\hat\beta_1]} \pause = \frac{b_1 - 0}{SE[b_1]}
\]
\pause
and a \pvalue{} is 
\[ 
