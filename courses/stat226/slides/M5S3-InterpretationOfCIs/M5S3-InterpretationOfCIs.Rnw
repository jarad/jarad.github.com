...
random variables $X_1,X_2,\ldots$ where $I_i$ is the indicator of the event, i.e.
\[
...
Then 
\[
\lim_{i\to\infty} \frac{I_i}{i} = 1-\alpha.
\]
\pause
<<fig.width=12>>=
...
false sense for that you have actually learned.
\pause
