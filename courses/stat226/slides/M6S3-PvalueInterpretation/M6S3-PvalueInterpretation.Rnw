...
\begin{itemize}[<+->]
\item $H_a: \mu< m_0$ implies \pvalue{} = $\phantom{2}P(T_{n-1} < t),$
...
Assume $X_i\iid N(\mu,\sigma^2)$ and $H_0: \mu=30$.
\pause 
From a random sample with 30 observations you find $\overline{x} = 35.2$ and 
$s=11.57$ 
\pause 
which results in 
\[ 
t = \frac{35.2-30}{11.57/\sqrt{30}} \approx 2.462.
\]
\pause 
...
P(T_{n-1} < -1.4089) &= \pause 0.0838 \\
P(T_{n-1} > 1.4089 \mbox{ or } T_{n-1} < -1.4089) &=  0.1677
\end{array} \]
\end{frame}
...
\hline
$H_a: \mu \ne m_0$ & 0.0522 & Fail to reject null hypothesis \\
$H_a: \mu > m_0$   & 0.9739 & Fail to reject null hypothesis \\
$H_a: \mu < m_0$   & 0.0261 & Reject null hypothesis\\
\end{tabular}
\end{center}
\end{frame}
...
Alternative Hypothesis & \pvalue{} & Conclusion \\
\hline
$H_a: \mu \ne m_0$ & 0.0522 & Reject null hypothesis \\
$H_a: \mu > m_0$   & 0.9739 & Fail to reject null hypothesis \\
$H_a: \mu < m_0$   & 0.0261 & Reject null hypothesis\\
\end{tabular}
\end{center}
...
The \alert{significance level $\alpha$ is the Type I error}.
