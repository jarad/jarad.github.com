...
\begin{frame}
\frametitle{Confidence interval for population mean}

Recall that due to the CLT, $\overline{X} \sim N(\mu,\sigma^2/n)$ 
\pause 
where
\begin{itemize}[<+->]
...
probability (usually written as a proportion). 
...
\pause
Then $E[X_i]=\mu$ is unknown, \pause but $SD[X_i] = \sigma = \$20$ is known. 
\pause 
We obtained a sample of size $n=64$ with a sample mean $\overline{x}=\$105$. 
\pause 
To obtain the 68\% confidence interval, we calculate
\[ \begin{array}{rl} 
\overline{x} \pm \frac{\sigma}{\sqrt{n}} = \pause 
(\overline{x} - \frac{\sigma}{\sqrt{n}},\overline{x} + \frac{\sigma}{\sqrt{n}}) =
\end{array} 
\]
...
\begin{frame}
\frametitle{General Confidence Intervals}

Suppose you have a \alert{significance level} $\alpha$ 
\pause which you can convert to a \alert{confidence level} $C=100(1-\alpha)\%$
and obtain a \alert{z critical value} $z_{\alpha,2}$
\end{frame}
