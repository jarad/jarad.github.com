...
\pause
You obtain a \alert{random sample} of observations from the population and 
...
Construct a 80\% confidence interval for the mean monthly balance. 
...
Then we calculate
\[ 
\overline{x} \pm \frac{\sigma}{\sqrt{n}} = 105 \pm 1.28 \frac{20}{\sqrt{64}}
= (101.8,108.2)
\]
\pause
