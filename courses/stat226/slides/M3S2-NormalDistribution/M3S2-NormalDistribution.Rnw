...
P(Y< c)   &= \phantom{1-{}} P(Y>-c) & \mbox{symmetric around 0}
\end{array} \]
...
\pause &= P\left( \frac{Y-5}{3} < -0.33 \right) & \mbox{symmetric around 0} \\
...
Let $Y$ be the amount of wheat flour purchased this month and assume 
$Y\sim N(20,4^2)$. 
Then 
\[ \begin{array}{rl}
P(Y>22) 
&= P\left( \frac{Y-20}{3} > \frac{25-20}{4} \right) \\
&= P(Z>1.25) \\
&= P(Z<-1.25) \\
&= 0.1056
\end{array} \]
...
\[ P(\mu-c\cdot \sigma < Y < \mu+c\cdot \sigma)  = 
P\left(-c < \frac{Y-\mu}{\sigma} \right) = P(-c < Z < c) \]
where $c$ is the number of standard deviations $Y$ is from the mean.
