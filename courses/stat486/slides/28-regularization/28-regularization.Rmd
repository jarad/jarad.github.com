...
```{r}
m_ridge <- MASS::lm.ridge(lprice ~ ., data = train, lambda = 0.035)
```

To make predictions, we need to center and scale our explanatory variables

```{r}
xtest <- scale(subset(test, select=-c(lprice)), 
               center = m_ridge$xm, scale = m_ridge$scales)
```

```{r}
p <- xtest %*% m_ridge$coef +   # X*beta
  m_ridge$ym                    # need to include mean
```
