...
\begin{frame}
\frametitle{Parameter estimation and inferece}
...
\quad\mbox{and}\quad
\mbox{pvalue} = 2P\left(T_{n-p}>\left|\frac{\hat\beta_j-b_j}{SE(\hat\beta_j)}\right|\right)
\]
where $T_{n-p} \sim t_{n-p}$ and $SE(\hat\beta_j)$ is the $j$th diagonal element of 
