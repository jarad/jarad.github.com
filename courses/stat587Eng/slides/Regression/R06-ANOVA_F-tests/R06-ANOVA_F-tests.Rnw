...
\frametitle{Calculating the sum of squared residuals (errors)}
  ...
  \[ \begin{array}{|l|c|c|}
	...
	\mbox{SSE} & 
	\uncover<5->{\hat{\sigma}^2 = \sum_{j=1}^{J} \sum_{i=1}^{n_j} r_{ij}^2}  & 
...
\small
	...
	\[ \mbox{F} = 
	\frac{\mbox{Extra sum of squares / Extra degrees of freedom}}{\mbox{Estimated residual variance in full model }(\hat\sigma^2)} \] \pause 
