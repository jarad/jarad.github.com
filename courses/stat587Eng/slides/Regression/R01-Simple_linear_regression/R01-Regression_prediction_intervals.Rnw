...
\title{Regression: uncertainty and prediction intervals}
...
\pm t_{n-2,1-a/2} \, \hat{\sigma}\sqrt{\frac{1}{n} + \frac{(\overline{X}^2-x)}{(n-1)s_x^2}}. \]
\nc\ec
\end{frame}
...
\bc
\[ \hat\beta_0 + \hat\beta_1 x \pause \,
\pm t_{n-2,1-a/2} \, \hat{\sigma}\sqrt{\alert{1+}\frac{1}{n} + \frac{(\overline{X}^2-x)}{(n-1)s_x^2}}. \]
\nc\ec
\end{frame}
