...
\title{Regression: choosing explanatory variables}
...
\item logarithmic: $f(x) = \log(x)$
\item centered: $f(x) = x - m$
\item scaled: $f(x) = x/s$
\end{itemize}
...
\]
\pause
then the new intercept ($\tilde\beta_0$) is the expected response when the 
...
\begin{itemize}
\item intercept $\tilde{\beta}_0 = (\beta_0-m\beta_1)$\pause and
...
If we use
\[ f(x) = x/s, \]
then the new slope, $\tilde\beta_1$, is the expected increase in the response when
...
\begin{itemize}
\item intercept $\tilde{\beta}_0 = \beta_0$ (unchanged)\pause and
