...
SXX &
= (n-1) s_x^2 
= (\Sexpr{sm$n}-1)\times \Sexpr{sm$s_X}^2 
= \Sexpr{(sm$n-1)*sm$s_X^2} \\

SYY &
= (n-1) s_Y^2 
= (\Sexpr{sm$n}-1)\times \Sexpr{sm$s_Y}^2 
= \Sexpr{(sm$n-1)*sm$s_Y^2} \\

SXY &
= (n-1) s_X s_Y r_{XY} 
= (\Sexpr{sm$n}-1) \times \Sexpr{sm$s_X} \times \Sexpr{sm$s_Y} \times \Sexpr{sm$r_XY} 
= \Sexpr{(sm$n-1)*sm$s_X*sm$s_Y*sm$r_XY} \\
...
= \Sexpr{sm$Ybar} - (\Sexpr{beta1})\times \Sexpr{sm$Xbar} 
= \Sexpr{beta0} \\
...
=  \Sexpr{sigma} \sqrt{\frac{1}{\Sexpr{sm$n}} 
  + \frac{\Sexpr{sm$Xbar}^2}{(\Sexpr{sm$n}-1)*\Sexpr{sm$s_X}^2}} 
= \Sexpr{SE_beta0} \\

SE(\hat{\beta}_1) &
= \hat{\sigma}\sqrt{\frac{1}{(n-1)s_x^2}} 
=  \Sexpr{sigma} \sqrt{\frac{1}{(\Sexpr{sm$n}-1)*\Sexpr{sm$s_X}^2}} 
= \Sexpr{SE_beta1} \\

p_{H_0:\beta_0=0} &
= 2P\left(T_{n-2} < -\left| \frac{\hat\beta_0}{SE(\hat\beta_0)} \right|\right) 
= 2P(t_{\Sexpr{sm$n-2}} < \Sexpr{T0}) 
= \Sexpr{2*pt(T0,sm$n-2)} \\

p_{H_0:\beta_1=0} &
= 2P\left(T_{n-2} < -\left| \frac{\hat\beta_1}{SE(\hat\beta_1)} \right|\right) 
= 2P(t_{\Sexpr{sm$n-2}} < \Sexpr{T1}) 
= \Sexpr{2*pt(T1,sm$n-2)} \\

CI_{95\%\, \beta_0} &
= \hat{\beta}_0 \pm t_{n-2,1-a/2} SE(\hat{\beta}_0) \\
&= \Sexpr{beta0} \pm \Sexpr{qt(.975,sm$n-2)}\times  \Sexpr{SE_beta0} 
= (\Sexpr{beta0 - qt(.975,sm$n-2) * SE_beta0}, 
   \Sexpr{beta0 + qt(.975,sm$n-2) * SE_beta0}) \\

CI_{95\%\, \beta_1} &
= \hat{\beta}_1 \pm t_{n-2,1-a/2} SE(\hat{\beta}_1) \\
&= \Sexpr{beta1} \pm \Sexpr{qt(.975,sm$n-2)}\times  \Sexpr{SE_beta1} 
= (\Sexpr{beta1 - qt(.975,sm$n-2) * SE_beta1}, 
   \Sexpr{beta1 + qt(.975,sm$n-2) * SE_beta1}) \\
\end{array} \]
\end{frame}
