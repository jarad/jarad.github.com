...
This design allowed us to examine the importance of ... stress (...number of years since a child's diagnosis [of a chronic disease]).
...
g +  geom_segment(aes(xend = years, yend = predict), 
...
\small
	...
	\bc
	We quantify this uncertainty using their posterior scale parameters:
	\[ \begin{array}{rlll}
...
\[ 
\hat{\beta}_p \pm t_{n-2,1-a/2} SE(\hat\beta_p)
\]
\pause
...
\bc
We can compute \pvalue{}s, \pause \\
e.g. $H_0:\beta_p\ge b_p$ vs $H_A: \beta_p < b_p$ has
\[ 
\mbox{\pvalue{}} = 
P\left(T_{n-2}<\frac{\hat{\beta_p}-b_p}{SE(\hat\beta_p)}\right) 
\]
\pause
and $H_0:\beta_p\le b_p$ vs $H_A: \beta_p > b_p$ has
\[
\mbox{\pvalue{}} = 
P\left(T_{n-2}>\frac{\hat{\beta_1}-b_p}{SE(\hat\beta_p)}\right)
\]
\pause
software default is usually $b_p = 0$.
\nc\ec
\end{frame}
...
\[ 
\beta_p|y \sim t_{n-2}(\hat\beta_p, SE(\hat\beta_p)^2).
\]
\pause
We can construct $100(1-a)\%$ two-sided credible intervals via
\[ 
\hat{\beta}_p \pm t_{n-2,1-a/2} SE(\hat\beta_p). 
\]
\pause
...
\[ \begin{array}{rl}
P(\beta_p<b_p|y) &= P\left(T_{n-2}<\frac{\hat{\beta_p}-b_p}{SE(\hat\beta_p)}\right) \\
P(\beta_1>b_p|y) &= P\left(T_{n-2}>\frac{\hat{\beta_p}-b_p}{SE(\hat\beta_p)}\right)
\end{array} \]
\pause
...
For a generic parameter $\beta$, you can convert between one-sided and 
two-sided \pvalue{}s as well as posterior probabilities using the following
formulas.

\vspace{0.1in} \pause

If $\hat\beta>0$ and $p$ is the \pvalue{} for the test $H_A:\beta \ne 0$, 
\pause
then 
\begin{itemize}[<+->]
\item $\phantom{1-{}}p/2$ is the \pvalue{} for the test $H_A:\beta> 0$,
\item $1-p/2$  is the \pvalue{} for the test $H_A:\beta< 0$,
