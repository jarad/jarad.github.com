...
\title{I02 - Maximum likelihood estimators}
...
\[ p(y|\theta) \]
\pause
where the parameter (vector) $\theta$ is unknown. 
\pause
The 
\begin{itemize}
...
\begin{itemize}
\item $\mathcal{S} = \{0,1,2,\ldots,n\}$  \pause
...
Suppose our data are
\begin{itemize}\small
\item a set of real numbers, i.e. between $-\infty$ and $\infty$, \pause
\item the population mean is $\mu$ and population variance is $\sigma^2$, \pause
\item a histogram is reasonably approximated by a bell-shaped curve, \pause
...
ggplot(d, aes(mu,sigma^2,z=density)) +
  geom_contour() +
