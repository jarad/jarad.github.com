...
\alert{Sample}: students in STAT 587-2

\vspace{0.1in} \pause

\alert{Statistics}: proportion of those students that have Gigabit routers. 
\nc\ec
\end{frame}
...
\alert{Statistics}: proportion that have Gigabit routers. 
\nc\ec
\end{frame}
