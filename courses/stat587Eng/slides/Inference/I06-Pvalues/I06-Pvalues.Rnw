...
@
\pause 
\item $P(Y \le y) \pause = 1-P(Y < y) \pause = 1-P(Y \le y-1)$: \pause
<<echo=TRUE>>=
...
\pause
\[
t = \frac{\overline{y}-3}{s/\sqrt{n}} = \Sexpr{round(t,2)}.
\]
...
\item its sampling distribution \emph{when the model is true is $T \sim \pause t_{\Sexpr{n-1}}$}, \pause and
\item there are three \emph{as or more extreme regions}:
  \begin{itemize}
  \item $T \le t$ \pause
  \item $T \ge t$ \pause
...
@
\pause 
\item $P(T \ge t) \pause = 1-P(Y < y) \pause = 1-P(Y \le y)$: \pause
<<echo=TRUE>>=
1-pt(t, df = n-1)
@
\end{itemize}
