...
dinvgamma <- function(x, shape, scale = 1) {
  dgamma(1/x, shape = shape, rate = scale)/x
}
...
$E[\sigma^2|y] = \Sexpr{round(post_mean_var)}$.
% \pause
% <<echo=TRUE>>=
...
(\Sexpr{round(mean_ci[1])},\Sexpr{round(mean_ci[2])}).
...
(\Sexpr{round(var_ci[1])},\Sexpr{round(var_ci[2])}).
\nc\ec
\end{frame}
...
We found
\begin{itemize} \small
\item $E[\sigma^2|y] = \Sexpr{round(post_mean_var)}$
\item 95\% CI for $\sigma^2$ is $(\Sexpr{paste(round(var_ci), sep=", ")})$
\end{itemize}
but these values have units of (bushels/acre)$^2$.

\vspace{0.05in} \pause

The standard deviation $\sigma = \sqrt{\sigma^2}$ would have units
(bushels/acre).
...
Find the density through transformations of random variables.
\pause
In R code,
<<echo=TRUE>>=
...
So the posterior median for $\sigma$ is \Sexpr{round(sd_median)}.
