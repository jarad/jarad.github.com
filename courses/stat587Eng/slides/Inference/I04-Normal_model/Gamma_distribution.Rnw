...
The random variable $X$ has a \alert{gamma distribution} with 
\alert{shape parameter $\alpha > 0$} and 
\alert{rate parameter $\lambda > 0$} 
\pause
if its probability density function is 
\[ 
f(x) = \frac{\lambda^\alpha}{\Gamma(\alpha)} x^{\alpha-1} e^{-\lambda x} \, \mathrm{I}(x>0).
\]
\pause
\bc
