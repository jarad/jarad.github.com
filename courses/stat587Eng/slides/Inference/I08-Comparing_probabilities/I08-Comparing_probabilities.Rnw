...
\title{I08 - Comparing probabilities}
...
\begin{itemize}
\item Frequentist: based on asymptotic distribution of $\frac{Y_1}{n_1} - \frac{Y_2}{n_2}$:
  \begin{itemize}
...
\item Frequentist: based on distribution of $Y_1,\ldots,Y_G$
  \begin{itemize}
  \item pvalue for a hypothesis test, e.g. $H_0: \theta_g=\theta$ for all $g$, \pause
  \item pvalue for a hypothesis test, e.g. $H_0: \theta_g=\theta_{g'}$ for specific $g$ and $g'$, \pause
  \item confidence interval for $\theta_g-\theta_{g'}$ for specific $g$ and $g'$, \pause
  \end{itemize}
\item Bayesian: based on posterior for $\theta_1,\ldots,\theta_G$:
  \begin{itemize}
  \item credible interval for $\theta_g-\theta_{g'}$ for specific $g$ and $g'$, \pause
  \item posterior model probability, e.g. $p(H_0|y)$, \pause and
  \item probability statements, e.g. $P(\theta_g<\theta_{g'}|y)$ for specific $g$ and $g'$. 
  \end{itemize}
\end{itemize}
\end{frame}
...
\[ 
H_0: \theta_g = \theta \,\mbox{for all}\, g
\quad
\mbox{versus}
\quad
