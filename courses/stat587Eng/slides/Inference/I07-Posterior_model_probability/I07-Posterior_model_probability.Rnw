...
Calculate 
\[ 
p(H_0|y) = P(\theta\le \theta_0)
\]
and
\[
p(H_A|y) = P(\theta > \theta_0).
\]
\nc\ec
\end{frame}
...
Let $Y_i\ind N(\mu,1)$ and $H_0: \mu=0$ vs $H_A: \mu\ne 0$.
...
ggplot(densities, aes(x = y, y = density, 
  ...
  geom_point(data = data.frame(y = y, 
             ...
             size = 4) + 
  labs(y = "Prior predictive density") +
  theme_bw()
@
\nc\ec
\end{frame}
...
\alert{prediction}
\pause
and priors must be informative. 
...
\pvalue{}s:
\begin{itemize}[<+->]
\item \pvalue{}s measure how incompatible your data are with the null hypothesis.
...
\begin{itemize}[<+->]
\item Measure (prior) predictive ability.
% \item The larger the posterior probability, the more predictive that hypothesis 
% was compared to the other hypotheses.
\item But this requires you to have at least two (or more) well-thought out models with
informative priors.
\end{itemize}
...
\begin{itemize}
\item Use posterior probabilities for one-sided alternative hypotheses.
