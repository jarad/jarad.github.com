...
\pause
Let $Y$ be he number initiated in a two-day period.
If we assume the number of new accounts each day is independent 
and each day is a Poisson with rate 10, then $Y\sim Po(20)$.
\pause
Then
\[ \begin{array}{rl}
P(Y>16) &= 1 - P(Y\le 16) \\
&= 1 - \sum_{y=0}^{16} \frac{\lambda^y e^{-\lambda}}{y!} \\
&= 1 - 0.221 = 0.779.
\end{array}
\hspace{2in}
\]
