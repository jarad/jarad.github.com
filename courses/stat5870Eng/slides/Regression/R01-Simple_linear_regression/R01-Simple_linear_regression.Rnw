...
slr_viz = ggplot(d, aes(x=x, y=y, group = mean)) + 
  ...
  geom_line(color = "magenta", linewidth = 2) + 
  coord_flip() +
...
The least squares (minimize $\sum_{i=1}^n r_i^2$), 
...
\[ 
P(\beta_j>b_j|y) = P\left(T_{n-2}>\frac{b_j-\hat\beta_j}{SE(\hat\beta_j)}\right) 
\quad \mbox{or} \quad
P(\beta_j<b_j|y) = P\left(T_{n-2}<\frac{b_j-\hat\beta_j}{SE(\hat\beta_j)}\right)
 \]
\pause
...
\bc
$H_0:\beta_j\ge b_j$ vs $H_A: \beta_j < b_j$ has
\[ 
\mbox{\pvalue{}} 
...
\[ 
= 2\times \min\left\{ P\left(T_{n-2}>\frac{\hat\beta_j-b_j}{SE(\hat\beta_j)}\right) , 
P\left(T_{n-2}<\frac{\hat\beta_j-b_j}{SE(\hat\beta_j)}\right)  \right\} \pause
= 2 \times P\left(T_{n-2}< -\left|\frac{\hat\beta_j-b_j}{SE(\hat\beta_j)}\right|\right)
\]
\pause
