...
% \item $\beta_1$ is the expected (additive) increase in the response for a one unit increase in the explanatory variable, \pause and
\item $d\beta_1$ is the expected (additive) increase in the response for a $d$ unit (additive) increase in the explanatory variable.
\end{itemize}
...
% \item $\beta_1$ is the expected (additive) increase in corn yield (bushels/acre) when fertilizer is increased by 1 lb/acre or
\item $d\beta_1$ is the \alert{expected} increase in corn yield (bushels/acre) when 
...
\begin{frame}
\frametitle{Regression with logarithms (plotted on the original scale)}
...
% % \item $\beta_1$ is the expected (additive) increase in $\log(Y)$ for a one unit increase in the explanatory variable, \pause and
% \item $d\beta_1$ is the expected (additive) increase in $\log(Y)$ for a $d$ unit increase in the explanatory variable.
% \end{itemize}
% 
% \end{frame}
...
% \item $e^{\beta_1}$ is the multiplicative increase in the median of $Y$ for a one
% unit increase in the explanatory variable, and
\item $e^{d\beta_1}$ is the \alert{multiplicative increase} in the \alert{median} 
of $Y$ for a $d$ unit (additive) increase in the explanatory variable.
\end{itemize}
...
\pause
\begin{itemize} \small
\item $e^{\beta_0}$ is the \alert{median} corn yield (bushels/acre) when fertilizer level is 0 (lbs/acre) \pause and
% \item $e^{\beta_1}$ is the multiplicative increase in median corn yield (bushels/acre)
% when fertilizer is increased by 1 lb/acre, \pause and
\item $e^{d\beta_1}$ is the \alert{multiplicative increase} in median corn yield (bushels/acre)
...
% \item $\beta_1$ is the expected (additive) increase in the response for a one unit increase
% in $\log(X)$
% \end{itemize}
% \end{frame}
...
\begin{itemize}
\item $\beta_0$ is the expected response when $X$ is 1 \pause and
\item $\beta_1 \log(d)$ is the expected (additive) increase in the response when $X$ increases
\alert{multiplicatively} by $d$\pause,e.g.
  \begin{itemize}
  \item $\beta_1 \log(2)$ is the expected (additive) increase in the response for each
  \alert{doubling} of $X$ \pause or
  \item $\beta_1 \log(10)$ is the expected (additive) increase in the response for each
...
\begin{itemize}
\item $\beta_0$ is the expected corn yield (bushels/acre) when fertilizer level is 1 lb/acre \pause and
\item $\beta_1 \log(2)$ is the expected (additive) increase in corn yield when fertilizer level is \alert{doubled}.
\end{itemize}
...
% \item $\beta_1$ is the expected (additive) increase in $\log(Y)$ for a one unit increase in $\log(X)$.
% \end{itemize}
% \end{frame}
...
\item $d^{\beta_1}$ is the \alert{multiplicative} increase in the \alert{median} of the response
when $X$ increases \alert{multiplicatively} by $d$, \pause e.g.
  \begin{itemize}
  \item $2^{\beta_1}$ is the \alert{multiplicative} increase in the \alert{median} of the response
  for each \alert{doubling} of $X$ \pause or
  \item $10^{\beta_1}$ is the \alert{multiplicative} increase in the \alert{median} of the
...
\item $2^{\beta_1}$ is the \alert{multiplicative increase} in median corn yield (bushels/acre) when fertilizer is \alert{doubled}.
\end{itemize}
...
\item When using the log of the response,
  ...
  \item $\beta_1$ determines the \alert{multiplicative} increase in the median response
  \end{itemize}
\item When using the log of the explanatory variable ($X$),
  \begin{itemize}
  \item $\beta_0$ determines the response when \alert{$X=1$}
  \item $\beta_1$ determines the increase in the response when there is a
...
<<diagnostics, dependson="model", echo=FALSE>>=
