...
\[\frac{\overline{Y}_1-\overline{Y}_2 - (\mu_1-\mu_2)}{\sqrt{\frac{S_1^2}{n_1} + \frac{S_2^2}{n_2}}} \sim t_{v}\]
...
\pause
  ...
  \end{itemize}
\item Bayesian: posterior for $\mu_1,\mu_2$, i.e. $p(\mu_1,\mu_2|y)$
  \begin{itemize}
...
\begin{frame}[fragile]
\frametitle{Frequentist analysis using t.test}

<<two-means-frequentist-t-test, dependson="d2",echo=TRUE>>=
t.test(sensitivity ~ process, data = d2, var.equal = TRUE)
@
...
\[
\frac{\overline{Y}_1-\overline{Y}_2 - (\mu_1-\mu_2)}{\sqrt{\frac{S_1^2}{n_1} + \frac{S_2^2}{n_2}}} \sim t_{n_1+n_2-2}
\]
