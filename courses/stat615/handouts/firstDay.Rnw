...
\item Suppose $Y_i \stackrel{ind}{\sim} N(\mu,\sigma^2)$ and 
$p(\mu,\sigma^2) \propto 1/\sigma^2$, then what is the (marginal) distribution of $\mu|y$?
\item Suppose $\theta \sim N(\mu,\Sigma)$ with  
$\theta = (\theta_1,\theta_2)^\top \in \mathbb{R}^2$, $\mu=(\mu_1,\mu_2)^\top$,
\[ 
...
$\theta \sim N(\mu,\Sigma)\mathrm{I}(0<\theta_1<\infty)\mathrm{I}(0<\theta_2<\infty)$.
What are the distributions $\theta_1|\theta_2$ and $\theta_2|\theta_1$ and how 
can these be used to construct a Gibbs sampler to sample from the distribution 
for $\theta$?
