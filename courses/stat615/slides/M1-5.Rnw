...
\pause
In order to ensure reversibility of our proposal, we need to negate momentum after we complete the leap-frog simulation, but so long as $p(\omega) = p(-\omega)$ this will not affect our acceptance probability. 
