...
so the full conditional is 
\[ p(\eta|\ldots) \propto \eta^{\I v/2} e^{-\frac{\eta}{2} \sum_{i=1}^\I \frac{1}{\varsigma_i}} \left( 1+\eta/c^2\right)^{-1} \eta^{-1/2}. \]

\vspace{0.1in} \pause

Let's us Metropolis-Hastings with proposal distribution 
\[ Ga\left(\frac{\I v}{2}+1, \frac{1}{2}\sum_{i=1}^\I \frac{1}{\varsigma_i}\right) \]
and acceptance probability $\min\{1,\rho\}$ where again
\[ \rho = \frac{ 1+\eta^{(i)}/c^2}{1+\eta^*/c^2} \sqrt{\frac{\eta^{(i)}}{\eta^*}}.\]
