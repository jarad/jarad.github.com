...
curve(dlaplace(x, 0, 1), -3, 3, lwd=2, xlab='theta', ylab='density')
@
...
Thus $\theta\sim \delta_c(\theta) = \delta_c$ indicate that the random variable $\theta$ is a degenerate random variable with $P(\theta=c)=1$. 
\end{frame}
...
curve(cdf(x), -2,2, xlab='theta', ylab='CDF', lwd=2)
@
...
where
\[ \begin{array}{rl}
p' &\pause= \frac{p N(y;0,V)}{p N(y;0,V) + (1-p) N(y;m,C+V)} \pause = \left(1 + \frac{(1-p) N(y;m,C+V)}{p N(y;0,V)} \right)^{-1}\\
C' &\pause= 1/(1/V+1/C) \\
m' &\pause= C'(y/V+m/C)
\end{array} \]
...
\begin{itemize}
\item Heavy tails allows the likelihood to overwhelm the prior. 
