...
\frame{\frametitle{}
    ...
    \begin{center}
    \only<1-| handout:0>{\multiinclude[<+>][format=pdf]{volatility}}
    \only<beamer:0| handout:1>{\includegraphics{volatility-0}}
    %\multiinclude[<+>][format=pdf]{volatility}
    \end{center}
}
...
\frame{\frametitle{}
    ...
    \begin{center}
    \only<1-| handout:0>{\multiinclude[<+>][format=pdf]{switching}}
    \only<beamer:0| handout:1>{\includegraphics{switching-0}}
    %\multiinclude[<+>][format=pdf]{switching}
    \end{center}
}
...
\frame{
    ...
    Systematic sampling
    \begin{itemize}
    \item Draw $U_1 \sim Unif(0,1)$
    \item Set $U_j = U_{j-1}+\frac{1}{J}$ for $j=1,2,\ldots,J$
    \item Invert cumulative sum of weights
    \end{itemize} \pause
    \column{0.5\textwidth}
    \multiinclude[format=pdf]{syst}
    \end{columns}
    \pause
...
\frame{
    \setkeys{Gin}{width=0.6\textwidth}
    Resampling can add unnecessary variance, e.g.
    \begin{center}
    \multiinclude[format=pdf]{addvar}
    \end{center}
}
...
\frame{
    ...
    \end{enumerate}
    \pause
    where $\mu_t^{(j)}$ is a point estimate of $x_t^{(j)}$, usually $\mu_t^{(j)}=E[x_t^{(j)}]$.
}
...
\frame{
    ...
    \begin{center}
    %\multiinclude[<+>][format=pdf]{MBF}
    \only<1-| handout:0>{\multiinclude[format=pdf]{MBF}}
    \only<beamer:0| handout:1>{\includegraphics{MBF-9}}
    \end{center}
}
...
\frame{
    ...
    if
    \[ \begin{array}{rl@{\quad}l}
    V|y_{1:t-1} &\sim  IG(\alpha_{V,t-1},\beta_{V,t-1}) \\
    \alpha,\beta,W|y_{1:t-1} &\sim N-IG(\mu_{t-1},\Sigma_{t-1},\alpha_{W,t-1},\beta_{W,t-1}) \\ \pause
    s_{t-1} &= (\alpha_{V,t-1},\beta_{V,t-1},\mu_{t-1},\Sigma_{t-1},\alpha_{W,t-1},\beta_{W,t-1})
    \end{array} \] \pause
    then
    \[ \begin{array}{rl@{\quad}l}
    V|y_{1:t} &\sim  IG(\alpha_{V,t},\beta_{V,t}) \\
    \alpha,\beta,W|y_{1:t},x_t,x_{t-1} &\sim N-IG(\mu_{t},\Sigma_{t},\alpha_{W,t},\beta_{W,t}) \\ \pause
    s_{t} &= (\alpha_{V,t},\beta_{V,t},\mu_{t},\Sigma_{t},\alpha_{W,t},\beta_{W,t})
    \end{array} \]
}
...
\frame{\frametitle{}
    ...
    \[ p(y_{1:T}) = \int \cdots \int \prod_{t=1}^T p(y_t|x_t)p(x_t|x_{t-1})p(x_0) dx_{0:T} d\theta .\] \pause
    This can be approximated using SMC methods by
    \[ p(y_{1:T}) \approx \frac{1}{J} \sum_{i=1}^J \tilde{w}_t^{(i)}\]
...
\frame{\frametitle{}
    ...
    \item R::KFAS
    \item R::dlm
    \item R::SMC (not being updated?)
    \item R::smcUtils (\url{http://www.pstat.ucsb.edu/faculty/niemi/software/})
    \end{itemize}
    }
}
