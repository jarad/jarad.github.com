...
&= (2\pi\sqrt{C})^{-1/2} \frac{b^a}{\mG(a)} \int (\phi)^{-1/2} e^{-(y-m)^2/2\phi C} (\phi)^{-(a+1)} e^{-b/\phi} \phi \\
&= (2\pi C)^{-1/2} \frac{b^a}{\mG(a)} \int (\phi)^{-(a+1/2+1)} e^{-[b+(y-m)^2/2C]/\phi} d\phi \\
...
\begin{itemize}[<+->]
\item $\theta_i\ind N(\mu,\phi_i), \phi_i = \tau^2 \pause \implies N(\mu,\tau^2)$ \pause
\item $\theta_i|\phi_i\ind N(\mu,\phi_i), \phi_i\ind Exp(1/2\tau^2) \pause \implies \theta_i \ind La(\mu,\tau)$ \pause
\item $\theta_i|\phi_i\ind N(\mu,\phi_i), \phi_i\ind IG(v/2,\tau^2/2) \pause \implies \theta_i \ind t_v(\mu,\tau^2)$
\end{itemize}
...
m' &= C'\left(\frac{m}{C} + \theta_i\sum_{i=1}^\I \frac{1}{\phi_i} \right)
\end{array} \]
...
&\propto \prod_{i=1}^\I \left[\prod_{j=1}^{n_i} e^{-(y_{ij}-\theta_i)^2/2\sigma^2} \prod_{i=1}^\I e^{-(\theta_i-\mu)^2/2\phi_i} \right] 
\end{array} \]
\pause
...
Let's us Metropolis-Hastings with proposal distribution 
\[ IG\left(\I-1, \sum_{i=1}^\I \frac{\phi_i}{2}\right) \]
...
Let's us Metropolis-Hastings with proposal distribution 
\[ Ga\left(\frac{\I v}{2}+1, \frac{1}{2}\sum_{i=1}^\I \frac{1}{\phi_i}\right) \]
and acceptance probability $\min\{1,\rho\}$ where again
\[ \rho = \frac{ 1+\eta^{(i)}/c^2}{1+\eta^*/c^2} \sqrt{\frac{\eta^{(i)}}{\eta^*}}.\]
...
and $\mu$, $\phi_i$, and $\tau^2$ get updated using only those $\theta_i$ that are non-zero. 
