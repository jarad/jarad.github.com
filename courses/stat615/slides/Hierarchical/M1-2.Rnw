...
\pause
i.e. $\theta$ has a $t$ distribution with $2a$ degrees of freedom, location $m$, and scale (variance) $bC/a$. \pause Thus if $C=1$, $a=v/2$, and $b=v\tau^2/2$, we have a $t$ distribution with $v$ degrees of freedom, location $m$, and scale $\tau^2$. 
\end{frame}
...
\pause
where $z=(\theta-m)/\sqrt{C}$ and $a=1/b$ \pause and thus $\theta$ has a Laplace distribution with location $m$ and scale $b/\sqrt{C}$. 
...
\pause
so we have 
\[ \phi_i|\ldots \ind IG([v+1]/2, [v\tau^2+(\theta_i-\mu)^2]/2). \]
