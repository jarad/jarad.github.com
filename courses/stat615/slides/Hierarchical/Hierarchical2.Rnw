...
\begin{enumerate}
\item Sample $\theta = (\theta_1,\ldots,G) \sim p(\theta|\cdots)$. \pause 
...
\mbox{ and } 
\mu|\tau^2,\ldots \sim N(\overline{\theta}, \tau^2/G)
\]
...
\item Sample $\mu,\sigma^2,\tau^2$:
  \begin{enumerate}
  \item Sample $\sigma^2 \sim IG(n/2,SSE)$.
...
\item Independently, sample $\theta_i \sim p(\theta_i|\cdots)$.
\item Sample $\sigma \sim p(\sigma|\cdots)$.
\item Independently, sample $\phi_i \sim p(\phi_i|\cdots)$.
...
\[ \theta_i|\cdots \sim \pause N\left(\left[ \frac{\mu}{\phi_i} + \frac{n_i}{\sigma^2}\overline{y}_i  \right], \left[\frac{1}{\phi_i} + \frac{n_i}{\sigma^2} \right]^{-1} \right) \]
...
where 
\[ \begin{array}{rl}
a' &= a + \sum_{i=1}^\I n_i /2 = a + n/2\\
b' &= b + \sum_{i=1}^\I \sum_{j=1}^{n_i} (y_{ij}-\theta_i)^2/2. 
\end{array} \]
\end{frame}
...
\[ 
p(\phi|\cdots) \propto \left[ \prod_{i=1}^\I N(\theta_i; \mu, \phi_i) Exp(\phi_i;1/2\tau^2) \right].
\]
\pause
So the individual $\phi_i$ are conditionally independent \pause with
\[ 
p(\phi_i|\cdots) \propto N(\theta_i; \mu, \phi_i) Exp(\phi_i;1/2\tau^2) \propto \phi_i^{-1/2} e^{-(\theta_i-\mu)^2/2\phi_i} e^{-\phi_i/2\tau^2}
 \]
\pause
If we perform the transformation $\eta_i = 1/\phi_i$, \pause we have 
\[
p(\eta_i|\cdots) \propto \eta_i^{-3/2} e^{-\frac{(\theta_i-\mu)^2}{2}\eta_i - \frac{1}{2\tau^2\eta_i}}
\] 
\pause
...
For the $t$ model, 
\[ \theta_i \ind N(\mu,\phi_i) \mbox{ and } \phi_i \ind IG(v/2,v\tau^2/2), \]
\pause
so we have 
\[ \phi_i|\cdots \ind IG([v+1]/2, [v\tau^2+(\theta_i-\mu)^2]/2). \]
...
\pause
so the full conditional is 
\[ p(\eta|\cdots) \propto \eta^{-\I/2} e^{-\sum_{i=1}^\I (\theta_i-\mu)^2/2\eta} \left( 1+\eta/c^2\right)^{-1} \eta^{-1/2} \]
...
so the full conditional is 
\[ p(\eta|\cdots) \propto \eta^{-\I} e^{-\sum_{i=1}^\I \phi_i/2\eta} \left( 1+\eta/c^2\right)^{-1} \eta^{-1/2}. \]
...
Let 
\[ \phi_i \sim IG(v/2, v\tau^2/2) \mbox{ and } \tau \sim Ca^+(0,c) \]
so the full conditional is 
\[ p(\eta|\cdots) \propto \eta^{\I v/2} e^{-\frac{\eta}{2} \sum_{i=1}^\I \frac{1}{\phi_i}} \left( 1+\eta/c^2\right)^{-1} \eta^{-1/2}. \]
...
Let 
\[ \theta_i \ind \pi \delta_0 + (1-\pi) N(\mu,\phi_i) \]
and independently $\pi \sim Beta(s,f)$, $\mu \sim N(m,C)$, and $\phi_i=\tau^2$ for normal model or $\phi_i \ind IG(v/2,v\tau^2/2)$ for the $t$ model. 

\vspace{0.2in} \pause

The full conditional for $\pi$ is
\[ \pi|\cdots \sim Beta\left(s+\sum_{i=1}^\I \I(\theta_i=0), f+\sum_{i=1}^\I \I(\theta_i\ne 0)\right)\]
