...
\[ \theta_g|\cdots \sim \pause N\left(\left[ \frac{1}{\phi_g}\mu + \frac{n_g}{\sigma^2}\overline{y}_g  \right], \left[\frac{1}{\phi_g} + \frac{n_g}{\sigma^2} \right]^{-1} \right) \]
...
\mu_g' &= \phi_g' \left( \frac{\mu}{\phi_g} + \frac{n_g}{\sigma^2}\overline{y}_g \right)
\end{array} \]
\end{frame}
...
\[ \pi|\cdots \sim Beta\left(s+\sum_{g=1}^G G(\theta_g=0), f+\sum_{g=1}^G G(\theta_g\ne 0)\right)\]
...
Updating $\mu$, $\phi_g$, and $\tau$, will be exactly the same as it was before
EXCEPT, you will only use $g$ such that $\theta_g \ne 0$. 
So 
\begin{itemize}
...
\end{array} \]
\pause
\item $\tau$ will still require a MH step, but the proposal should only depend on 
$g:\theta_g\ne 0$, \pause and
\item $\phi_g$ will be exactly as before when $\theta_g\ne 0$, 
but when $\theta_g=0$, then it actually doesn't matter what distribution you 
are sampling from (Carlin and Chib 1995).
\end{itemize}
