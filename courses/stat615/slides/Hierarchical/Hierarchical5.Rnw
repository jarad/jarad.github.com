...
\begin{itemize}
\item $m$ be its mass,
\item $\theta$ be its position, and
\item $\omega$ be its momentum.
\end{itemize}
\pause
The mass has 
\begin{itemize}
\item potential energy $U(\theta)$ (which is proportional to its height) and
\item kinetic energy $K(\omega)=\omega^2/(2m)$. 
\end{itemize}
\end{frame}
...
If we apply the mapping $T_s$ to point in some region $R$ of $(\theta,\omega)$ space with
