...
and independently assume $p(\sigma^2) \propto 1/\sigma^2$. 
...
We use the marginal likelihood as evidence in favor of the model, i.e. when comparing models those with higher marginal likelihoods should be prefered over the rest.
\end{frame}
...
{\small
\[
BF(M_\gamma:M_{\gamma'}) = \frac{BF(M_\gamma:M_{null}) }{BF(M_{\gamma'}:M_{null})} = \frac{p(y|M_\gamma,g)/p(y|M_{null},g)}{p(y|M_{\gamma'},g)/p(y|M_{null},g)} = \frac{p(y|M_\gamma,g)}{p(y|M_{\gamma'},g)}
\]
}
\pause
...
\begin{itemize}
\item As $g\to \infty$, $(M_\gamma:M_{null}) \to 0$. \pause (Lindley's Paradox) \pause
...
\]
as $R_\gamma \to 1$ and thus the empirical Bayes approach does not suffer from either paradox. \pause The empirical is model selection consistent if the true model is not the null model, but is inconsistent if it is.
\end{frame}
