...
&= p(\theta_T|y_{1:T}) \prod_{t=T}^1   p(\theta_{t-1}|\theta_t,y_{1:t-1}) \pause\\
&\propto p(\theta_T|y_{1:T}) \prod_{t=T}^1   p(\theta_t|\theta_{t-1},y_{1:t-1})p(\theta_{t-1}|y_{1:t-1}) \pause \\
&= p(\theta_T|y_{1:T}) \prod_{t=T}^1   p(\theta_t|\theta_{t-1})p(\theta_{t-1}|y_{1:t-1})
\end{array} \]
\end{frame}
...
\[ 
p(\theta|y) 
= p(\theta_T|y_{1:T})\prod_{t=1}^T p(\theta_t|\theta_{t-1})p(\theta_{t-1}|y_{1:t-1}).
\]
...
Consider Markov model where the states are observed directly,
\pause
