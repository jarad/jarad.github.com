...
\frame{\frametitle{Fully conjugate subjective Bayesian inference}
	...
	\[ \beta|\sigma^2 \sim N(b_0, \sigma^2 B_0) \qquad \sigma^2 \sim \mbox{Inv-}\chi^2(v_0,s_0^2) \]
	\pause then the posterior is 
	\[ \beta|\sigma^2,y \sim N(b_n, \sigma^2 B_n) \qquad \sigma^2|y \sim \mbox{Inv-}\chi^2(v_n,s_n^2) \]
	\pause with
	\[ \begin{array}{rl}
	B_n^{-1} &= \frac{1}{g}B_0^{-1} + \frac{1}{\sigma^2}X^\top X \\
	b_n &= B_n^{-1}\left[ \frac{1}{g}B_0^{-1}b_0 + \frac{1}{\sigma^2}X^\top y \right] \\
	v_n &= v_0+n \\
	v_ns_n^2 &= v_0s_0^2+(y-X b_0)^\top (XB_0X^\top+\I)^{-1}(y-X b_0) 
	\end{array} \]
}
