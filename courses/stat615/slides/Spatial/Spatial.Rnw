...
Assume
\[
Y_i \ind N(X_i\beta + \omega_i, \nu^2)
\]
or, alternatively,
\[
Y_i = X_\beta+\omega_i+\epsilon_i
\]
\pause
where
\begin{itemize}
\item $\beta$ are the regression parameters,
\item $\omega_i$ are assumed to come from an intrinsic CAR model with proximity matrix indicating those regions that share a border, and
\item $\epsilon_i\ind N(0,\nu^2)$.
\end{itemize}
...
% <<CARleroux, dependson = "CARBayes_analysis", echo=TRUE>>=
