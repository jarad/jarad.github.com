...
<<theta_traceplots, dependson='spLM_run'>>=
...
\begin{frame}
\frametitle{}
<<theta_posteriors, dependson='spLM_run'>>=
GGally::ggpairs(r$p.theta.samples[burnin:nreps,])
@
\end{frame}
...
<<beta_traceplots, dependson='spLM_run'>>=
...
<<theta_summary, dependson='spLM_run'>>=
...
<<beta_summary, dependson='spLM_run'>>=
...
\frame{\frametitle{CAR as a model for random effects}
 ...
 \pause 
	...
	\[ p(\omega_1,\ldots,\omega_S) \propto \exp\left( -\frac{1}{2\tau^2} \omega^\top [D_w-\rho W]\omega \right) \]
}
...
\frametitle{Housing price in Glasgow}
\begin{frame}
<<CARBayes_data>>=
#library(shapefiles)
#library(sp)
...
Assume 
\[ 
Y_i \stackrel{ind}{\sim} N(X_i\beta + \omega_i, \nu^2)
\]
\pause
where 
\begin{itemize}
...
\begin{frame}[fragile]
<<CARBayes_analysis, dependson='CARBayes_data', echo=TRUE>>=
W.nb <- poly2nb(propertydata.spatial, row.names = rownames(propertydata))
W.mat <- nb2mat(W.nb, style="B")
model.spatial <- S.CARiar(formula = log(price) ~ crime+rooms+sales+factor(type) + log(driveshop), 
                             data = propertydata, 
                             family = "gaussian", 
                             W = W.mat, 
                             burnin = 1000, 
...
\frame{\frametitle{GLMs with spatial random effects}\pause
  Let 
	...
	For example,
	\begin{itemize}[<+->]
	\item $Y_i\stackrel{ind}{\sim} Ber(p_i)$ with $p_i=\mathrm{\Phi}^{-1}(X_i^\top \beta + \omega_{s[i]})$ 
	\item $Y_i \stackrel{ind}{\sim} Po(\lambda_i)$ with $\lambda_i = \exp(X_i^\top\beta +\omega_{s[i]})$
	\end{itemize}
}
...
Assume 
\[ 
Y_i \stackrel{ind}{\sim} Po(E_ie^{\beta_0+\omega_i}) 
\]
\pause
where 
\begin{itemize}
