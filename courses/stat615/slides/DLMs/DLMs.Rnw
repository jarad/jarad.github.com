...
Recursively, given $p(\theta_{t+(k-1)}|y_{1:t})$ \pause
 ...
 &=& \int p(\theta_{t+k}|\theta_{t+(k-1)},y_{1:t})p(\theta_t|y_{1:t}) d\theta_{t+(k-1)} \\ \pause
 &=& \int p(\theta_{t+k}|\theta_{t+(k-1)})p(\theta_t|y_{1:t}) d\theta_{t+(k-1)}
 \end{eqnarray*}
 }
}
...
{\tiny
 ...
 &=& \int \frac{1}{(2\pi)^{p/2}|W_t|^{1/2}}\exp\left(-\frac{1}{2}(\theta_t-G_t\theta_{t-1})'W_t^{-1}(\theta_t-G_t\theta_{t-1})\right) \\ && \quad \frac{1}{(2\pi)^{p/2}|C_{t-1}|^{1/2}}\exp\left(-\frac{1}{2}(\theta_{t-1}-m_{t-1})'C_{t-1}^{-1}(\theta_{t-1}-m_{t-1})\right) d\theta_{t-1} \\ \pause
 &=& N(a_t,R_t)
 \end{eqnarray*} \pause
 \item One-step ahead predictive distribution for $y_t$ \pause
 \begin{eqnarray*} p(y_t|y_{1:t-1}) &=& \int p(y_t|\theta_t)p(\theta_t|y_{1:t-1})d\theta_t \\
 &=& \int N(y_t;F_t\theta_t,V_t)N(\theta_t;a_t,R_t)d\theta_t \\
 &=& \int \frac{1}{(2\pi)^{m/2}|V_t|^{1/2}}\exp\left(-\frac{1}{2}(y_t-F_t\theta_t)'V_t^{-1}(y_t-F_t\theta_t)\right) \\
 && \quad \frac{1}{(2\pi)^{p/2}|R_t|^{1/2}}\exp\left(-\frac{1}{2}(\theta_t-a_t)'R_t^{-1}(\theta_t-a_t)\right) d\theta_t \\ \pause
 &=& N(f_t,Q_t)
 \end{eqnarray*} \pause
 \item Filtered distribution for $\theta_t$ \pause
...
\frame{\frametitle{Latent state prior}
 ...
 \item $V[\theta_t|y_{1:t-1}] \pause = G_tC_{t-1}G_t'+W_t \pause = R_t$ \pause
...
\frame{\frametitle{One step ahead prediction}
 ...
 \item $V[y_t|y_{1:t-1}] \pause = F_tR_tF_t'+V_t \pause = Q_t$ \pause
...
\frame{\frametitle{Latent state posterior - linear regression approach}
 ...
 \item $V[\theta_t|y_{1:t}] \pause = (R_t^{-1}+F_t'V_t^{-1}F_t)^{-1} \pause = C_t$ \pause
 \item $E[\theta_t|y_{1:t}] \pause = C_t(R_t^{-1}a_t+F_t'V_t^{-1}y_t) \pause = m_t$ \pause
...
\frame{\frametitle{Latent state posterior - multivariate normal approach}
 ...
 Consider \[ p\left(\left. \left[ \begin{array}{c} y_t \\ \theta_t \end{array} \right] \right|y_{1:t-1}\right)\pause = N\left(\left[\begin{array}{c} f_t \\ a_t \end{array}\right], \left[\begin{array}{cc} Q_t & \uncover<4->{F_tR_t} \\ \uncover<4->{R_tF_t'} & R_t \end{array} \right]\right) \]

 \vspace{0.2in} \pause \pause

 \begin{itemize}
 \item MVN theory $\implies \theta_t|y_{1:t-1},y_t$ is normal \pause
 \item $E[\theta_t|y_{1:t}] \pause = a_t+R_tF_t'Q_t^{-1}(y_t-f_t)
  \pause = m_t$ \pause
 \item $V[\theta_t|y_{1:t}] \pause = R_t-R_tF_t'Q_t^{-1}F_tR_t \pause = C_t$ \pause
 \item $p(\theta_t|y_{1:t}) = N(m_t,C_t)$.
 \end{itemize}
}
...
\frame{\frametitle{Kalman filter}
 ...
 \[ a_t = G_tm_{t-1} \quad \mbox{and} \quad R_t =G_tC_{t-1}G_t'+W_t.\]
 \item Obtain one step ahead predictive $p(y_t|y_{1:t-1}) = N(f_t,Q_t)$ where
 \[ f_t = F_ta_t \quad \mbox{and} \quad Q_t = F_tR_tF_t'+V_t.\]
 \item Obtain posterior $p(\theta_t|y_{1:t}) = N(m_t,C_t)$ where
 \[ \begin{array}{rcl@{\quad \mbox{and} \quad}rcl}
 m_t &=& a_t+K_te_t & C_t &=& R_t-K_tQ_tK_t' \\
 e_t &=& y_t-f_t  & K_t &=& R_tF_t'Q_t^{-1}
 \end{array} \]
 \end{itemize} \pause
 $K_t$ is the \emph{Kalman gain} or \emph{adaptive coefficient}.
}
...
\frame{\frametitle{\phantom{Updated }Kalman filter}
 \begin{itemize}
 \item Assume $p(\theta_{t-1}|y_{1:t-1})=N(m_{t-1},C_{t-1})$.
 \item Obtain prior $p(\theta_t|y_{1:t-1}) = N(a_t,R_t)$ where
 \[ a_t = G_tm_{t-1} \quad \mbox{and} \quad R_t =G_tC_{t-1}G_t'+W_t.\]
...
\frame{\frametitle{Updated Kalman filter}
 \begin{itemize}
 \item Assume $p(\theta_{t-1}|y_{1:t-1})=N(m_{t-1},C_{t-1})$.
 \item Obtain prior $p(\theta_t|y_{1:t-1}) = N(a_t,R_t)$ where
 \[ a_t = G_tm_{t-1} \quad \mbox{and} \quad R_t =G_tC_{t-1}G_t'+W_t.\]
 ...
 \item Obtain posterior $p(\theta_t|y_{1:t}) = N(m_t,C_t)$ where
  \begin{itemize}
  \item If $y_t$ is not observed, $m_t=a_t$ and $C_t=R_t$.
...
\frame{\frametitle{Linear trend model}
 ...
 \item $\theta_t \pause = (\mu_t,\beta_t)'$ \pause
...
\frame{\frametitle{Specifying a local level model in R}
  \begin{itemize}
\setbeamertemplate{itemize item}{\textcolor{black}{\bf $>$}}
  \item \texttt{library(dlm)} \pause
  \item \texttt{local.level = dlmModPoly(order=1)} \pause
  \item \texttt{?dlmModPoly} \pause

\begin{align*} dlmModPoly(&order = 2, dV = 1, dW = c(rep(0, order - 1), 1),\\
           &m0 = rep(0, order), C0 = 1e+07 * diag(nrow = order))
\end{align*} \pause

  \item \texttt{V(local.level) = 9.465} \pause
  \item \texttt{W(local.level) = 0.121} \pause
  \item \texttt{modLSup = dlmModPoly(1, dV = 9.465, dW = 0.121)}
  \end{itemize}
}
%
%\frame{\frametitle{Specifying a linear trend model in R}
%  \begin{itemize}
%\setbeamertemplate{itemize item}{\textcolor{black}{\bf $>$}}
%  \item \texttt{library(dlm)} \pause
%  \item \texttt{local.level = dlmModPoly(order=1)} \pause
...
where
\begin{eqnarray*}
\theta_1 &=& \pause (\alpha_1,\alpha_2,\ldots,\alpha_s)' \\
...
If $\theta_1 = (\alpha_1,\alpha_2,\ldots,\alpha_s)'$ and $W_t=0$, what is $\theta_t$?

\vspace{0.1in} \pause

\begin{eqnarray*}
\theta_2 &=& G\theta_1 = \pause (\alpha_2,\alpha_3,\alpha_4,\ldots,\alpha_s,\alpha_1)' \\ \pause
\theta_3 &=& G\theta_2 = \pause (\alpha_3,\alpha_4,\ldots,\alpha_s,\alpha_1,\alpha_2)' \\ \pause
&\vdots & \\ \pause
\theta_t &=& G\theta_{t-1} = \pause (\alpha_{j},\alpha_{j+1},\ldots,\alpha_s,\alpha_1,\ldots,\alpha_{j-1})' \\ \pause
\end{eqnarray*}
...
where
\begin{eqnarray*}
\theta_1 &=&  (\alpha_1,\alpha_s,\alpha_{s-1},\ldots,\alpha_2)' \\
F_t &=& F =  (1,0,0,\ldots,0) \\
...
where
\begin{eqnarray*}
\theta_1 &=&  (\alpha_1,\alpha_s,\alpha_{s-1},\ldots,\alpha_3)' \\
F_t &=& F =  (1,0,\ldots,0) \\
G_t &=& G=  \left[ \begin{array}{ccccc}
-1 & -1 & \cdots & -1 & -1 \\
1 & 0 & \cdots & 0 & 0 \\
...
\begin{eqnarray*}
\theta_2 &=& G\theta_1 = \pause (\alpha_2,\alpha_1,\alpha_s,\alpha_{s-1},\ldots,\alpha_4)' \\ \pause
\theta_3 &=& G\theta_2 = \pause (\alpha_3,\alpha_2,\alpha_1,\alpha_s,\alpha_{s-1},\ldots,\alpha_5)' \\ \pause
&\vdots & \\ \pause
\theta_t &=& G\theta_{t-1} \\
&=& \pause (\alpha_{j},\alpha_{j-1},\ldots,\alpha_1,\alpha_s,\alpha_{s-1},\ldots,\alpha_{j+2})' \\ \pause
\end{eqnarray*}
...
Suppose all $s$ seasons have a factor $\alpha_i$. Then, think of $\alpha$ as a linear combination of basis vectors
\[ \alpha = \sum_{i=1}^s \alpha_i u_i \]
where $u_i$ is the $s$-dimensional vector having the $i$th component equal to one and all other elements zero.
...
{\scriptsize
\[ \begin{array}{rl}
e_0 &= (1,1,\ldots,1)' \pause \\
c_1 &= (\cos \omega_1, \cos 2\omega_1, \ldots \cos s\omega_1)' \\
s_1 &= (\sin \omega_1, \sin 2\omega_1, \ldots \sin s\omega_1)' \pause \\
&\vdots \\
c_j &= (\cos \omega_j, \cos 2\omega_j, \ldots \cos s\omega_j)' \\
s_j &= (\sin \omega_j, \sin 2\omega_j, \ldots \sin s\omega_j)' \pause \\
&\vdots \\
c_{s/2} &= (\cos \omega_{s/2}, \cos 2\omega_{s/2}, \ldots \cos s\omega_{s/2})'
\end{array} \]
}
...
\frame{\frametitle{Fourier basis}
\begin{itemize}[<+->]
\item Basis for $\mathbb{R}^s$

\[ \alpha = a_0e_0+\sum_{j=1}^{s/2-1} (a_jc_j+b_js_j)+a_{s/2}c_{s/2}. \]
\pause
Assume $a_0=0$ since the mean will be modeled separately.
...
% \pause 
The conjugate harmonic is
\[ S_j^*(t) = -a_j \sin(t \omega_j) + b_j \cos (t \omega_j).\]
\pause
And the $j$ th harmonic evolves in sime according to 
\[ 
...
Consider simpler DLMs containing only seasonality of frequency $j<s/2$.
...
state vector $(S_j(t),S_j^*(t))'$, and observation matrix $F=[1\, 0]$.
...
\[ \theta_t = (S_1(t),S_1^*(t),\ldots,S_{\frac{s}{2}-1}(t),S_{\frac{s}{2}-1}^*(t),S_{\frac{s}{2}}(t))' \]
with evolution matrix
\[ G = \mbox{blockdiag}(H_1,\ldots,H_{\frac{s}{2}}) \]
and observation matrix
\[ F=[1\, 0\, 1\, 0\, \ldots\, 0\, 1]\]
and initial vector
\[ \theta_0 = (a_1,b_1,\ldots,a_{\frac{s}{2}-1},b_{\frac{s}{2}-1},a_{\frac{s}{2}})'. \]
}
...
<<echo=TRUE>>=
nottem.model = dlmModTrig(12,dV=2.315^2,dW=rep(0,11))
nottem.filter = dlmFilter(y-mean(y),nottem.model)
forecast = dlmForecast(nottem.filter, 12)
data.frame(mean = as.numeric(forecast$f),
           lm_mean = m$coefficients)
@
\end{frame}
...
<<echo=TRUE>>=
nottem.model = dlmModTrig(12, 2, dV=2.315^2,dW=rep(0,11))
nottem.filter = dlmFilter(y-mean(y),nottem.model)
forecast = dlmForecast(nottem.filter, 12)
data.frame(mean = as.numeric(forecast$f),
           lm_mean = m$coefficients)
...
\frametitle{Nottingham data}
<<echo=TRUE>>=
plot(f1, type='l', lwd=2, lty=2)
lines(forecast$f, col='red', lwd=2, lty=2)
@
\end{frame}
...
\end{eqnarray*}
where $r=\max\{p,q+1\}$ with $\phi_j=0$ for $j>p$ and $\psi_j=0$ for $j>q$ and $\epsilon\stackrel{iid}{\sim} N(0,\sigma^2)$. 

\begin{center}
\uncover<3>{\alert{(write on board)}}
\end{center}
}
...
V &=& 0 \\ \pause
W &=& RR'\sigma^2 \\ \pause
\theta_t &=& (\theta_{1,t},\ldots,\theta_{r,t})' \\ \pause
F &=& (1,0,\ldots,0) \\ \pause 
...
\phi_r & 0 & \cdots & 0 & 0
\end{array}\right] \\ \pause
R &=& (1, \psi_1,\ldots,\psi_{r-2},\psi_{r-1})' 
\end{eqnarray*}
}
...
\begin{eqnarray*}
\theta_t &=& (Y_t^{(p)},\delta_t,Y_t^{(g)},\theta_{t,2}^{(g)})' \\
F &=& (1,0,1,0) \\
...
\frame{\frametitle{Standard regression model}
\[ Y_t = \beta_1+\beta_2x_t+\epsilon_t, \qquad \epsilon_t \stackrel{iid} N(0,\sigma^2) \]

\vspace{0.2in} \pause

Let's make the regression coefficients dynamic:
\[ Y_t = \beta_{1,t}+\beta_{2,t}x_t+\epsilon_t, \qquad \epsilon_t \stackrel{iid}{\sim} N(0,\sigma^2) \]
and have temporal evolution of the regression coefficients, e.g. 
\[ \beta_{j,t}=\beta_{j,t-1}+w_{j,t} \]
for $j=1,2, w_{1,t}$ and $w_{2,t}$ independent. 
}

\frame{\frametitle{Dynamic regression model}
\[ \begin{array}{rll}
Y_t &= x_t'\theta_t + v_t & v_t \sim N(0,\sigma_t^2) \\
\theta_t &= G_t \theta_{t-1} + w_t & w_t \sim N_p(0,W_t) 
\end{array} \]

\vspace{0.2in} \pause

A common choice is $G_t =I_p$ and $W_t$ diagonal. 
}
...
A univariate CAPM assumes that 
\[ y_t = \alpha+\beta x_t +v_t, \qquad v_t \stackrel{iid}{\sim} N(0,\sigma^2). \]
}
...
Suppose you have a linear trend model for each unit $i$:
 ...
 \begin{itemize}
 \item What is $F_{i,t}$? \pause $F_{i,t}=(1,0)'$ \pause
...
\begin{itemize}
\item the diagonal of $V$ is non-zero and the off-diagonal may be and
\item the diagonal of $W$ is non-zero and the off-diagonal may be.
\end{itemize}
}
...
\end{array} \]
 ...
 \item What is $\theta_t^{(i)}$? \pause $\theta_t^{(i)} = (\beta_{1,t}^{(i)},\ldots,\beta_{p,t}^{(i)})'$
 \end{itemize}
}
...
y_t &= (F_t \otimes I_m)\theta_t + v_t & v_t \stackrel{iid}{\sim} N(0,V) \\
\theta_t &= (G\otimes I_m)\theta_{t-1} + w_t & w_t \stackrel{iid}{\sim} N(0,W)
\end{array} \]
where \pause
...
with $F_t=(1,x_t)', G=I_2,$ and $W=\mbox{blockdiag}(W_\alpha,W_\beta)$.
}
...
where
\[ \begin{array}{rl@{\qquad}l}
\theta_t &= (\theta_{1,t}',\ldots,\theta_{m,t}')' \\
F_{y,t} &= \mbox{blockdiag}(F_{i,t}) \\
F_{\theta,t} &= [I_p|\cdots|I_p]' \\
...
\[ \begin{array}{rl@{\qquad}l}
Y_{t} &= F_{y,t}F_{\theta,t}\lambda_{t}+v_{t}^*, & v_{t}^* \sim N_m(0,F_{y,t}V_{\theta,t}F_{y,t}'+V_{y,t}) \\
\lambda_t &= G_t\lambda_{t-1} + w_t, & w_t \sim N_k(0,W_t)
\end{array} \]
}
...
\tilde{\mu}_t &= \tilde{\mu}_{t-1}+\tilde{w}_t, & \tilde{w}_t \sim N_p(0,HWH')
\end{array} \]\pause
...
\[ L(\psi) \propto \prod_{t=1}^n \frac{1}{(2\pi)^{k/2} |Q_t|^{1/2}} \exp\left( -\frac{1}{2} (y_t-f_t)'Q_t^{-1}(y_t-f_t)\right).\]

\vspace{0.2in} \pause

Log-likelihood function \pause
\[ \ell(\psi) = C + -\frac{1}{2}\sum_{t=1}^n \log |Q_t|-\frac{1}{2}\sum_{t=1}^n (y_t-f_t)'Q_t^{-1}(y_t-f_t).\]
...
where $a_t = G_tm_{t-1}$ and $\tilde{R}_t = G_t \tilde{C}_{t-1} G_t'+\tilde{W}_t$.

\vspace{0.1in} \pause

One step ahead predictive density
\[ Y_t|y_{1:t-1} \sim St(f_t, \tilde{Q}_t\beta_{t-1}/\alpha_{t-1},2\alpha_{t-1})\]
with $f_t=F_ta_t$ and $\tilde{Q}_t = F_t\tilde{R}_tF_t'+\tilde{V}_t$.
...
\tilde{C}_t &=& \tilde{R}_t-\tilde{R}_tF_t'\tilde{Q}_t^{-1}\tilde{R}_t' \\
...
To implement, set \[ W_t = \frac{1-\delta}{\delta} G_t' C_{t-1} G_t\] or \[ \tilde{W}_t = \frac{1-\delta}{\delta} G_t' \tilde{C}_{t-1} G_t\]
...
\item $V[\phi_t|y_{1:t-1}] \pause = \frac{1}{\delta^*} V[\phi_{t-1}|y_{1:t-1}]$
\end{itemize}
}
...
%\[ y_i \stackrel{iid}{\sim} p N(\mu_0, \sigma_0^2) + (1-p) N(\mu_1, \sigma_1^2). \]
%\begin{center}
...
K_0 &=& (C_0^{-1}+G_1'W_1^{-1}G_1)^{-1} \\ \pause
k_0 &=& K_0(C_0^{-1}m_0 +G_1'W_1^{-1}\theta_1) \\ \pause
\\
...
K_T &=& (W_T^{-1}+F_T'V_T^{-1}F_T)^{-1} \\ \pause
k_T &=& K_T(W_T^{-1}G_T\theta_{T-1} + F_T'V_T^{-1}y_T) \\ \pause
\\
...
&\propto & N(\theta_t; k_t, K_t) \\ \pause
K_t &=& (W_t^{-1}+F_t'V_t^{-1}F_t+G_{t+1}'W_{t+1}^{-1}G_{t+1})^{-1} \\ \pause
k_t &=& K_t(W_t^{-1}G_t\theta_{t-1}+F_t'V_t^{-1}y_t+G_{t+1}'W_{t+1}^{-1}\theta_{t+1})
\end{eqnarray*}
}
}
...
H_t &=& (C_t^{-1}+G_{t+1}'W_{t+1}^{-1}G_{t+1})^{-1} \\
h_t &=& H_t(C_t^{-1}m_t+G_{t+1}'W_{t+1}^{-1}\theta_{t+1}) \\
\\
...
So instead of saying `\emph{the chain has converged}' you should be saying `\emph{the chain shows no lack of convergence}'.
}
...
Consider the model $y_i \stackrel{iid}{\sim} N(\mu,1)$ and our goal is to estimate $E[y_i]=\mu$. The Monte Carlo approximation is 
...
Recall from HW 1b, if $\sigma^2 \sim IG(a,b)$ and $x_i\stackrel{iid}{\sim} N(0,\sigma^2)$, \pause then
...
\end{array} \]
where $\mathrm\Phi_0^{-1}$ is an $m\times m$ observation precision matrix and $\mathrm\Phi_1^{-1}$ is a $p\times p$ evolution precision matrix. \pause It will be convenient to choose independent Wishart distributions for the prior for these precision matrices, \pause i.e. 
\[p\left(\mathrm\Phi_0^{-1},\mathrm\Phi_1^{-1}\right)=p\left(\mathrm\Phi_0^{-1}\right)p\left(\mathrm\Phi_1^{-1}\right)=\mathcal{W}\left(\mathrm\Phi_0^{-1}; \nu_0, S_0\right)\mathcal{W}\left(\mathrm\Phi_1^{-1}; \nu_1, S_1\right) \] \pause
where
...
where $SS_y = \sum_{t=1}^T \left(y_t-F_t\theta_t\right)\left(y_t-F_t\theta_t\right)'$.

\vspace{0.2in} \pause

\[ p\left(\mathrm\Phi_1^{-1}|\ldots\right) = \mathcal{W}\left(\nu_1+T/2, S_1+\frac{1}{2}SS_\theta\right) \]
where $SS_\theta = \sum_{t=1}^T \left(\theta_t-G_t\theta_{t-1}\right)\left(\theta_t-G_t\theta_{t-1}\right)'$.

\vspace{0.2in} \pause

Again, $p\left(\mathrm\Phi_0^{-1},\mathrm\Phi_1^{-1}|\ldots\right)=p\left(\mathrm\Phi_0^{-1}|\ldots\right)p\left(\mathrm\Phi_1^{-1}|\ldots\right)$.
...
\[ SS_{ii,t} = (\theta_{i,t}-G_{i,t}\theta_{i,t-1})(\theta_{i,t}-G_{i,t}\theta_{i,t-1})'.\]
}
...
e.g. matrix $V$
\[ \begin{array}{rl}
p(\mathrm\Phi_V|\ldots) &= W\left(a+\frac{T'}{2}, b+\sum_{t\in \mbox{obs}} (y_t -F_t\theta_t)(y_t -F_t\theta_t)'\right) .
\end{array} \]
}
