...
\frametitle{Nottingham data}
<<echo=TRUE>>=
ggplot(subset(d, harmonics==6), aes(month, effect, group=1)) + geom_line()
@
\end{frame}
...
ggplot(d, aes(month, effect, color=harmonics_f, shape=harmonics_f, group=harmonics_f)) + geom_point() + geom_line()
@
\end{frame}
