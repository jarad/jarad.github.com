...
<<seasonal_factor_model, cache=TRUE>>=
library(dlm)
y = as.numeric(nottem)
months = c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec")
x = as.factor(rep(months,length(y)/12))
summary(lm(y-mean(y)~x-1))

nottem.model = dlmModSeas(12,dV=2.315^2,dW=rep(0,11))
nottem.filter = dlmFilter(y-mean(y),nottem.model)
nottem.filter$m[241,]
-sum(nottem.filter$m[241,]) # Jan
@
\end{frame}
...
Let
\[ \omega_j = \frac{2\pi j}{s}, \quad j=0,1,\ldots,\frac{s}{2}.\]
...
s_1 &= (\sin \omega_1, \sin 2\omega_1, \ldots \sin s\omega_1)' \\
&\vdots \\
c_j &= (\cos \omega_j, \cos 2\omega_j, \ldots \cos s\omega_j)' \\
s_j &= (\sin \omega_j, \sin 2\omega_j, \ldots \sin s\omega_j)' \\
&\vdots \\
...
Assume $a_0$ since the mean will be modeled separately.

\vspace{0.2in}

\item Naturally periodic, e.g. $s_j(t+ks)=s_j(t)$ for $k=1,2,\ldots$ \pause since
\[ \sin \frac{2\pi(t+ks)j}{s} = \sin\left( \frac{2\pi tj}{2} + 2\pi kj\right) = \sin \frac{2\pi tj}{s}.\]
\end{itemize}
}
...
where $b_{s/2}=0$. \pause Then
\[ g_t = \sum_{j=1}^{s/2} S_j(t).\]
\pause The conjugate harmonic is
