...
\item $a=1/2$ is Jeffreys prior for the multinomial, and 
...
\]
\item Sample $\pi \sim Dir(a+n)$ where $n=(n_1,\ldots,n_H)$ and $n_h = \sum_{i=1}^n \I(\zeta_i=h)$. 
...
ggplot(data.frame(x=c(-3,10)), aes(x)) + 
  ...
  geom_ribbon(data=sm, aes(x=xx,y=mean, ymin=lb, ymax=ub), 
              fill='blue', alpha=0.5) +
  #  geom_line() +
  labs(x='x', y='density') +
  theme_bw()
...
\[ 
\mu_h|\mySigma_h \stackrel{ind}{\sim} N(m_h', v_h'^2) \quad \mySigma_h \stackrel{ind}{\sim} IW(D_h',c_h')
\]
where 
\[ \begin{array}{rl}
v_h'^2 &= (1/v_h^2 + n_h)^{-1} \\
