...
Suppose we have a set of $H$ unordered categories and we label these $1,\ldots,H$. \pause Independently, consumers choose one of the $H$ categories with the same probability. \pause Then a reasonable model is $Z_i\stackrel{ind}{\sim} Cat(H,p)$.
...
A special case is $H=2$ which is the binomial dsitribution. 
...
\[
p(P;a) = \frac{\mG(a_1+\cdots+a_H)}{\mG(a_1)\cdots \mG(a_H)} \prod_{h=1}^H p_h^{a_h}
\]
...
\item $\mbox{mode}(p_h) = \frac{a_h-1}{a_0-H}$.
\end{itemize}
...
\begin{itemize}
\item $a=1$ which is the uniform density over $\pi$ and 
\item $a=0$ which is an improper prior that is uniform in the $\log(\pi_h)$. The resulting posterior is proper if $y_h>0$ for all $h$. 
\end{itemize}
...
\]
\item Sample $\pi \sim Dir(a+n)$ were $n=(n_1,\ldots,n_H)$ and $n_h = \sum_{i=1}^n \I(\zeta_i=h)$. 
...
\[ \begin{array}{rl}
v_h'^2 &= (1/v_h^2 + n_h)^{-1} \\
m_h' &= v_h'^2(m_h/v_h^2 + n_h\overline{y}_h) \\
c_h' &= c_d + n_2/2 \\
d_h' &= d_h + \frac{1}{2}\left(\sum_{i:\zeta_i=h} (y_i-\overline{y}_h)^2 + \left(\frac{n_h}{1+n_h/v_h^2}\right)(\overline{y}_h - m_h)^2 \right) \\
\overline{y}_h &= \frac{1}{n_h} \sum_{i:\zeta_i=h} y_i
\end{array} \]
\end{enumerate}
...
\[ \begin{array}{rl}
\pi &\sim Dir(a) \\
\mu_h|\sigma_h^2 & \stackrel{ind}{\sim} N_p(m_h, v_h^2\mySigma_h) \\
...
\[ 
\mu_h|\mySigma_h \stackrel{ind}{\sim} N(m_h', v_h'^2) \quad \mySigma_h \stackrel{ind}{\sim} IG(v'_h, D'_h)
\]
where 
\[ \begin{array}{rl}
v_h'^2 &= (1/v_h^2 + n_h)^{-1} \\
m_h' &= v_h'^2(m_h/v_h^2 + n_h\overline{y}_h) \\
c_h' &= c_d + n_h \\
D_h' &= D_h + \sum_{i:\zeta_i=h} (y_i-\overline{y}_h)(y_i-\overline{y}_h)^\top + \left(\frac{n_h}{1+n_h/v_h^2}\right)(\overline{y}_h-\mu_h)(\overline{y}_h-\mu_h)^\top  \\
\overline{y}_h &= \frac{1}{n_h} \sum_{i:\zeta_i=h} y_i 
\end{array} \]
\end{enumerate}
...
model {
  for (i in 1:n) {
    y[i,1:p] ~ dmnorm(mu[zeta[i],1:p], Tau[1:p,1:p,zeta[i]])
    zeta[i] ~ dcat(pi[])
  }

  for (h in 1:H) {
    mu[h,1:p] ~ dmnorm(mu0,Tau[,,h])
...
<<jags_joint_run, dependson=c('jags_joint','jags_joint_data'), echo=TRUE, results='hide'>>=
