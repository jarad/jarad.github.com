...
Then 
\[
Y_i|\zeta_i=z \stackrel{ind}{\sim} N(\mu_{z},\sigma_{z}^2) \qquad \zeta_i \stackrel{ind}{\sim} Mult(1,\pi)
\]
where $\pi=(\pi_1,\ldots,\pi_H)$. 
...
dat = list(n=nrow(tmp), M=3, y=tmp$phi, a=rep(1,3))
@
