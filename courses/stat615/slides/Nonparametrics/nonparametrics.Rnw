...
\[ 
Y_i \stackrel{ind}{\sim} p(\cdot|\theta_i), \quad \theta_i \stackrel{ind}{\sim} G, \quad G\sim DP(aG_0).
\]

Alternatively, if we use the stick-breaking construction, we have 
\[ 
Y_i \stackrel{ind}{\sim} p(\cdot|\theta_i), \quad \theta_i \stackrel{ind}{\sim} \sum_{h=1}^\infty \pi_h \delta_{\theta_h^*}
\]
where $\theta_h^* \stackrel{ind}{\sim} G_0$ and $\pi \sim \mbox{stick}(a)$. 
...
where 
\begin{itemize}
\item $\pi_h = V_h\prod_{\ell<h} (1-V_\ell)$ for $h<H$,
\item $\pi_H = \prod_{\ell<H} (1-V_\ell)$, and
\item $V_h \stackrel{ind}{\sim} Be(1,a)$
\end{itemize}
...
% 
% \[ 
% Y_i \stackrel{ind}{\sim} N(\mu_i, \sigma_h^2) \quad 
% \left(\begin{array}{cc} \mu_i \\ \sigma_i^2 \end{array} \right) \sim \sum_{h=1}^H \pi_h \delta_{(\mu_h,)}
% \]
