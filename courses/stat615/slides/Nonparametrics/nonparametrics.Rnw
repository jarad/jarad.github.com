...
<<rnaseq_plot, dependson='rnaseq', warning=FALSE>>=
parms = names(hat)[-1]
plots = list()
...
<<mean_heights>>=
prop_group1 = .44
truth = data.frame(mean = c(70,60), 
                   sd   = c(4,3.5), 
                   p    = c(prop_group1, 1-prop_group1), 
                   group  = c('1','2'))
...
xx = 50:90
truth_plot = data.frame(xx=xx,
                 group1   = truth$p[1] * dnorm(xx, truth$mean[1], truth$sd[1]),
                 group2 = truth$p[2] * dnorm(xx, truth$mean[2], truth$sd[2])) %>%
  mutate(full = group1+group2)

ggplot(melt(truth_plot, id.var='xx'), aes(xx,value,group=variable,color=variable,linetype=variable)) + 
  geom_line()
@
\end{frame}
...
<<posterior_density_plot, dependson=c('posterior_density','mean_heights_plot')>>=
ggplot(merge(sm, truth_plot), aes(x=xx,y=full, ymin=lb, ymax=ub)) +
