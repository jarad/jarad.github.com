...
\[ 
Y_{ij} \ind G_j, \quad G_j \ind DP(\alpha_j G_{0j})
\]
...
Suppose we are interested in estimating a collection of random probability measures $G_1,\ldots,G_J$. \pause
We would like for the measures to be DPs marginally, i.e. 
\[ 
G_j \ind DP(\alpha_j G_{0j})
\]
\pause
...
\[
G_j \stackrel{d}{=} \sum_{h=1}^\infty \pi_h \delta_{\theta_{jh}^*}, \quad 
...
\[
G_j \ind DP(\alpha G_0) \qquad G_0 \sim DP(\beta G_{00})
\]
\pause
The stick-breaking process related to this model is 
\[ 
G_j \stackrel{d}{=} \sum_{h=1}^\infty \pi_{jh}\delta_{\theta_h^*}, \quad 
G_0 \stackrel{d}{=} \sum_{h=1}^\infty \lambda_h \delta_{\theta_h^*}, \quad 
\theta_h^* \sim G_{00}
\]
\pause
where 
...
\[ 
G_j \ind G, \quad G\sim DP(\alpha G_0),\quad G_0\equiv DP(\beta G_{00}).
\]
\pause
The stick-breaking process related to this model is 
\[
G_j \ind G \stackrel{d}{=} \sum_{h=1}^\infty \pi_h \delta_{G_h^*}, \quad 
\pi \sim \mbox{stick}(\alpha), \quad 
P_h^* \ind DP(\beta G_{00}).
\]
\pause
...
The DPpackage was (is?) the premiere package for Dirichlet process, but it hasn't been updated since 2012-09-14. 
...
<<DPpackage_density_estimation, dependson='rnaseq', echo=TRUE>>=
library(DPpackage)
prior = list(alpha=1,m1=rep(0,1),psiinv1=diag(0.5,1),nu1=4,
                     tau1=1,tau2=100)
