...
Then 
\[ \begin{array}{rl}
Y_i|\zeta_i=z &\ind N(\mu_{z},\sigma_{z}^2) \\ 
\zeta_i &\ind Cat(H,\pi)
\end{array} \]
...
\]
with a prior that is equal to the base measure. \pause Thus this model is equivalent to our finite mixture with the exception of the distribution for $\pi$. 
\end{frame}
...
\]
where are exactly the same as in the normal finite mixture MCMC.
...
dat = list(n=nrow(tmp), H=10, y=tmp$phi, a=1)
@
...
r = jags.samples(jm, c('mu','sigma','pi'), 1e3)
@
\end{frame}
...
\begin{frame}
<<mcmc_convergence,dependson='jags_run'>>=
