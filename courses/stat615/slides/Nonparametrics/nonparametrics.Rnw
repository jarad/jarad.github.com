...
dat = list(n=nrow(tmp), H=20, y=tmp$phi, a=1)
@
...
\[ 
\theta_i|\theta_1,\ldots,\theta_{i-1} \sim \left( \frac{a}{a+i-1} \right)G_0(\theta_i) + \sum_{j=1}^{i-1} \left(\frac{1}{\alpha+i-1} \right) \delta_{\theta_j}
\]
...
\[ 
\theta_i| \theta_{-i} \sim \left( \frac{a}{a+n-1} \right)G_0(\theta_i) + \sum_{j\ne i} \left(\frac{1}{\alpha+n-1} \right) \delta_{\theta_j}
\]
or, equivalently, 
\[ 
\theta_i| \theta_{-i} \sim \left( \frac{a}{a+n-1} \right)G_0(\theta_i) + \sum_{h=1}^{H^{(-i)}} \left(\frac{n_h^{(-i)}}{\alpha+n-1} \right) \delta_{\theta_h^*}
\]
...
\alpha \int p(y_i;\theta) dG_0(\theta) & h=H^{(-i)}+1
\end{array} \right.
\]
\pause
...
\alpha t_{2c}(y_i;m,v^2[d/c]) & h=H^{(-i)}+1
\end{array} \right.
\]
\pause
...
\[ 
\alpha|\ldots \sim Ga(a+H-1, b - \sum_{h=1}^{H-1} \log(1-V_h))
\]
\end{frame}
