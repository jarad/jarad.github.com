...
&= \left[ \prod_{t=1}^T N(y_t;a_t(\psi),R_t(\psi)) \right] p(\psi).
\end{array} \]
  For any $\psi$, the $a_t(\psi)$ and $R_t(\psi)$ can be obtained using the
  ...
  \[
  \rho = \rho(\psi^{(i)},\psi^*)
  = \frac{p\left(\psi^{(i)}|y\right)}{p\left(\psi^*|y\right)},
  \]
...
d <- expand.grid(variable = c("sigma","sigma_beta"),
  ...
  mutate(value = rgamma(n(),1) + (variable == "sigma")) # dummy data
