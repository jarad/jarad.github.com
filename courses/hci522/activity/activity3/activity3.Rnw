...
(integers only).
The satisfcation ratings are available in the file {\texttt ikea\_rating.csv}.
...
\begin{answer}
<<>>=
1-pt((7-6.8)/(1.01/sqrt(15)), df = 15-1)
@
\end{answer}
...
<<>>=
a <- 1-0.95
6.8 + c(-1,1)*qt(1-a/2, df = 15-1)*1.01/15
@
\end{answer}
...
mu_your <- rt(n_reps, df = 15-1)*(1.01/sqrt(15)) + 6.8
