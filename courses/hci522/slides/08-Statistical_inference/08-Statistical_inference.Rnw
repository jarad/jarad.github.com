...
\begin{definition}
A \alert{random sample} is a sampling technique that uses random mechanism to
...
\includegraphics[width=0.6\textwidth]{causal_inference}
\end{frame}
...
or no chatbot.
Amongst the 21 in the chatbot group,
the sample mean time was 2 minutes with a standard deviation of 1 minute.
Amongst the 19 in the no chatbot group,
the sample mean time was 1.5 minutes with a standard deviation of 0.75 minutes.
