...
\item What is the mean score on \url{http://mouseaccuracy.com} (normal, medium, 30 seconds) of students in the eSports club at Iowa State Univesity? \pause
\item
\end{itemize}
\end{frame}
...
so that so that you can determine if your experiment supports, or refutes,
your hypothesis.
\pause
