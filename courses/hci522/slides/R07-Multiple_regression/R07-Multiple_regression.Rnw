...
\title{R07- Multiple (Linear) Regression}
...
\frame{\frametitle{independent variables}
...
\pause
  ...
  \item Higher order terms ($X^2$) \pause
	\item Additional independent variables ($X_1,X_2$) \pause
...
\frame{\frametitle{Parameter interpretation}
  Model:
	...
	\begin{itemize}
	\item $\beta_0$ is the mean value of the dependent variable $Y_i$ when \alert{all} independent variables are zero. \pause
	\item $\beta_p,\,p\ne 0$ is the mean increase in the dependent variable for a one-unit increase in the $p^{th}$ independent variable \alert{when all other independent variables are held constant}. \pause
	\item $R^2$ is the proportion of the variability in the dependent variable explained by the model
	\end{itemize}
...
\subsection{Additional independent variables ($X_1+X_2$)}
...
\begin{itemize}
\item Intercept ($\beta_0$): The mean count of Longnose Dace when maximum depth and nitrate concentration are both zero is -18
(\Sexpr{round(ci[1,1])}, \Sexpr{round(ci[1,2])}).
\pause
\item Coefficient for maxdepth ($\beta_1$): Holding nitrate concentration constant,
each cm increase in maximum depth is associated with an additional 0.48
(\Sexpr{round(ci[2,1],2)}, \Sexpr{round(ci[2,2],2)})
Longnose Dace counted on average.
\pause
\item Coefficient for no3 ($\beta_2$): Holding maximum depth constant, each
mg/liter increase in nitrate concentration is associated with an addition 8.3
(\Sexpr{round(ci[3,1],1)}, \Sexpr{round(ci[3,2],1)})
Longnose Dace counted on average.
\pause
...
% % \begin{itemize}[<+->]
% % \item $\beta_0 = E[Y_i| \mbox{reference level}]$, i.e. mean dependent variable for the reference level
% %
% % {\color{gray}
% % Note: the only way $X_{i,1}=\cdots=X_{i,p}=0$ is if all indicators are zero, i.e. at the reference level.
% % }
% %
% % \item $\beta_p, p>0$: mean change in the dependent variable moving from the reference level to the level associated with the $p^{th}$ dummy variable
% %
% % {\color{gray}
...
% % For example,
% \begin{itemize}
% \item The mean lifetime for mice on the N/N85 diet is 32.7 weeks. \pause
% \item The mean increase in lifetime for mice on the N/R40 diet compared to the N/N85 diet is 12.4 weeks. \pause
...
of them has on the mean dependent variable depends on the value of the other.
\end{quote}
...
\item Longnose dace count: The effect of nitrate (no3) on longnose dace count depends on the maxdepth. \pause (Continuous-continuous)
\end{itemize}
...
\begin{itemize}
\item $Y_i$ be the dependent variable,
...
%
% \begin{itemize}
% \item $\beta_0$ is the mean dependent variable for category A and type 0
% \item $\beta_1$ is the change in dependent variable for moving from type 0 to type 1
% \item $\beta_2$ is the change in dependent variable for moving from category A to category B
% \item $\beta_3$ is the change in dependent variable for moving from category A to category C
% \end{itemize}
%
% \end{frame}
...
% {\footnotesize
% \begin{itemize}
% \item $\beta_0$ is the mean dependent variable for category A and type 0 \pause
% \item $\beta_1$ is the change in dependent variable for moving from type 0 to type 1 for category A \pause
% \item $\beta_2$ is the change in dependent variable for moving from category A to category B for type 0
% \item $\beta_3$ is the change in dependent variable for moving from category A to category C for type 0 \pause
% \item $\beta_4$ is the difference in change in dependent variable for moving from category A to category B for type 1 compared to type 0 \pause
% \item $\beta_5$ is the difference in change in dependent variable for moving from category A to category C for type 1 compared to type 0
% \end{itemize}
% }
%
% \end{frame}
...
\frametitle{Interpretation}
\begin{itemize}
\item In block B1 and treatment L, the mean cover is
\Sexpr{round(coef(mM)[1],1)} (\Sexpr{round(ci[1,1],1)}, \Sexpr{round(ci[1,2],1)}).
\pause
\item The mean increase in cover from block B2 to block B2 is
\Sexpr{round(coef(mM)[2],1)} (\Sexpr{round(ci[2,1],1)}, \Sexpr{round(ci[2,2],1)})
while holding treatment constant.
\pause
\item The mean increase in cover from treatment L to treatment Lf is
\Sexpr{round(coef(mM)[3],1)} (\Sexpr{round(ci[3,1],1)}, \Sexpr{round(ci[3,2],1)})
while holding block constant.
\pause
\item The mean increase in cover from treatment L to treatment LfF is
\Sexpr{round(coef(mM)[4],1)} (\Sexpr{round(ci[4,1],1)}, \Sexpr{round(ci[4,2],1)})
while holding block constant.
\pause
\item The main effects model explains \Sexpr{round(100*summary(mM)$r.squared)}\%
of the variability in cover.
\end{itemize}
\end{frame}
...
\begin{itemize}
\item In block B1 and treatment L, the mean cover is
\Sexpr{round(coef(mI)[1],1)} (\Sexpr{round(ci[1,1],1)}, \Sexpr{round(ci[1,2],1)}).
\pause
\item The mean increase in cover from block B2 to block B2 is
\Sexpr{round(coef(mI)[2],1)} (\Sexpr{round(ci[2,1],1)}, \Sexpr{round(ci[2,2],1)})
in treatment L.
\pause
\item The mean increase in cover from treatment L to treatment Lf is
\Sexpr{round(coef(mI)[3],1)} (\Sexpr{round(ci[3,1],1)}, \Sexpr{round(ci[3,2],1)})
in block B1.
\pause
\item The mean increase in cover from treatment L to treatment LfF is
\Sexpr{round(coef(mI)[4],1)} (\Sexpr{round(ci[4,1],1)}, \Sexpr{round(ci[4,2],1)})
in block B1.
\pause
\item The main effects model explains \Sexpr{round(100*summary(mI)$r.squared)}\%
of the variability in cover.
\end{itemize}
\end{frame}
...
\begin{itemize}
\item $Y_i$ be the dependent variable
\item $X_{i,1}$ be the continuous independent variable,
\item $B_i$ be a dummy variable for category B, and
\item $C_i$ be a dummy variable for category C.
\end{itemize}
...
%      xlab="Continuous independent variable", ylab="mean dependent variable",
...
% b1 = c(1, .5, -0.3)
% plot(0, 0, type='n', xlim=c(0,10), ylim=c(0,15), axes=F, frame=TRUE,
%      xlab="Continuous independent variable", ylab="",
...
\frametitle{Interpretation}
\begin{itemize}
\item For echo-locating bats that weigh 1 gram (log(Mass) is 0), the mean energy
expenditure is
\Sexpr{round(coef(mM)[1],1)} (\Sexpr{round(ci[1,1],1)}, \Sexpr{round(ci[1,2],1)})
watts.
\pause
\item The mean increase in the logarithm of energy expenditure per unit increase
in the logarithm of mass is
\Sexpr{round(coef(mM)[2],1)} (\Sexpr{round(ci[2,1],1)}, \Sexpr{round(ci[2,2],1)})
while holding type of bird or bat constant.
\pause
\item The mean increase in the logarithm of energy expenditure from echolocating
bats to non-echolocating bats is
\Sexpr{round(coef(mM)[3],1)} (\Sexpr{round(ci[3,1],1)}, \Sexpr{round(ci[3,2],1)})
while mass constant.
\pause
\item The mean increase in the logarithm of energy expenditure from echolocating
bats to non-echolocating birds is
\Sexpr{round(coef(mM)[4],1)} (\Sexpr{round(ci[4,1],1)}, \Sexpr{round(ci[4,2],1)})
while mass constant.
\pause
\item The main effects model explains \Sexpr{round(100*summary(mM)$r.squared)}\%
...
\frametitle{Interpretation}
\begin{itemize}
\item For echo-locating bats that weigh 1 gram (log(Mass) is 0),
the mean logarithm of energy expenditure is
\Sexpr{round(coef(mI)[1],1)} (\Sexpr{round(ci[1,1],1)}, \Sexpr{round(ci[1,2],1)})
watts.
\pause
\item The mean increase in the logarithm of energy expenditure per unit increase
...
\begin{itemize}
\item $Y_i$ be the dependent variable
\item $X_{i,1}$ be the first \alert{continuous} independent variable and
\item $X_{i,2}$ be the second \alert{continuous} independent variable.
\end{itemize}
...
% \begin{frame}
% \begin{itemize}
% \item $\beta_0$: mean dependent variable when independent variables are zero
% \item $\beta_1+\beta_3x_2$: mean change in the dependent variable for each unit change in $X_{i,1}$ when \alert{$X_{i,2}=x_2$}
% \item $\beta_2+\beta_3x_1$: mean change in the dependent variable for each unit change in $X_{i,2}$ when \alert{$X_{i,1}=x_1$}
% \end{itemize}
%
% \pause
%
...
% \item $\beta_1+\beta_3x_2$: mean change in the dependent variable for each unit change in $X_{i,1}$ when \alert{$X_{i,2}=x_2$}
% \item $\beta_2+\beta_3x_1$: mean change in the dependent variable for each unit change in $X_{i,2}$ when \alert{$X_{i,1}=x_1$}
% %\item $\beta_3X_{i,2}$: mean change in the effect of $X_{i,1}$ on the dependent variable when \alert{$X_{i,2}$ is not zero}
% %\item $\beta_3X_{i,1}$: mean change in the effect of $X_{i,2}$ on the dependent variable when \alert{$X_{i,1}$ is not zero}
% \end{itemize}
%
% \pause
%
...
\subsection{Summary}
\begin{frame}
\frametitle{Multiple regression independent variables}

The possibilities for independent variables are
  \begin{itemize}
  \item Higher order terms ($X^2$)
	\item Additional independent variables ($X_1$ and $X_2$)
