...
\item with the same mean $\mu$ \pause and
\item and standard deviation $\sigma$, \pause
\item that are independent.
\end{itemize}
...
\[
\mu|y \sim t_{n-1}(\overline{y}, s^2)
\]
\pause
where
\begin{itemize}
...
\item $t_{n-1}(\overline{y}, s^2)$ is a $T$ distribution \pause with
  \begin{itemize}
