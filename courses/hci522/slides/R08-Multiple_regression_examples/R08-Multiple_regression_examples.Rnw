...
\Sexpr{round(100*summary(m)$r.squared)}\% of the variability in children's height.
\end{itemize}
\end{frame}
...
\Sexpr{round(co[1],0)} in (\Sexpr{round(ci[1,1],0)},\Sexpr{round(ci[1,2],0)}).
\pause
\item For wool type A when moving from tension low to tension medium,
the mean number of breaks is
\Sexpr{-round(co[2],0)} (\Sexpr{-round(ci[2,2],0)},\Sexpr{-round(ci[2,1],0)})
lower.
\pause
\item For wool type A when moving from tension low to tension high,
the mean number of breaks is
\Sexpr{-round(co[3],0)} (\Sexpr{-round(ci[3,2],0)},\Sexpr{-round(ci[3,1],0)})
lower.
\pause
\item The model with tension
...
\Sexpr{round(co[1],0)} in (\Sexpr{round(ci[1,1],0)},\Sexpr{round(ci[1,2],0)}).
\pause
\item When moving from tension low to tension medium,
the mean number of breaks is
\Sexpr{-round(co[2],0)} (\Sexpr{-round(ci[2,2],0)},\Sexpr{-round(ci[2,1],0)})
lower while holding wool type constant.
\pause
\item When moving from tension low to tension high,
the mean number of breaks is
\Sexpr{-round(co[3],0)} (\Sexpr{-round(ci[3,2],0)},\Sexpr{-round(ci[3,1],0)})
lower while holding wool type constant.
\pause
