...
\frametitle{Sign test}
  The sign test calculates the probability of observing this many ones (or more extreme) if the null hypothesis is true. \pause For our one-sided hypothesis (removing leaves will decrease rusty leaves), that is the probability of observing 6, 7, or 8 ones. \pause This is 
...
\frame{\frametitle{Wilcoxon signed-rank test}
  Also known as the Wilcoxon signed-rank test: \pause
	...
	\item Calculate $Z=(S-E[S]+c)/SD[S]$ where $c$ is the appropriate continuity correction.
...
\frametitle{Rank-sum test}
  Also referred to as the Wilcoxon rank-sum test and the Mann-Whitney U test: \pause
	...
	\item Calculate $SD(U) = s_R \sqrt{\frac{n_1 n_2}{(n_1+n_2)}}$
		...
		\end{enumerate}
	\item Calculate $Z=(U+c-E[U])/SD(U)$ where c, the continuity correction, is either 0.5 or -0.5.
