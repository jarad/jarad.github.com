...
\frame{\frametitle{Paired t-test example}
	In the minilecture on the paired t-test, \pause we had
	\begin{itemize}[<+->]
	\item a one-sided hypothesis, namely the difference is greater than zero
	\item a test statistic $t=2.43$
	\item which has a $t$ distribution with 7 degrees of freedom
	\end{itemize}
