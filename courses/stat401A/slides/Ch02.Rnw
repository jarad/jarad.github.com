...
\frame{\frametitle{Z-statistics}
	Let's assume, we have 
	\begin{itemize}
	\item calculated a test statistic $z$ that
	\item has a $Z\sim N(0,1)$ sampling distribution if the null hypothesis is true. 
	\end{itemize}
	
	{\tiny We could easily replace $z$ with $t$ and have a sampling distribution that is $t_{v}$ for some $v$.}
	
	\vspace{0.2in} \pause
	
	Now, we can have one of three types of hypotheses:
	\begin{itemize}[<+->]
	\item Two-sided: $P(|Z|>|z|)$ \uncover<6->{$=P(Z>|z|)+P(Z<-|z|)$} 
	
	\uncover<7->{\hspace{1.6in}$=2P(Z<-|z|)$}
	\item One-sided:
		\begin{itemize}
		\item $P(Z>z)$ \uncover<8->{$=P(Z<-z)$}
		\item $P(Z<z)$ 
		\end{itemize}
	\end{itemize}
	...
	\uncover<9->{$F(c)=P(Z<c)$ is the \alert{cumulative distribution function} for the standard normal.}
}

\begin{frame}[fragile]
\frametitle{Symmetric distributions}
	The standard normal and $t$ distributions are both symmetric around zero.  \pause
<<z, echo=FALSE>>=
xx = seq(-3,-1.5,by=0.1)
yy = dnorm(xx)
par(mar=c(4,4,0,0)+.1)
...
axis(1, c(-1,0,1)*1.5, labels=c("-c","0","c"))
@
	\pause
	
	\[ P(Z>c) = P(Z<-c) \quad \mbox{blue area is equal to red area}  \]
\end{frame}


\subsection{Example}
\frame{\frametitle{Paired t-test example}
	In a paired t-test, \pause we might have
	\begin{itemize}[<+->]
	\item a one-sided hypothesis that the difference is greater than zero
	\item a test statistic $t=2.43$
	\item which has a $t$ distribution with 7 degrees of freedom when the null hypothesis is true.
	\end{itemize}
	
	\vspace{0.2in} \pause
	
	So we need to calculate 
	\[ P(t_{7}>2.43)\pause =1-P(t_7<2.43) \] \pause
	where $t_{7}$ represents a t-distribution with 7 degrees of freedom. 
}

\frame[containsverbatim]{\frametitle{Using SAS or R}
In SAS,
\begin{verbatim}
PROC IML;
  p = 1-CDF('T', 2.43, 7);
...
\frame{\frametitle{Paired t-test example}
  In a paired t-test, \pause we might have
	...
	So we need to calculate 
	\[ P(t_{7}>2.43) =1-P(t_7<2.43)=0.02 \] 
	
  \vspace{0.2in} \pause
  
  So, we reject the null hypothesis that there is no difference\pause, but how big is the difference?
}
...
Key steps in confidence interval construction:
\begin{enumerate}
\item Calculate statistic
\item Calculate standard error of the statistic
\item Find the appropriate critical value
\item Construct the interval
  \begin{itemize}
  \item Two-sided interval:
  \[ \mbox{statistic} \pm \mbox{critical value}\times\mbox{standard error} \]
  \item One-sided interval:
  \[ \mbox{statistic} - \mbox{critical value}\times\mbox{standard error} \]
  or
  \[ \mbox{statistic} + \mbox{critical value}\times\mbox{standard error} \]
  \end{itemize}
\end{enumerate}
...
\frametitle{Critical values}
	A related quantity are critical values for confidence interval construction, \pause e.g. 
	\[ (\overline{Y}_2-\overline{Y}_1) \pm \alert{t_{v}(1-\alpha/2)} SE(\overline{Y}_2-\overline{Y}_1). \]
...
curve(dnorm, -3, 3, ylab="Probability density function", 
      axes=F, frame=T, cex.main=1.5)
polygon(-c(xx,-1.5,-3), c(yy,0,0), col="red", border=NA)
axis(1, c(-1,0,1)*1.5, labels=expression(c("","0",expression("t[df](1-alpha/2)"))))
@
	The red area is $\alpha/2$. \pause Let $c=t_{v}(1-\alpha/2)$, \pause then we need $P(t_{v} < c) = 1-\alpha/2$.
\end{frame}
...
% Need example of confidence interval construction
...
\frame{\frametitle{Pvalue}
	...
	where 
	$t_{v}$ is a t-distribution with $df$ degrees of freedom. 
...
\frame{\frametitle{Confidence interval}
	...
	where 
	\pause $\pm$ indicates plus and minus \pause and $t_{v}(1-\alpha/2)$ is the value such that $P(t_{v} < t_{v}(1-\alpha/2))=1-\alpha/2$. \pause If $\alpha=0.05$ and $df=326$, then $t_{v}(1-\alpha/2)= 1.97$. \pause
