...
\frame{
\frametitle{Hypothesis framework}
{\small
	Let's assume, we have 
	\begin{itemize}[<+->]
  \item $Y_i \stackrel{iid}{\sim} N(\mu,\sigma^2)$ for $i=1,\ldots,n$ and have
	\item calculated a test statistic $t$, and
  ...
  \end{itemize}
	...
	\uncover<9->{$F(c)=P(t_\nu<c)$ is the \alert{cumulative distribution function} for a $t$ distribution with $\nu$ degrees of freedom.}
}
}
...
@
	...
	\[ P(T_\nu>c) = P(t_\nu<-c) \quad \mbox{blue area is equal to red area}  \]
\end{frame}


\frame{\frametitle{Paired t-test example}
	In the paired t-test example, we had a test statistic $t=2.43$ with a $t_7$ sampling distribution if the null hypothesis is true.
...
\begin{enumerate}
\item Calculate point estimate
\item Calculate standard error of the statistic
\item Set error level $\alpha$
...
@
\pause 
Let $c=t_{\nu}(1-\alpha/2)$, \pause then we need $P(t_{\nu} < c) = 1-\alpha/2$, i.e. the inverse of the cumulative distribution function.
\end{frame}



\frame{\frametitle{Paired t-test example}
  In paired t-test example, we had an estimate $\hat{\mu} = 10.5$ and a standard error of $4.3136$ with $7$ degrees of freedom. 
  ...
  \[ 10.5\pm 2.364624\cdot 4.3136 = (0.30,20.7) \]
  \item One-sided (positive) ($t_7(.95)=1.894579$) 
  \[ (10.5- 1.894579\cdot 4.3136,\infty) = (2.33,\infty) \]
  \item One-sided (negative) ($t_7(.95)=1.894579$)
  \[ (-\infty,10.5+ 1.894579\cdot 4.3136) = (-\infty,18.7) \]
  \end{itemize}
}
...
\frame{\frametitle{Two-sample t-test example}
  In the two-sample t-test example, we had an estimate $\hat{\mu_1-\mu_2} = 10.33643$ and a pooled standard error of $0.8190$ with $326$ degrees of freedom. 
  
  \vspace{0.2in} \pause
  
	The 90\%, i.e. $\alpha=0.10$, confidence intervals for $\mu$ are 
  \begin{itemize}
  \item Two-sided ($t_{326}(.95)=1.649541$) 
  \[ 10.33643\pm 1.649541\cdot 0.8190 = (9.0,11.7) \]
  \item One-sided (positive) ($t_{326}(.90)=1.285149$) 
  \[ (10.33643- 1.285149\cdot 0.8190,\infty) = (9.3,\infty) \]
  \item One-sided (negative) ($t_{326}(.90)=1.285149$)
  \[ (-\infty,10.33643+ 1.285149\cdot 0.8190) = (-\infty,11.4) \]
  \end{itemize}
}
