...
\frame{\frametitle{Z-statistics}
	...
	{\tiny We could easily replace $z$ with $t$ and have a sampling distribution that is $t_{\nu}$ for some $\nu$.}
...
\frametitle{Critical values}
	...
	\[ (\overline{Y}_2-\overline{Y}_1) \pm \alert{t_{\nu}(1-\alpha/2)} SE(\overline{Y}_2-\overline{Y}_1). \]
...
@
	The red area is $\alpha/2$. \pause Let $c=t_{\nu}(1-\alpha/2)$, \pause then we need $P(t_{\nu} < c) = 1-\alpha/2$.
\end{frame}
