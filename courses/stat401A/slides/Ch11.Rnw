...
\frame{\frametitle{Case statistics}
  \begin{definition}
	\alert{Leverage} is a measure of the distance between an observation's explanatory variable values and the average of the explanatory variable values in the entire data set. \vspace{0.2in} \pause 
	...
	\begin{definition}
	\alert{Cook's distance} is a measure of the \alert{overall} effect on estimated regression coefficients when removing an observation.
...
95\% of studentized residuals should be with -2 and 2. 
}
