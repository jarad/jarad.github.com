...
The F-test from the ANOVA table tests the null hypothesis $\beta_1=\cdots=\beta_p=0$.
...
The parameter estimates table provides tests and confidence intervals for individual $\beta_j$'s. 
...
\begin{frame}[containsverbatim]
\frametitle{SAS Code and Output - Type I and III SS}

Type I and III SS tables perform specific F-tests:
\begin{itemize}
\item Type I performs sequential tests
\item Type III performs conditional tests
\end{itemize}
...
& H_0 &H_1 \\
\hline 
\mbox{Type I SS} & \beta_0 & \beta_0 + \beta_1 X_1 \\
\mbox{Type III SS} & \beta_0 + \beta_2 X_2 + \beta_3 X_3 & \beta_0 + \beta_1 X_ 1 + \beta_2 X_2 + \beta_3 X_3 \\
\hline
\end{array} \]
