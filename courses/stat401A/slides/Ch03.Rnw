...
\begin{definition}
In statistics, \alert{data transformation} refers to the application of a deterministic mathematical function to each point in a data set \pause — that is, each data point $z_i$ is replaced with the transformed value $y_i = f(z_i)$, where $f$ is a function. 
\end{definition}
