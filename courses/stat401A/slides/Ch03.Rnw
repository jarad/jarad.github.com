...
\begin{itemize}[<+->]
\item If $y\in (0,1)$, then $f(y) = sin^{-1}(\sqrt{y})$.
...
If $z_{ij}=log(y_{ij})$ \pause and we run a two-sample t-test on the z's, \pause then we assume
...
xtable(ddply(d, .(country), summarise, n=length(mpg), mean=mean(mpg), sd=mean(mpg)))
@
...
Logged:
<<echo=FALSE, results='asis'>>=
xtable(ss<-ddply(d, .(country), summarise, n=length(lmpg), mean=mean(lmpg), sd=mean(lmpg)))
@
