...
\[ \begin{array}{rl}
\mu_i =&  \beta_0 + \beta_1 D_i + \beta_2 D_i^2 \\
&+ \beta_3 \I(V_i=A) D_i + \beta_4 \I(V_i=B) D_i  \\
&+ \beta_5 \I(V_i=A) D_i^2 + \beta_6 \I(V_i=B) D_i^2 
\end{array} \]
...
\begin{frame}
\frametitle{Why blocking helps}

Estimation of treatment effects is performed within block because all treatments are available within a block. 
