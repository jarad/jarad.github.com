...
\frame{\frametitle{Multiple regression}
  Recall the simple linear regression model is 
	...
	\pause where $Y_i$ is the response for observation $i$ and $X_{i,p}$ is the $p^{th}$ explanatory variable for observation $i$. 
}
...
par(mfrow=c(1,2), mar=c(5,4,0,0)+.1)
plot(count~no3, d)
plot(count~maxdepth, d)
...
\frame{\frametitle{Interpretation}
	...
	\item Coefficient of determination: The model explains 19\% of the variability in the count of Longnose Dace.
	\end{itemize}
}

\frame{\frametitle{Future}
	Possible explanatory variables:\pause
	\begin{itemize}[<+->]
	\item Additional explanatory variables  
	\item Higher order terms
	\item Dummy/indicator variables for categorical variables
	\item Interactions 
	\end{itemize}
}
