...
\frame{\frametitle{Regression with a categorical variable}
	...
	\[ \begin{array}{ll} 
	X_{i,1} = \I(\mbox{diet for observation $i$ is NP})\pause \\
	X_{i,2} = \I(\mbox{diet for observation $i$ is N/R50 lopro})\pause \\
	X_{i,3} = \I(\mbox{diet for observation $i$ is N/R50})\pause \\
	X_{i,4} = \I(\mbox{diet for observation $i$ is R/R50})\pause \\
	X_{i,5} = \I(\mbox{diet for observation $i$ is N/R40})\pause
	\end{array} \] 
...
For example, 
\begin{itemize}[<+->]
\item The expected lifetime for mice on the N/N85 diet is 37 weeks.
...
For example, 
\begin{itemize}
\item Longnose dace: The effect of no3 on longnose dace count depends on the maxdepth. (Continuous-continuous)
\item Case1002: The effect of mass on energy depends on the species type. (Continuous-categorical)
\item Yield: the effect of fertilizer type depends on the fertilizer brand (Categorical-categorical)
\end{itemize}
\end{frame}
...
\item $\beta_1$: expected change in the response for each change in $X_{i,1}$ when \alert{$X_{i,2}$ is zero}
\item $\beta_2$: expected change in the response for each change in $X_{i,2}$ when \alert{$X_{i,1}$ is zero}
...
\hline
&&&= \phantom{\beta_0+} &\beta_1 &&&+  \beta_3 &&x_2
\end{array} \]
...
<<>>=
d = read.csv("longnosedace.csv")
m = lm(count ~ maxdepth + no3 + maxdepth:no3, d)
summary(m)
@
...
Let category A be the reference level. Then observation $i$, let 
\begin{itemize}
\item $Y_i$ be the response 
...
<<>>=
summary(lm(log(Energy)~log(Mass)+Type, case1002))
@
\end{frame}
...
From \_The Statistical Sleuth (3rd ed)\_ page 250:
\begin{itemize}
\item when a question of interest pertains to an interaction
\item when good reason exists to suspect interaction or
