...
\item Yield: the effect of fertilizer level depends on the fertilizer brand (Categorical-categorical)
\end{itemize}
\end{frame}
...
\begin{frame}[fragile]
\frametitle{Illustration of interaction effect}

<<echo=FALSE, fig.width=8, out.width='0.9\\textwidth'>>=
opar = par(mfrow=c(1,2), mar=c(5,5,0,2)+.1)
b = coef(m)
plot(d$no3, b[2]+b[4]*d$no3, type="l", xaxs='i', axes=FALSE, frame=TRUE,
     xlab="Nitrate (mg/L)", ylab="Expected change in the longnose dace count\n for a one cm increase in maxdepth")
axis(1, unique(round(d$no3)))
axis(2, b[2], expression(beta[1]), las=1)
axis(2, c(.5,1,1.5))
x1 = 4; x2 = 5
segments(x1, b[2]+b[4]*x1, x2, b[2]+b[4]*x1)
segments(x2, b[2]+b[4]*x1, x2, b[2]+b[4]*x2)
text(x2, b[2]+b[4]*mean(c(x1,x2)), expression(beta[3]), pos=4)

mx = max(round(d$maxdepth))
plot(c(0,mx), b[3]+b[4]*c(0,mx), type="l", xaxs='i', xlim=c(0,mx), axes=FALSE, frame=TRUE,
     xlab="Max Depth (cm)", ylab="Expected change in the longnose dace count\n for a one mg/L increase in nitrate concentration")
axis(1, seq(from=0, to=mx, by=10))
axis(2, b[3], expression(beta[2]), las=1)
axis(2, seq(0,25, by=5))
x1 = 90; x2 = 100
segments(x1, b[3]+b[4]*x1, x2, b[3]+b[4]*x1)
segments(x2, b[3]+b[4]*x1, x2, b[3]+b[4]*x2)
text(x2, b[3]+b[4]*mean(c(x1,x2)), expression(10*beta[3]), pos=4)
text(mean(c(x1,x2)), b[3]+b[4]*x1, '10', pos=1)
par(opar)
@
...
\begin{itemize}[<+->]
\item $\beta_1$ is the change in the response when the continuous explanatory variable increases by one for any category
\item when the continuous explanatory variable is zero 
  \begin{itemize}
  \item $\beta_0\phantom{+\beta_2}\,\,$ is the expected response for category A
  \item $\beta_0+\beta_2$ is the expected response for category B
  \item $\beta_0+\beta_3$ is the expected response for category C
  \end{itemize}
\end{itemize}
\end{frame}
...
{\footnotesize
\begin{itemize}[<+->]
\item $\beta_0$ is the expected response when the continuous explanatory variable is zero and we are in category A
\item $\beta_1$ is the change in the response when the continuous explanatory variable increases by one for category A
\item When the continuous explanatory variable ($X_{i,1}$) is zero,
  \begin{itemize}
  \item $\beta_2$ is the change in the expected response when moving from category A to category B 
  \item $\beta_3$ is the change in the expected response when moving from category A to category C
  \end{itemize}
\item $\beta_1+\beta_4$ is the change in the response when the continuous explanatory variable increases by one for category B
\item $\beta_1+\beta_5$ is the change in the response when the continuous explanatory variable increases by one for category C 
\end{itemize}
}
\end{frame}
...
loc=c(1,3,6)
arrows(loc[1],b[1]+loc[1], loc[1], b[2]+loc[1], code=3)
text(loc[1], b[2]/2+loc[1], expression(beta[2]), pos=4)
arrows(loc[2], b[1]+loc[2], loc[2], b[3]+loc[2], code=3)
text(loc[2], mean(b[c(1,3)])+loc[2]+1, expression(beta[3]), pos=4)
arrows(loc[3], b[2]+loc[3], loc[3], b[3]+loc[3], code=3)
...
\begin{frame}[containsverbatim]
\frametitle{SAS code and output - with interaction}
...
\begin{frame}[fragile]
\frametitle{R code and output - with interaction}
<<>>=
...
summary(lm(Cover~Block+Treat, case1301, subset=Block %in% c("B1","B2") & Treat %in% c("L","Lf","LfF")))
@
