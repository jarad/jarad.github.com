...
\section{Exploratory analysis - partial summary}
\begin{frame}[fragile]
<<>>=
...
d_mse = data.frame(Response = ifelse(lg, "log(Price)", "Price"),
                   Interactions = rep(c("No","Yes"),4),
                   Criterion = c("AIC","BIC"),
                   MSE = round(MSE), 
                   Ratio = round(MSE/min(MSE),2))
@

<<mse, results='asis'>>=
# print(xtable(d_mse), include.names=FALSE)
@
...
new$YrSld = 2012
predict(mod.final, new, interval="confidence")
predict(mod.final, new, interval="prediction")
@
