...
\frametitle{Two-group analysis}
  ...
  \end{itemize}
  where 
  \[ s_p = \sqrt{\frac{ (n_1-1)s_1^2 + (n_2-1)s_2^2 }{n1+n_2-2}} \]
...
\frametitle{Multi-group analysis}
  ...
  but now	$j=1,\alert{\ldots,J}$ and $i=1,\ldots,n_j$
  ...
  \end{itemize}
  where 
  \[ s_p = \sqrt{\frac{ (n_1-1)s_1^2 + (n_2-1)s_2^2 \alert{+ \cdots + (n_J-1)s_J^2} }{n1+n_2\alert{+\cdots+n_J-J}}} \]
\end{frame}
...
\frametitle{Hypothesis test for comparison of two means (in multi-group data)}
  If $Y_{ij} \stackrel{ind}{\sim} N(\mu_j,\sigma^2)$ for $j=1,\ldots,J$ and we want to test the hypothesis
	...
	\pause and
	\[ s_p = \sqrt{\frac{(n_1-1)s_1^2 + (n_2-1)s_2^2+\cdots+(n_J-1)s_J^2}{n_1+n_2+\cdots+n_J - J }} \]
  and we compare this to a $t$ distribution with $n_1+n_2+\cdots+n_J-J$ degrees of freedom. 
\end{frame}
...
\[ H_0: \mu_4=\mu_1 \mbox{ vs } H_a: \mu_4\ne \mu_1. \]
\end{frame}
...
\begin{frame}[fragile]
\frametitle{Confidence interval for difference of two means (in multi-group data)}

  If $Y_{ij} \stackrel{ind}{\sim} N(\mu_j,\sigma^2)$ for $j=1,\ldots,J$, a $100(1-\alpha)$\% confidence interval for $\mu_1-\mu_2$ is 
	\pause 
	\[ \overline{Y}_1-\overline{Y}_2 \pm t_{df}(1-\alpha/2) SE(\overline{Y}_1-\overline{Y}_2) \]
	\pause where the $t$ critical value, $t_{n_1+n_2+\cdots+n_J-J}(1-\alpha/2)$, needs to be calculated using a statistical software.

  \vspace{0.2in} \pause
  
  For a 95\% confidence interval for the difference in mean lifetime between NP and N/N85 ($\mu_1-\mu_2$), we have 
<<>>=
...
\frametitle{One-way ANOVA F-test}
  ...
  \item[$H_a$:] at least one of the means is different
  \end{itemize}
...
\frame{\frametitle{Full vs reduced model}
  ...
  \vspace{-0.2in}
	...
	\mbox{Assumption} & 
	\uncover<2->{H_a:  Y_{ij} \stackrel{ind}{\sim} N\left(\mu_j, \sigma^2\right)} & \uncover<6->{
...
\frame{\frametitle{Testing Composite hypotheses}
	If $Y_{ij} \stackrel{ind}{\sim} N(\mu_i,\sigma^2)$ for $i=1,\ldots,J$ and we want to test the \alert{composite hypothesis} 
	\begin{itemize}
	\item $H_0: \mu_i=\mu_j$
	\item $H_1: \mu_i\ne \mu_j$ for some $i\ne j$
	\end{itemize}
