...
\frame{\frametitle{Multiple comparisons}
  How many combinations when choosing 2 items out of 6? \pause
	\[ {6\choose 2} \pause = \frac{6!}{2!(6-2)!} = \frac{6}{2!\cdot 4!} = \frac{6\cdot5}{2} = 15 \]
	\pause If 0.05 is our significance cutoff, what is the probability of falsely rejecting at least one true null hypothesis? \pause
	\[ 1-(1-0.05)^{15} \pause = 1-(0.95)^{15} = 1-0.46 = 0.54 \]
	
	\pause So there is a greater than 50\% probability of falsely rejecting a true null hypothesis! 
	
	\vspace{0.3in} \pause
	
	But we wanted the probability of making a mistake to be 0.05. 
}


\subsection{Multiple comparison adjustments}
\frame{\frametitle{Multiple comparison adjustments}
	...
	\hline\pause
	LSD & $t_{n-\I}(1-\alpha)$ & After significant $F$-test \\
	&& (no adjustment) \pause \\
	Dunnett && Compare all groups to control \pause \\
	Tukey-Kramer & $q_{\I,n-\I}(1-\alpha)/\sqrt{2}$ & All pairwise comparisons \pause \\
	Scheff\'e & $\sqrt{(\I-1)F_{(\I-1,n-\I)}(1-\alpha)}$ & All contrasts \pause \\
	\alert{Bonferroni} & $t_{n-\I}(1-\alpha/2k)$ &$k$ tests \\
	& $k=\I(\I-1)/2$ & (most generic) \\
	\hline
	\end{tabular}
	\end{center}
	}
...
\subsection{Example - Bonferroni adjustment}
\frame[containsverbatim]{\frametitle{SAS code and output for one-way ANOVA}
\tiny
\begin{verbatim}
                                        The GLM Procedure
...
\small
	Frankly, we've learned very little. 
	
	\vspace{0.1in} \pause
	
	We've learned that if $\mu_i= \mu_j$ for the following pairs, the data are highly unlikely: \pause
	\begin{multicols}{3}
	\begin{itemize}
	\item N/N85-N/R40
	\item N/N85-N/R50
	...
	\begin{itemize}
	\item[] Is this information useful? \pause 
	\item[] If you wanted the longest lifetime, which diet should you prefer? \pause 
	\item[] Does it really make a difference?
	\end{itemize}
}
