...
\begin{frame}
\frametitle{Probability of a Type I error}


\begin{definition}
A \alert{type I error} is the probability of rejecting a true null hypothesis or, equivalently, 
\end{definition}

\vspace{0.2in} \pause

\begin{remark}
We set $\alpha$ (=0.05) to control this error rate. 
\end{remark}

\vspace{0.2in} \pause

But if we do two independent tests, what is the error rate now? \pause 
\[ P(\mbox{at least 1 type I error}|\mbox{$m$ tests}) =  1-(1-\alpha)^m \] 
\pause
which is about 10\% 
...
\frametitle{Type I error for all pairwise comparisons of $J$ groups}
  ...
  which is $m=15$ tests if $J=6$. 
	\pause If $\alpha=0.05$ is our significance cutoff, what is the probability of falsely rejecting at least one true null hypothesis? \pause
