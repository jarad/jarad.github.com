...
binomial probability of success when $n=13$ and $\theta=0.09$.
...
So evaluate consistency by simulation, 
...
Make sure to look at the y-axis.
...
sim_study <- ddply(settings, .(n, theta), function(x) {
  ...
  lower <- qbeta(.025, 0.5+y, 0.5+x$n-y)
  upper <- qbeta(.975, 0.5+y, 0.5+x$n-y)
  
  # Fix intervals when y=0
  lower[y==0] <- 0
  upper[y==0] <- qbeta(.95, 0.5+0, 0.5+x$n-0)
  
  # Fix intervals when y=n
  upper[y==x$n] <- 1
  lower[y==x$n] <- qbeta(.05, 0.5+x$n, 0.5+x$n-x$n)
