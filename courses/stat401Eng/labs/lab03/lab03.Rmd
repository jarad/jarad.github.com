...
provides a way to evaluate the Monte Carlo error in our estimate. 
...
(multiplied by the width of the interval which is just 1 in this example)
...
y <- 2*exp(-abs(x))
m <- mean(y)
s <- qnorm(.975)*sd(y)/sqrt(length(y))
m + c(-1,1)*s

# Compare with the truth
integrate(function(x) exp(-abs(x)), lower = -1, upper = 1)
```
</div> 
