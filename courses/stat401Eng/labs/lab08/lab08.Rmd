...
If you wanted an alternative level to be the reference level, 
use `relevel()`.

```{r}
case0501$Diet <- relevel(case0501$Diet, ref = "NP")
table(case0501$Diet)
```

Notice how NP is now the first level. 

Now  when you run the regression the reference level will be NP. 
