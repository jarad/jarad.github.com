...
\item $A \subset \Omega$ means the set $A$ is a subset of $\Omega$
\end{itemize}
\end{definition}
...
\begin{example}
The set of all possible sums of two 6-sided dice rolls are 
...
For the following $A,B \subset \Omega$,
...
But, of course, our real world experience would favor to pick model 1 
...
{\textcolor{cyan}{Independence: }}
    ...
    probabilities: $P(A \intersection B) = P(A) \cdot P(B)$\\[.01in]
...
\item any $A$ and $A^C$ where $A \subset \Omega$
\end{itemize}
...
\[ \begin{array}{rl}
P(7|\mbox{Win}) 
&= \frac{P(\mbox{Win}|7)P(7)}{\sum_{i=2}^{12} P(\mbox{Win}|i)P(i)}  \\ \\
&= \frac{P(7)}{P(7)+P(11)}.
\end{array} \]
