...
\pause
(i.e. the explanatory variable only has two values), 
\pause
we can code it as 
\[ \begin{array}{rl} 
X_i &=
\left\{ 
\begin{array}{rl} 
1 & \mbox{obseration $i$ is one state} \\ 
0 & \mbox{observation $i$ is the other state} 
\end{array} 
\right.
\pause \\ \\
&= \I(\mbox{observation $i$ is ``one state''})
\end{array} \]
...
Then 
\begin{itemize}[<+->]
\item $\beta_0\phantom{+\beta_1}\,\,\,$ is the expected response for the ``other state'', 
\item $\beta_0+\beta_1$ is the expected response for the ``one state'', and therefore
\item $\beta_1$ is the expected difference (``one state'' minus ``other state'') in lifetimes.
\end{itemize}
...
E[\mbox{Lifetime}|\mbox{N/R50}] &= \pause E[Y_i|X_i=1] &= \pause \beta_0 +\beta1 \pause 
\end{array} \]
\pause
and 
\[ \begin{array}{rl}
...
<<>>=
case0501$X <- case0501$Diet == "N/R50"
(m <- lm(Lifetime ~ X, data = case0501, subset = Diet %in% c("NP","N/R50")))
confint(m)
predict(m, data.frame(X=TRUE), interval = "confidence") # Expected lifetime on N/R50
@
