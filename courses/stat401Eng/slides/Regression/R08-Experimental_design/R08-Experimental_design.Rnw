...
The mean break pressure (pounds) was 270 (241,299) for Gorilla and 
309 (279, 338) for Titebond. 
Moving from Gorilla to Titebond glue caused an increase in break pressure
of 39 (-3,80).
Glue type accounted for 46\% of the variability in in break pressure.
...
Then a regression model for these data is 
\[
F_w \ind N(\beta_0+\beta_1T_w + \beta_2M_w + \beta_3T_wM_w, \sigma^2)
\]
\pause
where 
\begin{itemize}
\item $\beta_1$ is the expected difference in force when moving from Gorilla to
Titebond glue for Spruce, \pause 
\item $\beta_2$ is the expected difference in force when moving from Spruce  
