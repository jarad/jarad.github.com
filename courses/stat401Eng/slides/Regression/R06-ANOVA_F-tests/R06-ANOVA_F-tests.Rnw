...
SST = & (349-1)\times 8.97^2 = 28000 \pause \\
...
\end{array} \]
\pause
F statistic is off by 0.1 relative to the table later, because of rounding
of 8.97. 
...
\small
	...
	\[ \mbox{F} = \frac{\mbox{Extra sum of squares / Extra degrees of freedom}}{\hat{\sigma}^2} \] \pause 
	\item A pvalue is $P(F_{ndf,ddf} > \mbox{F})$
		\begin{itemize}
...
\frame{\frametitle{Lack-of-fit F-test}
  ...
  \vspace{0.1in} \pause
	...
	regression (reduced) model is not adequate.
