...
\begin{itemize}
\item reject the null hypothesis if $pvalue<a$ or 
\item fail to reject the null hypothesis if $pvalue \ge a$.
\end{itemize}
...
\begin{itemize}
\item the data are incompatible with the null hypothesis if $pvalue < a$ or 
\item there is insufficient evidence to indicate the data are incompatible with 
the null hypothesis if $pvalue \ge a$.
\end{itemize}
