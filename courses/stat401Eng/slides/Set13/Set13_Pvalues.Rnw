...
\begin{itemize}
\item the data are incompatible with the null if $pvalue < a$ or 
\item there is insufficient evidence to indicate the data are incompatible with the null if $pvalue <a$.
\end{itemize}
