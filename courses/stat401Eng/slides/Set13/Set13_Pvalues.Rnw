...
\[ 
\hat\theta(Y) \stackrel{\cdot}{\sim} N(E[\hat\theta], SE[\hat\theta])
\]
\pause
...
\pause
For example, if $Y\sim Bin(n,\theta)$ and you want to test $H_0: \theta \le 0.4$
vs $H_A: \theta > 0.4$ when observing $y=345$ successes out of $n=789$ attempts,
\pause the code is

<<echo=TRUE>>=
prop.test(345, 789, p = 0.4, alternative = "greater")
@
\end{frame}
