...
d <- bind_rows(
  ...
  data.frame(process = 2, sensitivity = rnorm(34,  9.3, 3.5)),
  data.frame(process = 3, sensitivity = rnorm( 7, 10.0, 5.0))) %>%
...
for $i=1,2,\ldots,J$ 
\pause
and you are interested in the relationship amongst the $\mu_j$. 
...
m <- lm(sensitivity ~ process, data = d)
anova(m)
@
...
# Estimate of the probability that one mean is larger than the rest
draws %>% 
