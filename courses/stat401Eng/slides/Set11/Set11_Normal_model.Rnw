...
n <- 9
y <- rnorm(n) # default is mean is 0 and sd is 1
curve(dnorm(x, mean = mean(y), sd = sqrt(1/n)), -2, 2, 
      xlab = expression(mu),
...
\[ 
\overline{y} \pm z_{.025} s/\sqrt{n}
\]
where $a = \int_{z_a}^{\infty} \frac{1}{\sqrt{2\pi}}e^{-x^2/2} dx$, 
i.e. the area to the right of $z_a$ under the pdf of a standard normal is $a$.
...
<<echo=TRUE, fig.height=4>>=
s <- sum((y-0)^2)
curve(MCMCpack::dinvgamma(x, shape = n/2, scale = s/2), 0, 4,
...
<<echo=TRUE>>=
draws <- MCMCpack::rinvgamma(1e5, shape = s/2, scale = n/2)
...
<<echo=TRUE>>=
draws <- rgamma(1e5, shape = s/2, rate = n/2)
...
\pause
Thus, we are typically interested in marginal posterior for $\mu$: 
\pause
\[
...
to construct the interval using 
\[ 
\overline{y} \pm t_{n-1,.025} S/\sqrt{n}
\]
where the area to the right of $t_{n-1,a}$ under the pdf of a 
standard $t$ is $a$.
...
% \pause
% We measure 100 randomly selected fields in Iowa and 
% find the average is 198 bushels per acre and the 
% sample standard deviation is 20 bushels per acre. 
% \pause
% Provide a 95\% credible interval for the 
% 
% 
% 
% \end{frame}
