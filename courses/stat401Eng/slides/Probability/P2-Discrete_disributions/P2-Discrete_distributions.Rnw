...
\title{Set04 - Discrete Distributions}
...
\pause
Intuitive idea:  
\textcolor{cyan}{If the value of a numerical variable depends on the outcome 
of an experiment, we call the variable a {\it random variable}.}

\vspace{0.1in} \pause

Examples of random variables from HootOwlHoot:
\begin{itemize}[<+->]
\item Win (1) or Loss (0)
\item Number of cards played
\item Number of suns played
\item Number of owls remaining
\item Owl score
\end{itemize}
...
then the probability mass function for $Y$ is 
\[ \arraycolsep=2pt\def\arraystretch{2.2} 
\begin{array}{c|cccccc}
\hline
y & 1 & 2 & 3 & 4 & 5 & 6 \pause \\
\hline
P(Y=y) & \frac{1}{6} & \frac{1}{6} & \frac{1}{6} & \frac{1}{6} & \frac{1}{6}  & \frac{1}{6} \pause \\ 
P(Y\le y) & \frac{1}{6} & \frac{2}{6} & \frac{3}{6} & \frac{4}{6} & \frac{5}{6}  & \frac{6}{6} \\
\hline
\end{array} \]
\end{example}
\end{frame}
...
if we place mass $P(X=x)$ at corresponding points $x$. 
...
\pause 
Thus, the binomial probability mass function is 
\[ 
