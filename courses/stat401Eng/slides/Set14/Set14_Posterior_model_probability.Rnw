...
= \left[ \sum_{k=1}^J \frac{p(y|H_k)}{p(y|H_j)}\frac{p(H_k)}{p(H_j)} \right]^{-1}
\]
\pause
where 
\[
...
Let $Y \sim N(\mu,1)$ and consider the hypotheses $H_0:\theta = 0$ and 
$H_A:\theta \ne 0$ with $\theta|H_A\sim N(0,C)$ and, for simplicity, 
$p(H_0) = p(H_A) = 0.5$.
\pause 
...
Thus 
\[
p(H_0|y) = \left[ \frac{p(y|H_0)}{p(y|H_A)}  \right]^{-1} \pause 
= \left[ \frac{N(y;0,1)}{N(y;0,1+C)}  \right]^{-1}
\]
\pause
...
\item But this requires you to at least two well-thought out models\pause, i.e. 
no vague priors.
\end{itemize}
