...
<<data_example, echo = FALSE, fig.height=4>>=
y <- rnorm(9, mean = 200, sd = 20) # Get some data
...
\pause
But because this \alert{confidence interval} happens to correspond to a 
...
<<data_example_print, echo=TRUE, eval=FALSE, dependson="data_example", size="normalsize">>=
<<data_example>>
@

<<data_example_result, echo=TRUE, dependson="data_example", size="normalsize">>=
c(L,U)
@
\end{frame}
...
proportion is, nor where we should believe it is. \pause
...
\[ \begin{array}{llll}
\mbox{Model} & \mbox{Estimator} & \mbox{Confidence Interval} & \mbox{Type} \\
\hline
Y_i \ind N(\mu,\sigma^2) & \hat\mu=\overline{y} & \hat\mu \pm t_{n-1,a/2}s/\sqrt{n} & \mbox{exact} \\
Y_i \ind N(\mu,\sigma^2) &  \hat\mu=\overline{y} & \hat\mu \pm z_{a/2}s/\sqrt{n} & \mbox{approximate} \\
Y_i \ind Ber(\theta) &  \hat\theta=\overline{y} & \hat{\theta} \pm z_{a/2} \sqrt{\hat\theta(1-\hat\theta)/n} & \mbox{approximate} \\
Y \sim Bin(n,\theta) &  \hat\theta=y/n & \hat{\theta} \pm z_{a/2} \sqrt{\hat\theta(1-\hat\theta)/n} & \mbox{approximate} \\
\hline
\end{array} \]
