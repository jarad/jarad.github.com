...
\[ Y_{i,j} \ind N(\mu_i,\sigma_i^2)\]
for $i=1,2,3$ and $j=1,\ldots,n_i$. \pause
...
Recall that
\[
\mu_i|y_i \ind t_{n_i-1}(\overline{y}_i, s_i^2/n_i)
\]
\pause
and that a draw for $\mu_i$ can be obtained by taking
\[
\overline{y}_i + T_{n_i-1} s_i/\sqrt{n_i}, \quad T_{n_i-1} \ind t_{n_i-1}(0,1).
\]
\pause
...
\item Frequentist: 
  ...
  \end{itemize}
\item Bayesian: based on posterior for $\mu_1,\ldots,\mu_I$
  \begin{itemize}
