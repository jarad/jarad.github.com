...
\item Frequentist: based on distribution of $Y_1,\ldots,Y_G$
  ...
  \item confidence interval for $\theta_k-\theta_{k'}$ for a specified $k$ and $k'$, \pause
  \end{itemize}
\item Bayesian: based on posterior for $\theta_1,\ldots,\theta_G$:
  \begin{itemize}
  \item credible interval for $\theta_k-\theta_{k'}$ for a specified $k$ and $k'$, \pause
  \item posterior model probability, e.g. $p(H_0|y)$, \pause and
  \item probability statements, e.g. $P(\theta_k<\theta_{k'}|y)$ for a specified $k$ and $k'$. 
  \end{itemize}
\end{itemize}
\end{frame}
...
\[ 
H_0: \theta_g = \theta \,\forall\, g
\quad
\mbox{versus}
\quad
H_A: \theta_k \ne \theta_{k'} \mbox{ for some }k,k'
\]
