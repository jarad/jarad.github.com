...
\begin{itemize}
\item {\bf Mean is unknown} while SD is known to be 20
\item Mean is known to be 200 while {\bf SD is unknown}
\item {\bf Both are mean and standard deviation are unknown}
\end{itemize}
...
@
\pause
These are both close to the exact values.
...
\begin{frame}
\frametitle{Normal model (unknown population mean and  population variance)}
