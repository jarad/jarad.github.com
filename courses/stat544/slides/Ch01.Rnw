...
\frame{\frametitle{Axioms of probability}
  \begin{definition}
	...
	\begin{enumerate}[1.]
	\item $P(A)\ge 0$ for any $A\in \Omega$ \pause
	\item $P(\Omega)=1$ \pause
	\item If $A_1, A_2,\ldots\in \Omega$ are pairwise disjoint, then $P(A_1\mbox{ or }A_2\mbox{ or }\ldots) = \sum_{i=1}^\infty P(A_i)$. 
	\end{enumerate}
	\end{definition}
}

\frame{\frametitle{Craps probabilities}
