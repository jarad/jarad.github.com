...
\newcommand{\myOmega}{\mathrm{\Omega}}
...
\begin{frame}[fragile]
<<bird_data, cache=TRUE, fig.width=18, fig.height=15>>=
...
\frametitle{Random intercept, random slope model} 
  ...
  \beta_s &\stackrel{ind}{\sim} N(\mu_\beta,\mySigma_\beta) 
  \end{array} \]
  where $\beta_s=(\beta_{s,0},\beta_{s,1})'$ \pause and $\sigma^2$, $\mu_\beta$, and $\mySigma_\beta$ are parameters to be estimated. 
...
% Equivalently,
% 
% \[ y_j \stackrel{ind}{\sim} N(X_j\beta_j, \sigma_y^2\I_{n_j}), \quad \beta_j \stackrel{ind}{\sim} N(\mu_\beta,\mySigma_\beta) \]
...
<<echo=TRUE>>=
m2 = lmer(y~I(year-2005) + (I(year-2005)|abbrev), d)
summary(m2)
@
\end{frame}
...
\[ p(\sigma, \mu_\beta, \mySigma_\beta) \propto p(\sigma)p(\mu_\beta)p(\mySigma_\beta) \]
\pause 
and 
\begin{itemize}
\item $\sigma \sim Ca^+(0,1)$,
\item $p(\mu_\beta) \propto 1$\pause, and
\item $\mySigma_\beta\sim$ ?
\end{itemize}
\end{frame}
...
Special cases:
\begin{itemize}
\item If $\nu=d+1$ and $S$ is diagonal, then each of the correlations in $\mySigma$ has a marginal uniform prior.  \pause
\item Jeffreys prior 
...
If $\mySigma \sim IW(\nu,S)$, then $\mySigma_{ii} \sim IG([\nu-(d-1)]/2, S_{ii}/2)$. \pause In particular, if $\nu=d+1$ and $S=\I$ (to ensure marginally uniform priors on the correlations), then $\mySigma_{ii} \sim IG(1,1/2)$. 

\vspace{0.2in} \pause

The problems
\begin{itemize}[<+->]
\item although the correlations are marginally uniform, they are not independent \emph{a priori} of the variances (diagonal elements of $\mySigma$),
...
Let 
\[ \mySigma = \mbox{diag}(\sigma)\myOmega \mbox{diag}(\sigma) \]
where 
\begin{itemize}
\item $\sigma$ is a vector of standard deviations
\item $\myOmega$ is a correlation matrix
\end{itemize}
\pause
...
Now we can put whatever prior we want on $\sigma$ and $\myOmega$, e.g. $\sigma_i \stackrel{ind}{\sim} Ca^+(0,?)$. 
\end{frame}
...
\[ p(\myOmega) = |\myOmega|^{\eta-1} \]
where $\myOmega$ is a correlation matrix with implicit dimension $d$ and $\eta>0$ is the shape parameter.
...
\begin{frame}[fragile]
<<stan_model, cache=TRUE, echo=TRUE>>=
model = "
data {
...
\begin{frame}[fragile]
<<stan, cache=TRUE, dependson=c('bird_data','stan_model')>>=
...
The model
  ...
  \beta_s &\stackrel{ind}{\sim} N(\mu_\beta,\mySigma_\beta) \pause \\
  \sigma_s &\stackrel{ind}{\sim} LN(\mu_\sigma,\tau_\sigma)
  \end{array} \]
\pause 
and a prior

\[ p(\mu_\sigma, \tau_\sigma, \mu_\beta, \mySigma_\beta) \propto p(\mu_\sigma)p(\tau_\sigma)p(\mu_\beta)p(\mySigma_\beta) \]
\pause 
and 
\begin{itemize}
