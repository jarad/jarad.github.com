...
joint = data.frame(abbrev    = rownames(ranef(m)$abbrev),
                   joint = c(fixef(m)[1] + ranef(m)$abbrev[,1],
                             fixef(m)[2] + ranef(m)$abbrev[,2]),
                   parameter = rep(c("intercept","slope"), 
                                   each=nrow(ranef(m)$abbrev)))
...
\begin{frame}[fragile]
<<stan2, cache=TRUE, dependson=c('bird_data','stan_model2')>>=
