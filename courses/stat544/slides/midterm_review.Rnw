...
\frame{\frametitle{Introduction to multiparameter models}
\small
...
\frame{\frametitle{Normal model}
  ...
  \pause where $\sigma_n^2 = \left[\mu_0\sigma_0^2+(n-1)s^2+\frac{\kappa_0n}{\kappa_0+n}(\overline{y}-\mu_0)^2 \right]/(\nu_0+n)$. \pause 
