...
= \frac{q(\theta^*|y)/g(\theta^*|\theta^t)}{q(\theta^t|y)/g(\theta^t|\theta^*)} \pause 
...
\begin{frame}[fragile]
\frametitle{Example: Normal-Cauchy model (bad starting value)}
...
qplot(1:n, theta) +
  labs(x="Time", y=expression(theta), title="Independence Metropolis-Hastings")
@
\end{frame}
...
\begin{itemize}
\item in a region where $q(\theta^t|y)>>g(\theta^t)$ \pause then
\item any proposal $\theta^*$ such that $p(\theta^*|y)\approx g(\theta^*)$ \pause 
\end{itemize}
will result in 
\[ r = \frac{g(\theta^t)}{q(\theta^t|y)}\frac{q(\theta^*|y)}{g(\theta^*)} \pause \approx 0 \]
...
qplot(1:n, samps)+labs(x="Time", y=expression(theta), title="Random-walk Metropolis")
@
\end{frame}

\begin{frame}[fragile]
\frametitle{Example: Normal-Cauchy model with poor initial value}
...
random_walk = function(n, tau, theta0, target) {
  ...
  for (i in 2:n) {
    ...
    logr = log(target(proposed,y))-log(target(theta[i-1],y))
    theta[i] = ifelse(log(runif(1))<logr, proposed, theta[i-1])
  }
...
Recall that the optimal random-walk tuning parameter (if the posterior is normal) is $2.4^2 Var(\theta|y)/d$ \pause where $Var(\theta|y)$ is the unknown posterior covariance matrix. \pause We can estimate $Var(\theta|y)$ using the sample covariance matrix of draws from the posterior. 
...
\item Estimate $\mathrm\Sigma_{b+1}$ using the sample covariance matrix of all previous draws. 
\item If $b<B$, set $b=b+1$ and return to step 2. 
\item Run $K$ iterations of the MCMC using $2.4^2\mathrm\Sigma_B/d$ and throw away all previous draws.
\end{enumerate}
...
\[ p(\alpha,\beta|y) \propto \left[ \prod_{i=1}^n \frac{B(\alpha+y_i, \beta+n_i-y_i)}{B(\alpha,\beta)} \right] (\alpha+\beta)^{-5/2} \]
...
<<dependson='beta_binomial',message=FALSE, warning=FALSE>>=
ggpairs(samps, 
...
\item There are two main Metropolis-Hastings proposals
\begin{itemize}
