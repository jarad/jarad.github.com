...
\frame{\frametitle{Numerical integration}
  ...
  \[ E[h(\theta)|y] = \int h(\theta)p(\theta|y) d\theta \approx \sum_{S=1}^S w_s h\left(\theta^s\right)p(\theta^s|y) \]
  where 
    \begin{itemize}
...
p = q(x,0)*sum(q(x,0))
mean(p*x)
mean(x[sample(length(x),prob=p,replace=TRUE)])
...
sum(p*x) 
# But small MC sample
mean(x[sample(length(x),101,prob=p,replace=TRUE)])
@
\end{frame}
...
\frame{\frametitle{Rejection sampling}
  Suppose you wish to obtain samples $\theta\sim p(\theta|y)$, \pause rejection sampling performs the following \pause 
	...
	\item $p(\theta|y)$ only needs to be known up to a normalizing constant (although then $M$ won't be the acceptance probability).
	\end{itemize}
...
\frametitle{Example: Normal-Cauchy model}
  ...
  Choose a standard normal as a proposal distribution, i.e. 
...
\frametitle{Example: Normal-Cauchy model}
  ...
  \[ g(\theta)=\frac{1}{\sqrt{2\pi}}  e^{-(\theta-y)^2/2} \]
  \pause with 
  \[ w(\theta) = \frac{p(\theta|y)}{g(\theta)} = \frac{1}{\frac{1}{\pi}(1+\theta^2)} \]
...
qplot(x=theta,y=weight)+geom_point()+labs(title="Importance weight")
@
\end{frame}
...
library(weights)
mean(weight*theta/sum(weight)) # Estimate mean
wtd.hist(theta, 100, prob=TRUE, weight=weight)
curve(q(x,0)/py(0), add=TRUE, col="red", lwd=2)
@
\end{frame}
...
<<resampling, eval=FALSE, warning=FALSE, echo=TRUE>>=
# resampling
new_theta = theta[sample(length(theta), replace=TRUE, prob=weight)] # internally normalized
hist(new_theta, 100, prob=TRUE, main="Unweighted histogram of resampled draws")
curve(q(x,0)/py(0), add=TRUE, col="red", lwd=2)
@
...
qplot(theta,weight,geom="point")+ylim(0,max(weight))+xlim(-theta[whmax],theta[whmax])
@
\end{frame}
