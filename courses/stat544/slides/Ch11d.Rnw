...
\item For $t=1,\ldots,T$, perform the kernel $K(\theta,\theta')$ to obtain a sequence $\theta^1,\ldots,\theta^T$. 
\end{enumerate}
...
\small
  ...
  \[ E_{p(\theta|y)}[h(\theta)|y] = \int_{\Theta} h(\theta) p(\theta|y) d\theta \]
...
mean(r2) # should be 0
var(r2)  # should 1
quantile(r2,c(.025,.975)) # should be -1.96,1.96
@
\end{frame}
...
\frametitle{Thinning}
<<thinning>>=
samps = data.frame(iteration=1:length(r1), 
                   rho = rep(c(.99,.9,0), each=length(r1)),
                   x=c(r1,r2,r3))
sq = seq(10,1000,by=10)
ddply(samps, .(rho), summarize, 
      full=effectiveSize(x), thinned=effectiveSize(x[sq]))
@
\end{frame}
...
For MCMC that have tuning parameters, use burn-in (warm-up) to tune tuning parameters. 
...
\begin{frame}[fragile]
\frametitle{Alternative use for burn-in}
<<tune_plot, fig.width=8>>=
hist(inference$theta, prob=T)
...
\item Run multiple chains according to suggestions in BDA
  ...
  \item Run until effective sample size is around 1000 \pause 
  \end{enumerate}
