...
An ergodic MCMC algorithm with transition kernel $K(\theta^{t-1},\theta^{t})$ constructed to sample from $p(\theta|y)$ is the following: \pause
\begin{enumerate}[<+->]
\item Set $\theta^0$.
...
\begin{itemize}
\item What should I use for $\theta^0$?
...
BDA3 suggests 10-100 per chain for a total of 100-2000 effective samples.
...
\small
  ...
  where $\rho_k$ is the $k^{th}$ autocorrelation of the $h(\theta|y)$ values.
	\end{itemize}
}
...
# Quantiles (should be 0.025 and 0.975)
ddply(d, .(rho), function(x) as.data.frame(mcmcse::mcse(x$x< -1.96)))
ddply(d, .(rho), function(x) as.data.frame(mcmcse::mcse(x$x<  1.96)))
@
\end{frame}
