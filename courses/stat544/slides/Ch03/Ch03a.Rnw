...
\frametitle{Marginal posterior for $\mu$}
  ...
  where $c = t_{n-1,1-\alpha/2}$ is the t-critical value from before. \pause This is exactly the same as the $100(1-\alpha/2)$\% confidence inteval.
\end{frame}
...
\frametitle{Predictive distribution}
  ...
  \pause
  with $\sigma^2|y \sim \mbox{Inv-}\chi^2(n-1,S^2)$. \pause Now, we can use the Normal-Inv-$\chi^2$ theory, to find that 
...
\frametitle{Marginal posterior for $\mu$}
  ...
  An equal-tailed $100(1-\alpha)$\% credible inteval can be obtained via 
  \[ m' \pm c s'/\sqrt{k'} \]
  where $c = t_{v',1-\alpha/2}$ is the t-critical value from before. 
\end{frame}
...
calculate $m', k', v',$ and $s'$ and then 
\begin{enumerate}[<+->]
\item simulate $\sigma^2 \sim \mbox{Inv-}\chi^2(v',(s')^2)$ and
