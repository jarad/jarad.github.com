...
&= \exp\left(-\frac{1}{2} tr(\mS^{-1} S_0) \right)
\end{array} \]
where 
...
Let $Y_i\sim N(\mu,S)$ with conjugate prior $\mu \sim N_K(m,C)$
\[ \begin{array}{rl}
p(\mu|y) &\propto p(y|\mu)p(\mu) \pause \\
&\propto \exp\left(-\frac{1}{2}\sum_{i=1}^n (y_i-\mu)^\top S^{-1} (y_i-\mu) \right)\exp\left(-\frac{1}{2}\mu-m)^\top C^{-1} (\mu-m) \right) \pause \\
...
Let the $K\times K$ matrix $\mS$ have an inverse Wishart distribution, i.e. $\mS \sim IW(v,W^{-1})$, with scalar $v>0$ and positive definite scale matrix $W$. 

\vspace{0.2in} \pause

The pdf for $\mS$ is 
\[ 
p(\mS) \propto |W|^{v-K-1}/2\exp\left( -\frac{1}{2} tr(W\mS^{-1} \right).
\]
\end{frame}
...
Let $Y_i\stackrel{ind}{\sim} N(\mu,\mS)$ with conjugate prior 
\[ 
