...
\item $CoVar[Y_k,Y_{k'}] = -n\pi_k\pi_{k'}$ for $k\ne k'$
\end{itemize}
...
\item $CoVar[\pi_k,\pi_{k'}] = \frac{-a_k a_{k'}}{a_0^2(a_0+1)}$
\end{itemize}
...
Let $Y=(Y_1,\ldots,Y_K)\in \mathbb{R}^K$ have a multivariate normal distribution, i.e. $Y\sim N_K(\mu,\mS)$ with mean $\mu$ and positive definite variance-covariance matrix $\mS$.
...
\item $E[Y_k] = \mu_k$
\item $Var[Y_k] = \mS_{kk}$
\item $CoVar[Y_k,Y_{k'}] = \mS_{k,k'}$ \pause
...
Let the $K\times K$ matrix $\mS$ have an inverse Wishart distribution, 
i.e. $\mS \sim IW(v,W^{-1})$, 
...
p(\mS) \propto 
|\mS|^{-(v+K+1)/2}
\exp\left( -\frac{1}{2} tr\left(W\mS^{-1}\right) \right).
\]
...
\item Use the scaled Inverse wishart distribution (see BDA3 pg 74)
\item Use the separation strategy, i.e. $\mS = \Delta \Lambda \Delta$ where $\Delta$ 
