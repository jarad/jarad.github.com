...
\begin{frame}[fragile]
\frametitle{Ordinal hierarchical model in Stan}

<<ordinal_model, echo=TRUE, cache=TRUE, eval=FALSE>>=
ordinal_model = "
data {
  int <lower=1> n_products;
  int <lower=0> y[n_products,5];
}

parameters {
  real<lower=0,upper=1> alpha_diff[3];
  real mu[n_products];
  real eta;
  real<lower=0> tau;
}
...
transformed parameters {
  ordered[4] alpha;
  simplex[5] theta[n_products];

  alpha[1] <- 0;
  for (i in 1:3) alpha[i+1] <- alpha[i] + alpha_diff[i];
...
<<run_ordinal_model, echo=TRUE, cache=TRUE, dependson='ordinal_model', eval=FALSE>>=
...
<<echo=TRUE, cache=TRUE, dependson='run_ordinal_model', eval=FALSE>>=
r
@
\end{frame}
...
ggplot(mu_m, aes(x=value, fill=product)) +  geom_density(alpha=0.5)
@
...
alpha = c(0,.36,.6,1.11)
mu = c(1.49,-.38)
curve(dnorm(x,mu[1],1), -3, 5, lwd=2, xlab="", axes=F, frame=T, ylab="")
curve(dnorm(x,mu[2],1), lwd=2, add=TRUE)
...
abline(v=mu, lty=2, col="blue")
axis(3, at=mu, labels=expression(mu[2],mu[10]), col="blue")
text((-1:3)+.5, 0, 1:5, col="seagreen")
@
\end{frame}
