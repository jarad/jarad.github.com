...
\small
  ...
  \[ E[h(\theta)] = \int_{\Theta} h(\theta) p(\theta) d\theta \]
	...
	where
	\[ v_J = \frac{1}{J} \widehat{V[h(\theta)]} \pause \approx \frac{1}{J^2} \sum_{s=1}^J \left[ h\left(\theta^{(j)}\right) - \hat{h}_J \right]^2. \]
	\end{itemize}
\end{frame}
