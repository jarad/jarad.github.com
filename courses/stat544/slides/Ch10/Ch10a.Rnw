...
(py = sum(q(theta,y)*w))                 # gridding based approach
...
\small
  ...
  \[ E[h(\theta)] = \int_{\Theta} h(\theta) p(\theta) d\theta \]
	...
	\[ \hat{h}_J \stackrel{d}{\to} N(E[h(\theta)], v_J) \pause \]
	where
	\[ v_J = \frac{1}{J} Var[h(\theta)] \pause \approx \frac{1}{J^2} \sum_{s=1}^J \left[ h\left(\theta^{(j)}\right) - \hat{h}_J \right]^2. \]
	\end{itemize}
\end{frame}
...
cumvar = function(theta) {
  J = length(theta)
  cumvar = numeric(J)
  for (j in 1:J) { cumvar[j] = var(theta[1:j])/j }
  return(cumvar)
}
