...
\frame{\frametitle{Numerical integration}
  ...
  and
    \begin{itemize}
    \item $\theta^{(s)} \stackrel{iid}{\sim} g(\theta)$ (for some proposal distribution $g$),
...
\frametitle{Example: Normal-Cauchy model}
  ...
  \[ q(y) = \int q(\theta|y) d\theta \]
  \item or in calculating the posterior mean, i.e. 
  \[ E[\theta|y] = \int \theta p(\theta|y) d\theta = \int \theta \frac{q(\theta|y)}{q(y)} d\theta. \]
  \end{enumerate}
\end{frame}
...
(qy = sum(q(theta,y)*w))                 # gridding based approach
...
\[ E[h(\theta)|y) \approx \sum_{s=1}^S w_s h\left(\theta^{(s)}\right)p\left(\left.\theta^{(s)}\right|y\right) = \sum_{s=1}^S w_s h\left(\theta^{(s)}\right) \frac{q\left(\left.\theta^{(s)}\right|y\right)}{q(y)} \]
<<normal_cauchy_posterior_expectation, dependson=c('data','normal_cauchy_posterior'), echo=TRUE>>=
h = function(theta) theta

sum(w*h(theta)*q(theta,y)/qy)
@
...
or a sequence or random variables $X_1,X_2,\ldots$ and a random variable $X$:
\begin{itemize}[<+->]
...
\pause where $\theta^{(j)} \stackrel{ind}{\sim} g(\theta)$.  \pause We know
\begin{itemize}
\item SLLN: $\hat{h}_J$ converges almost surely to $E[h(\theta)]$. \pause
\item CLT: if $h^2$ has finite expectation, \pause then
\[ \frac{\hat{h}_J-E[h(\theta)]}{\sqrt{v_J/J}} \stackrel{d}{\to} N(0,1) 
\pause \quad \mbox{where} \quad
v_J = Var[h(\theta)] \pause \approx 
\frac{1}{J} \sum_{j=1}^J \left[ h\left(\theta^{(j)}\right) - \hat{h}_J \right]^2 \]
\pause
...
\frame{\frametitle{Definite integral}
  ...
  \item For $j=1,\ldots,J$,
    \begin{enumerate}
    \item sample $\theta^{(j)} \sim Unif(0,1)$ and
    \item calculate $h\left(\theta^{(j)}\right)$.
    \end{enumerate}
  \item Calculate
  \[ \mathrm{I} \approx \frac{1}{J} \sum_{j=1}^J h(\theta^{(j)}). \]
  \end{enumerate}
}
...
cumvar = function(theta) {
  J = length(theta)
  cumvar = numeric(J)
  for (j in 1:J) { cumvar[j] = var(f(theta[1:j]))/j }
  return(cumvar)
}
...
\frame{\frametitle{Infinite bounds}
  ...
  \item For $j=1,\ldots,J$,
    \begin{enumerate}
    \item sample $\theta^{(j)} \sim N(0,1)$ and
    \item calculate $h\left(\theta^{(j)}\right)$.
    \end{enumerate}
  \item Calculate
...
\item Sample a proposal $\theta^*\sim g(\theta)$ \pause and $U\sim Unif(0,1)$.  
\item Accept $\theta=\theta^*$ as a draw from $p(\theta|y)$ if 
$U\le q(\theta^*|y)/M^*g(\theta^*)$, otherwise return to step 1. 
\end{enumerate}
\pause
...
\begin{itemize}[<+->]
\item For a given proposal distribution $g(\theta)$, 
the optimal $M^*$ is $M^*=\sup_\theta q(\theta|y)/g(\theta)$. 
...
\begin{frame}[fragile]
\frametitle{Example: Normal-Cauchy model}
<<rejection_sampling_plot, dependson='data', echo=FALSE, fig.width=8>>=
M = sqrt(2*pi)
...
gg + stat_function(fun=function(x) M*g(x,0), col=clrs[2]) +
  stat_function(fun=function(x) q(x,0), col=clrs[1]) + 
