...
ggplot(d, aes(x, posterior, group = 1)) +
  geom_line() +
  geom_vline(data = melt(estimates,variable.name='estimator', 
                         measure.vars = c('mean','median','mode')), 
...
ggplot(d, aes(x, posterior, group=1)) +
  ...
  geom_segment(data = interval, aes(x = lb, xend = ub, y=y, yend=y, 
...
<<simulation, dependson='data'>>=
sim = data.frame(x = rbeta(10000, a+y, b+n-y))
ggplot(sim, aes(x=x)) + 
  geom_histogram(aes(y=..density..), binwidth=.01) + 
  stat_function(color = 'red', fun=dbeta, args=list(shape1=a+y, shape2=b+n-y))
@
...
c(mean = mean(sim$x), median = median(sim$x))
quantile(sim$x, c(.025,.975)) # Equal-tail
quantile(sim$x, .05) # Upper
quantile(sim$x, .95) # Lower
@
...
ggplot(melt(dd, 
            ...
            value.name = 'density'),
       aes(theta, density, color = distribution, linetype = distribution)) +
  geom_point() +
  theme(legend.position="bottom") 
@
\end{frame}
...
<<integrate, dependson='data', echo=TRUE>>=
f = function(theta) {
...
<<nonconjugate_plot_unnormalized, dependson='integrate'>>=
