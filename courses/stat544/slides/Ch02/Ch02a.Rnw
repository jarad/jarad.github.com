...
\frame{\frametitle{Point estimation}
	...
	\item Mode: $\hat{\theta}_{Bayes} = \mbox{argmax}_\theta \, p(\theta|y)$ minimizes $L\!\left(\theta,\hat{\theta}\right) = -\I\left(\theta = \hat{\theta}\right)$  \pause also called \alert{maximum a posterior (MAP)}
	\end{itemize}
}
