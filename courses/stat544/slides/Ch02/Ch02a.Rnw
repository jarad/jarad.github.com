...
When constructing the $Be(a,b)$ prior with the binomial likelihood, we 
can interpret the parameters in the following way:
\pause
\begin{itemize}[<+->]
...
The posterior mean is 
\[ 
E[\theta|y] = \frac{2}{12} \frac{1}{2} + \frac{10}{12} \frac{3}{10} = \frac{4}{12}.
\]
...
<<simulation, dependson='data', fig.width=7>>=
...
ggplot(dd %>% gather(Distribution, density, -theta),
