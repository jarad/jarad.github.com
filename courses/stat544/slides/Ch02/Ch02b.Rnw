...
\frame{\frametitle{Natural conjugate prior} \small
  ...
  \[ C' = \left[\frac{1}{C} + \frac{n}{s^{2}}\right]^{-1} \qquad m' = C'\left[\frac{m}{C} + \frac{n}{s^{2}}\overline{y}\right]. \]
}
...
<<jags_binomial_plot, dependson='jags_binomial', echo=TRUE>>=
plot(r)
@
\end{frame}
...
<<jags_normal_plot, dependson='jags_normal', echo=TRUE>>=
plot(r)
@
\end{frame}
...
<<jags_normal_var_plot, dependson='jags_normal_var', echo=TRUE>>=
plot(r)
@
\end{frame}
...
<<stan_binomial_sampling, dependson='stan_binomial', echo=TRUE, message=FALSE>>=
...
<<stan_binomial_results, dependson='stan_binomial_sampling', echo=TRUE, message=FALSE>>=
r
@
\end{frame}
...
<<stan_normal_mean_sampling, dependson='stan_normal_mean', echo=TRUE, message=FALSE>>=
r = sampling(m, data = dat)
@
\end{frame}
...
<<stan_normal_var_sampling, dependson='stan_normal_var', echo=TRUE, message=FALSE>>=
r = sampling(m, data = dat)
@
\end{frame}
