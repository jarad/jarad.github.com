...
\[ \beta|\sigma^2 \sim N(b_0,\sigma^2 B_0) \]
...
= \frac{\mathrm{\Gamma}\left(\frac{n-1}{2}\right)}{\pi^{(n+1)/2}n^{1/2}}||y-\overline{y}||^{-(n-1)} \frac{\left(1+g\right)^{(n-1-p)/2}}{\left(1+g(1+R^2))^{(n-1)/2} \right)}
 \]
...
\frame{\frametitle{Housing prices in Ames}
