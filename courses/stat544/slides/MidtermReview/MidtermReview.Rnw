...
\frame{\frametitle{Single-parameter models (Ch 2)}
  ...
  \item Point estimation  depends on utility function
    ...
    \item Mode minimizes 0-1 error
    \end{itemize}
  \item Computation, $E[h(\theta)|y]$ 
    \begin{itemize}
    \item SLLN
    \item CLT
    \end{itemize}
...
\frame{\frametitle{Normal model}
  ...
  \pause where $s^2 = \frac{1}{n} \sum_{i=1}^n (y_i-\overline{y})^2$. \pause
...
In order to make a decision, a utility (or loss) function, i.e. $U(\theta,\delta)=-L(\theta,\delta)$, must be set. \pause 
Then the optimal Bayesian decision is to maximize expected utility (or minimize expected loss), i.e. 
\[
\mbox{argmax}_\delta \int U(\theta,\delta) p(\theta) d\theta
\]
\pause
where $p(\theta)$ represents your current state of belief, \pause i.e. it could be a prior or a posterior depending on your perspective. 
