...
# Find reasonable starting values by matching moments
theta = (dat$y+0.5)/(dat$n+1)
mean(theta) # ~= 0.5, so alpha~=beta

# Force alpha=beta, and then use moment matching with the variance
# which is vr = 1/4(2*alpha+1) implying 
# alpha = (4/v-1)/2
vr = var(theta) 
inits = list(alpha=(4/vr-1)/2, beta=(4/vr-1)/2)
...
since the $\theta_i$ are conditionally independent. \pause Thus, the we actually ran the following block Gibbs sampler:
\begin{enumerate}[<+->]
