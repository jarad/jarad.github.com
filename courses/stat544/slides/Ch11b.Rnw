...
\frame{\frametitle{Gibbs sampling}
  Suppose our target distribution is $p(\theta|y)$ with $\theta=(\theta_1,\theta_2)$  \pause and we can sample from $p\left(\theta_1|\theta_2,y\right)$ and $p\left(\theta_2|\theta_1, y\right)$\pause. Beginning with an initial value $\left(\theta_1^0, \theta_2^0\right)$\pause, the \alert{Gibbs sampler} is 
	\begin{enumerate}[\quad1.]
	\item sample $\theta_1^{t}\sim p\left(\theta_1|\theta_2^{t-1},y\right)$ \pause and then 
	\item sample $\theta_2^{t}\sim p\left(\theta_2|\theta_1^{t},y\right)$. 
	\end{enumerate}
...
\begin{frame}[fragile]
<<bivariate_normal_animation, fig.show='animate', echo=FALSE, cache=TRUE>>=
bivariate_normal_animation(x, rho)
@
\end{frame}
...
\end{array} \]
and thus $\sigma^2|\mu,y \sim \mbox{Inv-}(v', (s')^2)$ where
\[ \begin{array}{rl}
v' & = v+n \\
...
for (i in 1:n_iter) {
  # Sample mu
  Cp = 1/(n/sigma^2+1/C)
  mp = Cp*(sum_y/sigma^2+m/C)
  mu = rnorm(1, mp, Cp)
  
  # Sample sigma
  vpsp2 = vs2 + sum((y-mu)^2)
  sigma = 1/sqrt(rgamma(1, vp/2, vpsp2/2))
...
\frame{\frametitle{$K$-component Gibbs sampler}
	...
	\[ \begin{array}{rl}
	\theta_1^{t} &\sim p\left(\theta_1|\theta_2^{t-1},\ldots, \theta_K^{t-1},y\right) \pause \\ \\
	\theta_2^{t} &\sim p\left(\theta_2|\theta_1^{t}, \theta_3^{t-1},\ldots, \theta_K^{t-1},y\right) \pause\\
	&\vdots \\
	\theta_k^{t} &\sim p\left(\theta_k|\theta_1^{t}\ldots,\theta_{k-1}^{t}, \theta_{k+1}^{t-1},\ldots, \theta_K^{t-1},y\right) \\
	&\vdots \pause\\
	\theta_K^{t} &\sim p\left(\theta_K|\theta_1^{t},\ldots, \theta_{K-1}^{t},y\right) \pause
	\end{array} \]
	The distributions above are called the \alert{full conditional distributions}. \pause If some of the $\theta_k$ are vectors, then this is called a \alert{block} Gibbs sampler.
}
...
\[ \begin{array}{rl}
p(\mu|\eta,\sigma^2,\tau^2,y) &= \prod_{i=1}^n p(\mu_i|\sigma^2,\tau^2,y_i) \\
p(\mu_i|\eta,\sigma^2,\tau^2,y_i) &= N\left(\left[ \frac{1}{\sigma^2/n_i} + \frac{1}{\tau^2} \right] \left[ \frac{\overline{y}_i}{\sigma^2/n_i} + \frac{\eta}{\tau^2} \right] , \left[ \frac{1}{\sigma^2/n_i} + \frac{1}{\tau^2} \right]^{-1} \right) \\
p(\eta|\mu,\sigma^2,\tau^2,y) &= N\left( \overline{\mu}, \tau^2/\mathrm{I}\right) \\
p(\sigma^2|\mu,\eta,\tau^2,y) &= IG(a_\sigma+n/2, b_\sigma + \sum_{i=1}^\mathrm{I} \sum_{j=1}^{n_j} (y_{ij}-\mu_i)^2/2) \\
p(\tau^2|\mu,\eta,\sigma^2,y) &= IG(a_\tau+\mathrm{I}/2, b_\tau + \sum_{i=1}^\mathrm{I} (\mu_i-\eta)^2/2) 
\end{array} \]
...
\frame{\frametitle{Metropolis-within-Gibbs}
  We have discussed two Markov chain approaches to sampling from target distributions:
	...
	Gibbs sampling assumed we can sample from $p(\theta_k|\theta_{-k})$ for all $k$\pause, but what if we cannot sample from all of these full conditional distributions? \pause For those $p(\theta_k|\theta_{-k})$ that cannot be sampled directly, a Metropolis-Hastings step can be substituted. 
}
...
gibbs_and_metropolis = function(x0, n_points, rho) {
  ...
  for (i in 2:n_points) {
    ...
    logr   = dnorm(x_prop,   rho*x[i,1], v, log=TRUE) - 
             dnorm(x[i-1,2], rho*x[i,1], v, log=TRUE)
...
\begin{frame}[fragile]
<<gibbs_and_metropolis_animation, fig.show='animate', echo=FALSE, warning=FALSE>>=
bivariate_normal_animation(x, rho)
@
\end{frame}
...
\pause
for $i=1,\ldots,\mathrm{I}$, $j=1,\ldots,n_i$, $n = \sum_{i=1}^\mathrm{I} n_i$ and \pause prior 
\[ p(\eta,\tau^2,\sigma) \propto Ca^+(\tau^2; 0, b_\tau) Ca^+(\sigma^2; 0, b_\sigma). \]
...
p(\mu_i|\eta,\sigma^2,\tau^2,y_i) &= N\left(\left[ \frac{1}{\sigma^2/n_i} + \frac{1}{\tau^2} \right] \left[ \frac{\overline{y}_i}{\sigma^2/n_i} + \frac{\eta}{\tau^2} \right] , \left[ \frac{1}{\sigma^2/n_i} + \frac{1}{\tau^2} \right]^{-1} \right) \\
p(\eta|\mu,\sigma^2,\tau^2,y) &= N\left( \overline{\mu}, \tau^2/\mathrm{I}\right) \\
p(\sigma^2|\mu,\eta,\tau^2,y) &\propto IG(n/2, \sum_{i=1}^\mathrm{I} \sum_{j=1}^{n_j} (y_{ij}-\mu_i)^2/2)Ca^+(0,b_\sigma) \\
p(\tau^2|\mu,\eta,\sigma^2,y) &\propto IG(\mathrm{I}/2, \sum_{i=1}^\mathrm{I} (\mu_i-\eta)^2/2) Ca^+(0,b_\tau)
\end{array} \]
where $n_i\overline{y}_i = \sum_{j=1}^{n_i} y_{ij}$ and $\mathrm{I}\,\overline{\mu} = \sum_{i=1}^\mathrm{I} \mu_i$. 
...
To sample from $p(\theta|y) \propto IG(a,b) Ca^+(0,b_\theta)$, we have a variety of possibilities. \pause Here are three:
\begin{enumerate}[<+->]
\item Rejection sampling with $\theta^*\sim IG(a,b)$ and thus $M^*_{opt} = Ca^+(0;0,b_\theta)$ and the acceptance probability is $Ca^+(\theta^*;0,b_\theta) / M^*_{opt}$.
\item Independence Metropolis-Hastings with $\theta^*\sim IG(a,b)$ and thus the acceptance probability is $Ca^+(\theta^*;0,b_\theta)/Ca^+(\theta^t;0,b_\theta)$.
\item Random-walk Metroplis-Hastings with $\theta^* \sim g(\cdot|\theta^t)$ and acceptance probability is $q(\theta^*|y)/q(\theta^t|y)$. 
\end{enumerate}
...
We will use a Gibbs sampler to sample from $\theta_1,\ldots,\theta_n,\alpha,\beta$\pause, so we need to derive the following conditional distributions:\pause
\begin{itemize}
...
\end{itemize}
For shorthand, we often use $\theta_i|\ldots$ where $\ldots$ indicates \emph{everything else}.
\end{frame}
...
<<mcmc_summary, fig.width=10>>=
r_subset = r[r$parameter %in% c("alpha","beta"),]
ddply(r_subset, .(parameter), summarize, acceptance_rate = length(unique(value))/max(iteration))
ggplot(r_subset, aes(x=iteration, y=value))+geom_line()+facet_wrap(~parameter)
@
\end{frame}
...
\begin{enumerate}
\item $u^t|\theta^{t-1} \sim \mbox{Unif}\{u:0<u<p(\theta^{t-1}|y)\}$ \pause and
\item $\theta^t|u^t \sim \mbox{Unif}\{\theta: u^t<p(\theta|y) \}$.
\end{enumerate}
\end{frame}
...
\begin{itemize}
\item Gibbs sampling breaks down a hard problem of sampling from a high dimensional distribution to a set of easier problems sampling from low dimensional full conditional distributions.
