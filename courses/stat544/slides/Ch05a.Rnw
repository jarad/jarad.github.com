...
\frame{\frametitle{Posterior distribution for hierarchical models}
  ...
  &= \int \cdots \int  \prod_{i=1}^n  p(y_i|\theta_i)p(\theta_i|\phi) d\theta_1 \cdots d\theta_n \pause \\
...
p(y|\alpha,\beta) &= \prod_{i=1}^n p(y_i|\alpha,\beta) \pause \\
&= \prod_{i=1}^n \int p(y_i|\theta_i)p(\theta_i|\alpha,\beta) d\theta_i \pause \\
...
\end{array} \]
\pause
Thus $y_i|\alpha,\beta \sim Beta-binomial(n_i,\alpha,\beta)$. 
\end{frame}
...
r = sampling(m, dat, c("mu","eta","theta"))
@
\end{frame}
...
\begin{frame}[fragile]
\frametitle{Stan - improper prior}
<<stan2_run, echo=FALSE, cache=TRUE>>=
<<stan2>>
@
\end{frame}
...
e = 0.2
ggplot(rbind(new_d,new_d2), aes(x=lcl, 
                     xend=ucl, 
                     y=game+e*(prior=="improper"), 
                     yend=game+e*(prior=="improper"), 
                     color=prior))+
  geom_segment(lwd=2, alpha=0.5) + 
  labs(x=expression(theta), y="game")
@
