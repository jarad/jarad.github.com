...
For example, suppose $Y_i\stackrel{ind}{\sim} Ber(\theta)$ \pause and $P(\theta=\theta_j) = 1/11$
where $\theta_j=j/10$ 
\pause 
for $j=0,\ldots,10$. 
\pause 
The posterior is 
\[ P(\theta=\theta_j|y) 
\propto \frac{1}{11} \prod_{i=1}^n (\theta_j)^{y_i}(1-\theta_j)^{1-y_i} \pause 
= \frac{1}{11} (\theta_j)^{n\overline{y}}(1-\theta_j)^{n(1-\overline{y})} 
\]
\pause
