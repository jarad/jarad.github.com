...
\[ P(\theta=d_j|y) = \frac{p_j p(y|d_j)}{\sum_{k=1}^J p_k p(y|\theta_k)} \propto p_j p(y|d_j). \]

\vspace{0.2in} \pause

For example, suppose $Y_i\stackrel{ind}{\sim} Ber(\theta)$ \pause and $P(\theta=d_j) = 1/11$
where $d_j=j/10$ 
\pause 
for $j=0,\ldots,10$. 
\pause 
...
= \frac{1}{11} (d_j)^{n\overline{y}}(1-d_j)^{n(1-\overline{y})} 
\]
\pause
