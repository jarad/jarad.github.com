...
Then this pvalue app below will calculate (via simulation) the probability the null hypothesis is true.  
...
\[ \mbox{pvalue} \stackrel{?}{=} P(\mbox{data}|H_0\mbox{ true}) \pause \ne P(H_0\mbox{ true}|\mbox{data}). \]
\pause 
but we can use Bayes rule to show us that these probabilities cannot be equated
\[ p(H_0|y) = \frac{p(y|H_0)p(H_0)}{p(y)} \pause = \frac{p(y|H_0)p(H_0)}{p(y|H_1)p(H_1)+p(y|H_1)p(H_1)} \]
\pause
