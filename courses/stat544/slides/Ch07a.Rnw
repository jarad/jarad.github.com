...
\[ P(\theta=j/10|y) \propto \frac{1}{11} \prod_{i=1}^n (j/10)^{y_i}(1-j/10)^{1-y_i} = \frac{1}{11} (j/10)^{n\overline{y}}(1-j/10)^{n(1-\overline{y})} \]
\pause
If $j=0$ ($j=1$), any $y_i=1$ ($y_i=0$) will make the posterior probability zero.
...
ggplot(melt(d, id.var="theta"), aes(x=theta, y=value, color=variable)) + geom_point(size=5)
@
...
Let $Y\sim p(y|\theta)$ and $H_j: \theta = (E_{j-1},E_j]$ for $j=1,\ldots,J$. \pause Just calculate the area under the curve, i.e. 
...
\[ \theta \sim Be(1,1) \pause \quad \mbox{and thus} \quad \theta|y \sim Be(1+n\overline{y},1+n(-y)1-\overline{y}). \]
\end{frame}
...
\[ p(H_j|y) \pause = \frac{p(y|H_j)p(H_j)}{p(y)} \pause = \frac{p(y|H_j)p(H_j)}{\sum_{k=1}^J p(y|H_j)p(H_j)} \pause \propto p(y|H_j)p(H_j).  \]
\pause
...
\frame{\frametitle{Binomial model}
  ...
  &= \frac{0.5^n}{\frac{1}{Be(a,b)} \int_0^1 \theta^{a+n\overline{y}-1}(1-\theta)^{b+n-n\overline{y}-1} \theta} \pause\\
  &= \frac{0.5^n}{\frac{Be(a+n\overline{y},b+n-n\overline{y})}{Be(a,b)}} \pause \\
  &= \frac{0.5^n Be(a,b)}{Be(a+n\overline{y},b+n-n\overline{y})}
  \end{array} \]
...
Thus, in some sense $\alpha,\beta \to 0$ puts minimal prior data into the analysis. 

\vspace{0.2in} \pause If $\theta \sim Be(e,e)$, then 

\pause 

\[ BF(H_0:H_1) = \frac{0.5^n Be(e,e)}{Be(e+n\overline{y},b+n-n\overline{y})} \stackrel{e\to 0}{\longrightarrow} \infty \]
since $Be(e,e) \stackrel{e\to\infty}{\longrightarrow} \infty$. 
...
<<fig.width=8>>=
d = expand.grid(e=10^(0:-4), ybar=seq(0,1,by=0.01), n=10)
bf = function(n,ybar,a=1,b=1) exp(n*log(0.5)+lbeta(a,b)-lbeta(a+n*ybar,b+n-n*ybar))
...
ggplot(d, aes(y, post_prob_H0, color=factor(A))) + 
  geom_line() + 
  labs(y="Bayes Factor, BF(H0:H1)")
@
\end{frame}
