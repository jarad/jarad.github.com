...
&\propto \prod_{i=1}^n \myPhi(X_i'\beta)^{y_i}[1-\myPhi(X_i'\beta)]^{1-y_i} e^{-(\beta-\beta_0)^\top\mySigma_\beta^{-1}(\beta-\beta_0)/2} 
\end{array} \]
...
&= Pr(Y_i=1) \pause\\
&= Pr(Z_i>0) \pause\\
&= Pr(X_i'\beta+\epsilon>0) & \epsilon \sim N(0,1) \pause\\
&= Pr(\epsilon>-X_i'\beta) \pause\\
&= Pr(\epsilon<X_i'\beta) & \mbox{symmetry of standard normal} \pause\\
&= \myPhi(X_i'\beta) 
\end{array} \]
...
and the \emph{complete data likelihood} is
\[ p(y,Z|\beta) \propto \prod_{i=1}^n N(Z_i|X_i'\beta,1)\left[\I(Z_i>0)\I(y_i=1)+\I(Z_i\le 0)\I(y_i=0)\right] \]

\pause 

Thus the posterior distribution is 
\[ p(\beta,Z|y) \propto p(y|Z,\beta) p(\beta) = p(y|Z) p(Z|\beta) p(\beta) \]
\pause and we will derive the full conditionals for $p(\beta|Z,y)$ and $p(Z|\beta,y)$. 
\end{frame}
...
\[ \begin{array}{rl}
p(\beta|\ldots) \propto& p(y|Z) p(Z|\beta) p(\beta) \pause \\
\propto& p(Z|\beta)p(\beta) \\
=& \prod_{i=1}^n N(Z_i|X_i'\beta,1) N(\beta|\beta_0,\mySigma_\beta) \pause \\
...
\[ \begin{array}{rl}
\hat{\mySigma}_\beta &= [\mySigma_\beta^{-1} + X^\top X]^{-1} \pause \\
...
\[ \begin{array}{rl}
p(Z|\ldots) \propto& p(y|Z) p(Z|\beta) p(\beta) \pause \\
\propto& p(y|Z)p(Z|\beta) \\
=& \prod_{i=1}^n N(Z_i|X_i'\beta,1)\left[\I(Z_i>0)\I(y_i=1)+\I(Z_i\le 0)\I(y_i=0)\right] 
\end{array} \]
\pause  Thus the $Z_i$ are conditionally independent with distribution 
...
\end{array} \]
\pause where 
\[ \tilde{X} = [X\quad Zm] \qquad \tilde{\beta} = (\beta, u)^\top \]
where $X$ is the design matrix for fixed effects and $Zm$ is the design matrix for the random effects. 
\pause A common assumption is that the random effects are $u\sim N(0,\sigma^2 \I)$. \pause Thus the prior on $\tilde{\beta}$ is 
\[ \tilde{\beta} = 
\left( \begin{array}{c} \beta \\ u 
\end{array} \right) \sim 
N\left(\left[\begin{array}{c} \beta_0 \\ 0   \end{array} \right], \left[ \begin{array}{cc} \mySigma_\beta & 0 \\ 0 & \sigma^2 \I
\end{array} \right] \right) \]

\pause 

where the precision of the prior is 
\[ \left[ \begin{array}{cc} \mySigma_\beta & 0 \\ 0 & \sigma^2 \I
\end{array} \right]^{-1} = \left[ \begin{array}{cc} \mySigma_\beta^{-1} & 0 \\ 0 & \frac{1}{\sigma^2} \I
\end{array} \right] \]
\end{frame}
...
\[ 
p(Z,\beta,u,y,\sigma^2|y) \propto p(y|Z)p(Z|\tilde{\beta})p(\tilde{\beta}|\sigma^2)p(\sigma^2) 
\]
...
If we choose $\sigma\sim Unif(0,10)$, then 
\pause 

\[ \begin{array}{rl}
p(\sigma^2|\ldots) &\propto p(y|Z)p(Z|\tilde{\beta})p(\tilde{\beta}|\sigma^2)p(\sigma^2) \pause \\
&= p(\tilde{\beta}|\sigma^2)p(\sigma^2) \pause \\
%&= p(\tilde{\beta})p(u|\sigma^2)p(\sigma^2) \\
&\propto p(u|\sigma^2)p(\sigma^2) \pause \\
&\propto \prod_{u=1}^U N(u_i|0,\sigma^2) \frac{1}{\sigma}\I(0<\sigma^2<100) \pause \\
&\propto (\sigma^2)^{-U/2} e^{-\frac{1}{2\sigma^2}u'u} (\sigma^2)^{-1/2}\I(0<\sigma^2<100)  \pause \\
&= (\sigma^2)^{-\frac{U-1}{2}-1}e^{-\frac{u'u}{2\sigma^2}}\I(0<\sigma^2<100) 
\end{array} \]
\pause Thus $\sigma^2 \sim IG([U-1]/2, u'u/2)$ truncated to be smaller than 100. 
...
d = read.csv("Ch16a-cornFungus.csv")
d = d[d$leaf==1,]
...
We can model this using data augmentation by introducing a latent utility $Z_{ik}$ for each shopper-fruit combination. \pause Then the response is 
\[ Y_i = \mbox{argmax}_k Z_{ik} \]
\pause and there is great flexibility in how the $Z_{ik}$ are modeled. 
\end{frame}
...
\[ \begin{array}{rl}
Y_i &= \I(Z_i>0) \pause \\
Z_i &\stackrel{ind}{\sim} Logistic(X_i'\beta,1)
\end{array} \]
...
<<>>=
m = glm(low~., family=binomial(link=probit), data=birthwt[,-10])
r = BayesLogit::logit(m$y, X<-model.matrix(m))
