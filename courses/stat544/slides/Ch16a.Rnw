...
\pause and 
\[ Z_i \stackrel{ind}{\sim} N(X_i'\beta,1) \qquad \beta\sim N(b,B) \]
...
\[ p(\beta,Z|y) \propto p(y|Z,\beta) p(Z,\beta) \pause = p(y|Z) p(Z|\beta) p(\beta) \pause = p(y,Z|\beta)p(\beta) \]
...
=& \left[\prod_{i=1}^n N(Z_i|X_i'\beta,1)\right] N(\beta|b,B) \pause \\
=& N(Z|X\beta,\I) N(\beta|b,B)  
\end{array} \]
\pause and thus 
...
\hat{\beta} &= \hat{\mySigma}_\beta [B^{-1}b + X^\top Z]
\end{array} \]
...
\end{array} \]
\pause  Thus the $Z_i$ are conditionally independent \pause with distribution 
...
\pause A common assumption is that the random effects are $u\sim N(0,\sigma^2 \I)$. \pause Thus the distribution on $\tilde{\beta}$ is 
\[ \tilde{\beta} = 
...
\[ 
p(Z,\beta,\alpha,\sigma^2|y) \propto p(y|Z)p(Z|\tilde{\beta})p(\tilde{\beta}|\sigma^2)p(\sigma^2) 
\]
...
\[ \beta_j|\tau_j^2 \stackrel{ind}{\sim} N(b_j,\tau_j^2), \qquad \tau_j^2\sim \mbox{Inv}(v_j,B_j). \]
\pause
