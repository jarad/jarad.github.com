...
Let 
\[ \mySigma = \mbox{diag}(\sigma)\mO \mbox{diag}(\sigma) \]
where 
\begin{itemize}
\item $\sigma$ is a vector of standard deviations
\item $\mO$ is a correlation matrix
\end{itemize}
\pause
...
Now we can put whatever prior we want on $\sigma$ and $\mO$, e.g. $\sigma_i \stackrel{ind}{\sim} Ca^+(0,?)$. 
\end{frame}
...
\[ p(\mO) = |\mO|^{\eta-1} \]
where $\mO$ is a correlation matrix with implicit dimension $d$ and $\eta>0$ is the shape parameter.
