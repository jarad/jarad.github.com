...
The real question is whether this idea generalizes, i.e. is true for other representatives of these genotypes. \pause Let $\tilde{y}_A$ and $\tilde{y}_B$ be some future observation of seedling weight (on the same tray). \pause We might be interested in 
...
delta = rnorm(length(gamma1), gamma2-gamma1, sqrt(2)*sigmae)
as.data.frame(mcse(delta>0))
ddply(data.frame(q=c(.052,.5,.975)), .(q), function(x) as.data.frame(mcse.q(delta, q=x$q)))
@
