...
\frametitle{Classical regression}
  ...
  \[ \frac{[n-k]s^2}{\sigma^2} \sim \chi^2_{n-k} \pause \implies 1/s^2 \sim \mbox{Inv-}\chi^2(n-k,\sigma^2) \]
...
For the regression coefficients, in a default Bayesian regression, we have
  \[ \beta|y \sim t_{n-k}(\hat{\beta}, s^2(X'X)^{-1}),  \]
and in classical regression analysis, we have 
  \[ \hat{\beta} \sim t_{n-k}(\beta, s^2(X'X)^{-1}).  \]

\vspace{0.2in} \pause

For the error variance, in a default Bayesian regression, we have
  \[ \sigma^2|y \sim \mbox{Inv-}\chi^2(n-k,s^2) \]
and in classical regression analysis, we have
  \[ 1/s^2 \sim \mbox{Inv-}\chi^2(n-k,\sigma^2). \]
  
\vspace{0.2in} \pause

In the Bayesian statements, $\beta$ is random and $Y$ is fixed while in the classical statements, $Y$ is random and $\beta$ is fixed.
...
\frame{\frametitle{Fully conjugate subjective Bayesian inference}
	...
	\[ \beta|\sigma^2 \sim N(m_0, \sigma^2 C_0) \qquad \sigma^2 \sim IG(a_0,b_0) \]
	\pause then the posterior is 
	\[ \beta|\sigma^2,y \sim N(m_n, \sigma^2 C_n) \qquad \sigma^2|y \sim IG(a_n,b_n) \]
	\pause with
	\[ \begin{array}{rl}
	m_n &= m_0 + C_0X'(XC_0X'+\I)^{-1}(y-X m_0) \\
	C_n &= C_0-C_0X'(XC_0X'+\I)^{-1}XC_0 \\
	a_n &= a_0+n/2 \\
	b_n &= b_0+(y-X m_0)'(XC_0X'+\I)^{-1}(y-X m_0)/2
	\end{array} \]
}
%\end{comment}
...
\frametitle{Simulating from the posterior}
  ...
  &= \int \int \delta(\gamma-f(\beta,\sigma^2)) p(\beta|\sigma^2,y) p(\sigma^2|y) d\beta d\sigma^2 \\
  \end{array} \]
...
\[ \beta|\sigma^2 \sim N(m_0, \sigma^2 C_0), \sigma^2 \sim IG(a_0,b_0) \implies \]
\[ \beta|\sigma^2,y \sim N(m_n, \sigma^2 C_n), \sigma^2|y \sim IG(a_n,b_n) \]
