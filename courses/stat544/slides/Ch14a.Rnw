...
\frametitle{Classical regression}
  ...
  \[ \frac{[n-k]s^2}{\sigma^2} \sim \chi^2_{n-k} \pause \implies s^2 \sim \mbox{Inv-}\chi^2(n-k,\sigma^2) \]
...
and in classical regression analysis, we have
  \[ s^2 \sim \mbox{Inv-}\chi^2(n-k,\sigma^2). \]
...
\frame{\frametitle{Fully conjugate subjective Bayesian inference}
	...
	\[ \beta|\phi \sim (m_0, \sigma^2 C_0) \qquad \phi \sim IG(a_0,b_0) \]
	\pause then the posterior is 
	\[ \beta|\phi,y \sim (m_n, \sigma^2 C_n) \qquad \phi|y \sim IG(a_n,b_n) \]
	\pause with
	\[ \begin{array}{rl}
...
quantile(mx, c(.025,.5,.975))
@
This does not require any asymptotics or approximations.
\end{frame}
