...
\end{array} \]
The prior is 
\[ p(\rho_1,\ldots,\rho_6, \sigma_\alpha,\sigma_\beta) \propto Ca^+(\sigma_\alpha;0,0.1) Ca^+(\sigma_\beta;0,0.1) \]
}
...
<<compile-model, cache=TRUE, message=FALSE, dependson="model">>=
m <- stan_model(model_code = model)
@

\begin{frame}[fragile]
<<run-mcmc, dependson=c("data","compile-model"), cache=TRUE, echo=TRUE>>=
d = d[d$parity!=7,]
dat = list(y = d$nba, 
           parity = d$parity,
...
<<stan-summary, cache=TRUE, dependson="run-mcmc", echo=TRUE>>=
time
s = summary(r)$summary
@

<<dependson="stan-summary", eval=FALSE>>=
...
\begin{frame}[fragile]
<<sow, dependson="stan-summary", cache=TRUE>>=
...
\begin{frame}[fragile]
<<standard-deviations, dependson="run-mcmc", cache=TRUE>>=
...
Who is expected to have more number born alive: \pause
\begin{itemize}
...
{\small
Current sow (for parity $p$ and average grandsire):
\[ \begin{array}{rl}
E[\tilde{y}_s|y] &= E\large[E[\tilde{y}_s|\rho,\alpha,\beta]|y \large] \pause \\
&= E\left[ \left. e^{\rho_p+\alpha_s}\right|y\right] \pause \\
&\approx \frac{1}{K} \sum_{k=1}^K e^{\rho_p^{(k)}+\alpha_s^{(k)}}
\end{array} \]
\pause New sow (for parity 1 and average grandsire):
\[ \begin{array}{rl}
...
\begin{enumerate}
\item Obtain the $k^{th}$ \emph{joint draw} from the posterior for $\rho_p^{(k)}$, $\rho_1^{(k)}$, $\alpha_s^{(k)}$, and $\sigma^{(k)}_\alpha$. 
\item Calculate $\mu^{(k)}_{s} = e^{\rho_p^{(k)}+\alpha_s^{(k)}}$. \pause 
\item Calculate $\mu^{(k)}_{new} = e^{\rho_p^{(k)}+\alpha_{new}^{(k)}}$ where 
 $\alpha_{new}^{(k)} \sim N\left(0,\left[\sigma_{\alpha}^{(k)}\right]^2\right)$. \pause 
\item Calculate $\delta^{(k)} = \mu_{new}^{(k)} - \mu_s^{(k)}$.
\end{enumerate}

\vspace{0.2in} \pause

So $\delta^{(k)}$ is a realization of the expected difference in the number born
alive between a new sow and current sow $d$.
}



\begin{frame}[fragile]
<<zero-nba, dependson="run-mcmc", echo=TRUE>>=
sow_summary <- d |>
  group_by(sowid) |>
  summarize(
...
exp_nba_old2 <- exp(rhos[, 2] + alphas[, sow])
...
\begin{frame}[fragile]
<<zero-nba-run, dependson="zero-nba", echo=TRUE>>=
...
\begin{frame}[fragile]
<<parity-four, dependson="run-mcmc", fig.width=8, echo=TRUE>>=
...
exp_nba_old5 = exp(rhos[,5] + alphas[,sow])
...
Estimates of the expected number born alive for each sow with Monte Carlo 
uncertainty:

<<est-nba, dependson=c("zero-nba","parity-four"), echo=TRUE>>=
rbind(
  new  = as.data.frame(mcse(exp_nba_new )),
  old2 = as.data.frame(mcse(exp_nba_old2)),
  old5 = as.data.frame(mcse(exp_nba_old5))
) |> round(2)
@
...
<<difference-expected-parity, dependson=c("zero-nba","parity-four"), echo=TRUE>>=
rbind(
  `parity2 - new`     = as.data.frame(mcse( exp_nba_old2 - exp_nba_new  )),
  `parity5 - new`     = as.data.frame(mcse( exp_nba_old5 - exp_nba_new  )),
  `parity5 - parity2` = as.data.frame(mcse( exp_nba_old5 - exp_nba_old2 ))) |>
  round(2)
@
\end{frame}
...
Suppose the only cost difference is in the number born alive, \pause then $U(\tilde{y}_i) = u_1 \tilde{y}_i$ \pause and want 
...
\begin{itemize}
\item Old sow (parity $p$ and average grand-sire):
