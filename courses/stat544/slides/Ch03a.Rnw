...
In addition, the marginal distribution for $\mu$ is 
  ...
  &= \frac{\mathrm{\Gamma}([v+1]/2)}{\mathrm{\Gamma}(v/2)\sqrt{v\pi}s}\left(1+\frac{1}{v}\left[\frac{x-m}{s}\right]^2\right)^{-(v+1)/2} \pause \\
...
&= (\sigma^2)^{-n/2}\exp\left( -\frac{1}{2\sigma^2}\sum_{i=1}^n (y_i-\overline{y}+\overline{y}-\mu)^2 \right) \times \frac{1}{\sigma^2} \\
...
\[  p(\tilde{y}|y) = \int p(\tilde{y}|\mu,\sigma^2) p(\mu|\sigma^2,y) p(\sigma^2|y) d\mu d\sigma^2 \]
\pause
...
\item Simulate $\sigma^2 \sim \mbox{Inv-}\chi^2(v',(s')^2)$.
\item Using this value, simulate $\mu\sim  N(m',\sigma^2/k')$.
\item Using $\mu$ and $\sigma^2$ from above, simulate $\mu\sim N(\mu,\sigma^2)$.
\end{enumerate}
\end{frame}
