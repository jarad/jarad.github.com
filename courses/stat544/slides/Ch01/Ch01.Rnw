...
\frame{\frametitle{Conditional probability}
	\begin{definition}
	If $A$ and $B$ are events in $F$, and $P(B)>0$, \pause then the 
  ...
  \begin{example}[Craps conditional probability]
	\[ P(7|\mbox{Win}) \pause
	= \frac{P(7\mbox{ and Win})}{P(\mbox{Win})} = \frac{P(7)}{P(\mbox{Win})} = \frac{6/36}{8/36} = \frac{6}{8} \]
	\end{example}
}
...
\[ 
P(B) = \sum_{i=1}^\infty P(B\mbox{ and }A_i) = \sum_{i=1}^\infty P(B|A_i)P(A_i).
\]
\end{corollary}
...
Then 
\[ 
P(\mbox{Win}) \pause = \sum_{i=2}^{12} P(\mbox{Win} \mbox{ and }A_i) 
 = P(7) + P(11) 
 = \frac{6}{36} + \frac{2}{36} 
 = \frac{8}{36} 
 = \frac{2}{9}.
...
\frame{\frametitle{Bayes' Rule}
	...
	\[ P(7|\mbox{Win}) 
	\pause = \frac{P(\mbox{Win}|7)P(7)}{P(\mbox{Win})} 
	= \frac{1\cdot P(7)}{P(\mbox{Win})} = \frac{6/36}{8/36} = \frac{6}{8} \]
	\end{example}
}
