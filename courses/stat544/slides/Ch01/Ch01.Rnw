...
\frame{\frametitle{Kolmogorov's axioms of robability}
  \begin{definition}
	Given a sample space $\Omega$ and event space $F$, a \alert{probability} is a function $P:E\to \mathbb{R}$ that satisfies \pause 
	\begin{enumerate}[1.]
	\item $P(A)\ge 0$ for any $A\in F$ \pause
	\item $P(\Omega)=1$ \pause
	\item If $A_1, A_2,\ldots\in F$ are pairwise disjoint, then $P(A_1\mbox{ or }A_2\mbox{ or }\ldots) = \sum_{i=1}^\infty P(A_i)$. 
	\end{enumerate}
	\end{definition}
}
...
\frame{\frametitle{Conditional probability}
	\begin{definition}
	If $A$ and $B$ are events in $F$, and $P(B)>0$, \pause then the \alert{conditional probability of A given B}, written $P(A|B)$, \pause is 
...
\frame{\frametitle{Bayes' rule}
	\begin{theorem}
	If $A$ and $B$ are events in $F$, \pause then \alert{Bayes' rule} states
...
d = data.frame(x = seq(0,3,by=0.1)) %>% 
  mutate(prior = dgamma(x,a,b),
         "normalized model" = dgamma(x, 1, y),
         posterior = dgamma(x, a+length(y), b+sum(y)))

m = melt(d, id.var='x', variable.name="Distribution", value.name="density")
...
d = data.frame(x = seq(0,5,by=0.1)) %>% 
  mutate(prior = dgamma(x,a,b),
         "normalized model" = dgamma(x, length(y), sum(y)),
         posterior = dgamma(x, a+length(y), b+sum(y)))
...
But what about other measured variables, e.g. Caucasian, leaves in MN, of Scandanavian descent, etc. \pause Taken to its logical extreme, each probability becomes a statement about one single event\pause, e.g. for this individual. 
