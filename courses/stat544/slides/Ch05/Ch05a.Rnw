...
\frame{\frametitle{Andre Dawkin's three-point percentage}
  Suppose $Y_i = \sum_{j=1}^{n_i} Y_{ij}$ are the number 3-pointers Andre Dawkin's makes in season $i$, \pause and assume
\[ 
Y_i \ind Bin(n_i,\theta_i) \pause
\quad \mbox{or, equivalently,} \quad
Y_{ij} \ind Ber(\theta_i) \, i=1,\ldots,n_i
\]
  where
  \begin{itemize}[<+->]
...
\frame{\frametitle{Andre Dawkin's three-point percentage}
  Suppose $Y_i= \sum_{j=1}^{n_i} Y_{ij}$ are the number of 3-pointers Andre Dawkin's makes in \alert{game} $i$, and assume
\[ 
...
\]
  ...
  Do these models make sense?
  \begin{itemize}
  \item The 3-point percentage every \alert{game} is the same, i.e. $\theta_i=\theta$.
...
\frame{\frametitle{Hierarchical models}
  Consider the following model
	\[ \begin{array}{ll}
	y_{ij} &\stackrel{ind}{\sim} p(y|\theta_i) \pause \\
...
\frame{\frametitle{Three-pointer example}
	Let $Y_g$ be the number of three-pointers made in game $g$. 
	\pause 
	Assume
	\[ \begin{array}{ll}
	Y_g &\stackrel{ind}{\sim} Bin(n_g,\theta_g) \pause \\
	\theta_g&\stackrel{ind}{\sim} Be(\alpha,\beta) \pause \\
...
\frametitle{Decomposed posterior}
  \[ Y_g \stackrel{ind}{\sim} Bin(n_g,\theta_g) \quad \theta_g\stackrel{ind}{\sim} Be(\alpha,\beta) \quad \alpha,\beta \sim p(\alpha,\beta) \]
...
\pause
Thus $y_g|\alpha,\beta \stackrel{ind}{\sim} \mbox{Beta-binomial}(n_g,\alpha,\beta)$.
\end{frame}
