...
<<dawkins_data, out.width='0.7\\textwidth'>>=
...
@
  ...
  \item $\eta \sim LN(\Sexpr{m},\Sexpr{C})$
  \end{itemize}
  \pause where $LN(\Sexpr{m},\Sexpr{C})$ is a log-normal distribution, i.e. $\log(\eta) \sim N(\Sexpr{m},\Sexpr{C})$.
}
...
\begin{frame}[fragile]
\frametitle{Stan}
<<stan_run, dependson=c('informative_prior','stan'), echo=TRUE, results='hide'>>=
...
\begin{frame}[fragile]
\frametitle{Stan - default prior}
<<stan_post2, dependson="stan2_run", echo=TRUE, out.width='0.7\\textwidth'>>=
r2
@
\end{frame}
