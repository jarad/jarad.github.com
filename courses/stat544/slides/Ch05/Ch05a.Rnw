...
\frame{\frametitle{A prior distribution for $\alpha$ and $\beta$}
  ...
  \item $\eta \sim LN(0,3)$
  \end{itemize}
  \pause where $LN(0,3)$ is a log-normal distribution, i.e. $\log(\eta) \sim N(0,3)$.
}
...
n = 1e4
prior_draws = mutate(data.frame(mu  = runif(n, 0 , 1), eta = rlnorm(n, 0, 3)),
                      alpha = eta*   mu,
                      beta  = eta*(1-mu))
...
\begin{frame}[fragile]
\frametitle{Stan - improper prior}
<<stan2_plot, dependson="stan2_run", out.width='0.7\\textwidth'>>=
