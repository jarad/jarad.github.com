...
@
  ...
  \pause where $LN(\Sexpr{m},\Sexpr{C})$ is a log-normal distribution, 
  i.e. $\log(\eta) \sim N(\Sexpr{m},\Sexpr{C}^2)$.
...
theta23 = rbeta(length(samples$alpha),
              samples$alpha + d$made[game],
              samples$beta  + d$attempts[game] - d$made[game])
ggpairs(data.frame(theta22=theta22,theta23=theta23), lower=list(continuous='density')) + theme_bw()
@
\end{frame}
