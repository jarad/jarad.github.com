...
\frame{\frametitle{Exchangeability with covariates}
	...
	\item a distribution $p(y|\theta,x)$  such that $y_i\stackrel{ind}{\sim} p(y|\theta,x)$, and
	...
	\item a distribution $p(\theta|\phi,x)$  such that $\theta_i\stackrel{ind}{\sim} p(\theta|\phi,x)$, and 
...
\frame{\frametitle{Normal hierarchical models}
  Suppose we have the following model
	...
	p(\mu,\tau) &\propto p(\tau)
	\end{array} \]
	\pause with $j=1,\ldots,n_i$, $i=1,\ldots,\I$, and $n=\sum_{i=1}^J n_i$. \pause This is a normal hierarchical model. 
	
	\vspace{0.2in} \pause
	
	For the moment, we assume $\sigma^2=s^2$ is known for computational reasons.
}
...
\frame{\frametitle{Posterior distributions}
	The necessary conditional/marginal posterior are present in section 5.4 of BDA. Let 
...
Then
	...
	V_\mu^{-1} &= \sum_{j=1}^J \frac{1}{s_i^2+\tau^2} \pause & 
...
\frame{\frametitle{Simulation study}
  ...
  Use $p(\tau) \propto \mbox{Unif}(0,10)$. 
}
