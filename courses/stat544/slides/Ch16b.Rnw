...
&= E[u_1 \tilde{y}_{new} - u_1 \tilde{y}_d|y] \pause \\
&= u_1 E[\tilde{y}_{new} - \tilde{y}_d|y] \pause \\
&= u_1 E[\delta|y] \\
\end{array} \]
...
\frame{\frametitle{Utility functions}
Suppose the utility function also involves moving a new sow in, \pause then $U(\tilde{y}_i) = u_1 \tilde{y}_i - u_2\I(i=new)$ \pause and want 
\[ E[U(\tilde{y}_{new})|y] - E[U(\tilde{y}_d)|y] = u_1 E[\delta|y] -u_2 > 0 \] 
to pick $new$. 

\vspace{0.2in} \pause 

Now suppose an older sow (or this particular sow) needs more medications, \pause then $U(\tilde{y}_i) = u_1 \tilde{y}_i - u_2\I(i=new) - u_3\I(i=d)$ \pause and want 
\[ E[U(\tilde{y}_{new})|y] - E[U(\tilde{y}_d)|y] = u_1 E[\delta|y] - u_2 + u_3 > 0 \] 
to pick $new$.  
...
\[ \tilde{y}_d^{(k)} \sim Po\left(e^{\rho_p^{(k)}+\alpha_d^{(k)}}\right) \]
