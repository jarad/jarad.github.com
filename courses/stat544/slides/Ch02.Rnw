...
\item Quick review of probability
  ...
  \item Bayes' rule
  \end{itemize}
\item Applications of probability
  \begin{itemize}
  \item Down's syndrome
  \end{itemize}
...
\frame{\frametitle{Events}
  \begin{definition}
	...
	Six-sided die-tossing example: \pause
	\begin{itemize}
	\item $\Omega=\{1,2,3,4,5,6\}$ \pause
	\item Events:
		\begin{itemize}
		\item $A$: an odd number is rolled \pause
		\item $B$: a number less than 4 is rolled \pause
		\item $C$: a 1, 2, 3, 4, 5, or 6 is rolled
		\end{itemize}
	\end{itemize}
}
...
\frame{\frametitle{Pairwise disjoint}
	\begin{definition}
	Two events $A_1$ and $A_2$ are \alert{disjoint} (or \alert{mutually exclusive}) if both $A_1$ and $A_2$ cannot occur simultaneously. \pause The events $A_1, A_2,\ldots$ are \alert{pairwise disjoint} (or \alert{mutually exclusive}) if $A_i$ and $A_j$ cannot occur simultaneously for any $i\ne j$. 
	\end{definition}
	...
	\begin{itemize}
	\item $A_1$: an odd number is rolled, $A_2:$ an even number is rolled \pause
	\item $A_i$: $i$ is rolled for $i=1,2,\ldots,6$ 
	\end{itemize}
}
...
\frame{\frametitle{Axioms of probability}
	...
	Six-sided die-tossing example:
	\begin{itemize}
	\item $A_i$: $i$ is rolled for $i=1,2,\ldots,6$ \pause
	\item $P(A_i)=1/6$
	\end{itemize}
}
...
\frame{\frametitle{Conditional probability}
	...
	\item $A$: an odd number is rolled \pause
	\item $B$: a number less than 4 is rolled \pause 
	\item $P(A|B) = \pause \frac{2/6}{1/2} = \pause 2/3$
	\end{itemize}
}
...
\frame{\frametitle{Bayes' rule}
	...
	\[ P(A|B) = \frac{P(B|A)P(A)}{P(B)}\phantom{= \frac{P(B|A)P(A)}{P(B|A)P(A)+P(B|A^c)P(A^c)}} \]
	\end{theorem}
	...
	Six-sided die-tossing example:
	\begin{itemize}
	\item $A$: an odd number is rolled
	\item $B$: a number less than 4 is rolled \pause 
	\item $P(A|B) \pause =  \frac{2/3\cdot 1/2}{1/2} \pause =  2/3$
	\end{itemize}
}

\frame{\frametitle{Bayes' rule}
	\begin{theorem}
	If $A$ and $B$ are events in $S$, then \alert{Bayes' rule} states
	...
	\item $A$: an odd number is rolled
	\item $B$: a number less than 4 is rolled \pause 
	\item $A^c$: an even number is rolled \pause 
	\item $P(A|B) \pause = \pause \frac{2/3\cdot 1/2}{2/3\cdot 1/2+1/3\cdot 1/2} \pause = \frac{1/3}{1/3+1/6} \pause = 2/3$
	\end{itemize}
}



\section{Applications of probability}
