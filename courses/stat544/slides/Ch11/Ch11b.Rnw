...
length(unique(x[,2]))/length(x[,2]) # acceptance rate
@
\end{frame}
...
\[ \begin{array}{rl}
p(\sigma^2|\mu,\eta,\tau^2,y) \pause &\propto IG(n/2, \sum_{i=1}^\mathrm{I} \sum_{j=1}^{n_j} (y_{ij}-\mu_i)^2/2)Ca^+(0,b_\sigma) \pause \\
p(\tau^2|\mu,\eta,\sigma^2,y) \pause &\propto IG(\mathrm{I}/2, \sum_{i=1}^\mathrm{I} (\mu_i-\eta)^2/2) Ca^+(0,b_\tau)
\end{array} \]
\pause 
