...
\frametitle{Two component Gibbs sampler}
  Suppose our target distribution is $p(\theta|y)$ with $\theta=(\theta_1,\theta_2)$  \pause and we can sample from $p\left(\theta_1|\theta_2,y\right)$ and $p\left(\theta_2|\theta_1, y\right)$\pause. Beginning with an initial value $\left(\theta_1^0, \theta_2^0\right)$\pause, an \alert{iteration} of the \alert{Gibbs sampler} involves
	...
	\item Sampling $\theta_1^{t}\sim p\left(\theta_1|\theta_2^{(t-1)},y\right)$. \pause 
	\item Sampling $\theta_2^{t}\sim p\left(\theta_2|\theta_1^{t},y\right)$.
	\end{enumerate}
	
	By the Law of Large Numbers, $(\theta_1^{(t)},\theta_2^{(t)})$ converges to samples from $p(\theta|y)$. 
...
To sample from $p(\tau|y) \propto IG(\tau^2;a,b) Ca^+(0,b_\tau)$ (or equivalently $p(\sigma|y)$), we have a variety of possibilities. \pause Here are three:
\begin{enumerate}[<+->]
...
We will use a dependson to sample from $\theta_1,\ldots,\theta_n,\alpha,\beta$\pause, so we need to derive the following conditional distributions:\pause
\begin{itemize}
