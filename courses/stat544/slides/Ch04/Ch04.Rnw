...
\pause 
where $\J(\hat{\theta})$ is the observed information, i.e. 
\[ \J(\hat{\theta}) = -\frac{d^2}{d\theta^2} \log p(\theta|y)_{|_{\theta=\hat{\theta}}}. \]
\end{frame}
...
Since $\hat{\theta}\to\theta_0$ as $n\to\infty$, $\I(\hat{\theta}) \to \I(\theta_0)$ as $n\to\infty$. \pause And we have 
...
% \begin{frame}
% \frametitle{Posterior propriety - discrete data}
% 
% \begin{theorem}
% If the data are discrete and the prior is proper, then the posterior is proper.
% \end{theorem}
% 
% \pause
% 
% \begin{proof}
% We need to show $p(y)<\infty$ for all $y\in\mathcal{Y}$. 
% \pause
% \[ \begin{array}{rll}
% p(y) &= \int p(y|\theta)p(\theta) d\theta \pause \\
% &\le \sum_{y\in\mathcal{Y}} \int p(y|\theta)p(\theta) d\theta \pause \\
% &= \int \sum_{y\in\mathcal{Y}} p(y|\theta)p(\theta) d\theta & \mbox{TT} \pause \\
% &= \int p(\theta) d\theta) \pause \\
% &= 1
% \end{array} \]
% \end{proof}
% 
...
% 
% \begin{frame}
% \frametitle{Posterior propriety - continuous data}
% 
% \begin{theorem}
% If the data are continuous and the prior is proper, 
% then the posterior is \emph{almost always} proper.
% \end{theorem}
% 
% \pause
% 
