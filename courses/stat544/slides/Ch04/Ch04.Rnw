...
\pause 
\begin{theorem}
If the parameter space $\Theta$ is continuous and $A$ is in a neighborhood around $\theta_0$ with $Pr(\theta\in A)>0$, then $Pr(\theta\in A|y)\to 1$ as $n\to\infty$. 
\end{theorem}
...
\item Posterior median: $\approx \frac{a+y-1/3}{a+b+n-2/3}$
...
ggplot(mutate(plot_d, af = paste("a = b =",a), nf = paste("n =", n)),
  ...
  geom_line() + 
  facet_grid(nf~af) + 
  geom_vline(xintercept=0.3, style=3) + 
  theme_bw()
@
\end{frame}
