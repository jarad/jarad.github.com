...
\begin{frame}[fragile]
<<convergence_of_the_posterior_distribution_discrete, echo=TRUE>>=
n <- 1000
theta0 <- 0.3
d <- data.frame(
    n     = 1:n,
...
\begin{frame}[fragile]
<<convergence_of_the_posterior_distribution_continuous, echo=TRUE>>=
a <- b <- 1 # prior
...
\begin{frame}[fragile]
<<beta_binomial_consistency, echo=TRUE>>=
...
\begin{frame}
\frametitle{Bernstein-von Mises THoerem}
