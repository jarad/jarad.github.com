...
\item For {\bf group 1}, answer the following questions to 2 decimal place
	\begin{enumerate}
	\item Provide a 95\% credible interval for $\alpha$. (1 pt)
	
	\answer{(-3.32,0.27)}\bigskip
	
	\item Provide a 95\% credible interval for $\beta$. (1 pt)
	
	\answer{(-4.63,0.25)}\bigskip
	
	\item Provide a 95\% credible interval for the success probability if {\tt x} is zero. (2 pts)
	
	\answer{Because $e^\alpha/(1+e^\alpha)$ is monotonic, we can evalute the endpoints of the interval from part 3(b)i. in $e^\alpha/(1+e^\alpha)$ to get (0.03, 0.57).}\bigskip\bigskip\bigskip
	
	\item Provide a point estimate for the success probability if {\tt x} is one. (1 pt)
	
	\answer{Plug in medians for $\alpha_1$ and $\beta_1$ to get 
	\[
	...
	\answer{For iteration values $\alpha^{(j)}$ and $\beta^{(j)}$, calculate $e^{\alpha^{(j)}+\beta^{(j)}}/(1+e^{\alpha^{(j)}+\beta^{(j)}})$. Then take the 2.5\% and 97.5\% quantiles of these values.}
