...
\smallbrk\smallbrk\smallbrk

\item Calculate the probability of having Zika for an individual with a positive CDC test. (5 pts)

\longcorrect{
\[
P(D|+) = \frac{P(+|D)P(D)}{P(+|D)P(D) + [1-P(-|D^c)][1-P(D)]} = \frac{0.9\times 0.1}{0.9\times 0.1 + [0.2][0.9]} = \frac{1}{3}
\]
}
...
\longcorrect{The optimal diagnostic test is the one that will minimize expected costs. The expected costs for the CDC test is $50+10\times 0.01+1\times 0.1 = 0.78$ while the expected costs for the MD test is $10+10\times 0.03+1\times 0.36 = 0.76$. Thus the optimal test is the MD test.}
...
\[
p(y|\theta) = (\theta+1)y^\theta \I(0<y<1).
\]
...
\item Are you concerned about convergence of the Markov chain? Why or why not? (2 pts)
...
\item Are the groups or scenarios more variable? Explain your answer. (2 pts)

\vfill\vfill

\item Suggest a test statistic for which you could calculate a posterior predictive pvalue that could identify heavy-tails in the scenario means and defend this suggestion. (4 pts)
